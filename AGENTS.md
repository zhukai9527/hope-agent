...
- **IM KB 访问 opt-in（D10 红线放开，WS8）**：IM 默认仍零访问；放开靠**账号级 `ChannelAccountConfig.settings.kbAccessOptIn`（owner GUI-only，默认关）+ 群聊额外 per-chat 确认 `kbAccessChats`（群内 `/kb on`）**。判定走 `crate::channel::im_kb_access_allowed(channel_id, account_id, chat_id, is_group)`：DM 只需账号 opt-in；群聊需账号 opt-in **且** 该 chat 在确认列表；账号查不到 / channel_id 不匹配 → **fail closed**。IM 身份经 `ChannelKbContext{channel_id,account_id,chat_id,is_group}` 真接线透传：dispatcher（顶层 IM turn 填本轮身份）→ `ChatEngineParams.channel_kb_context` → `configure_agent` → `agent.channel_kb_context` → `ToolExecContext.channel_kb_context` → `KnowledgeAccessContext::resolve`（在此读 config 算出 `im_access_allowed` bool）；`subagent` 工具把父轮 `ctx.channel_kb_context` 透传给子 `SpawnParams.origin_channel_kb_context` → 子 `ChatEngineParams.channel_kb_context`，故 **IM-origin 子代理按 origin 账号/群聊判 opt-in，不洗权限**。`effective_kb_access` 的 IM gate `im_lineage_denied` 对 `im_access_allowed` 纯函数化（短路单测覆盖：opt-in 关归零 / DM 放行 / 群聊未确认归零 / IM-origin subagent 无 opt-in 归零 / opt-in 放行 / incognito 压过 opt-in）
...
- **作用域闭合（D11 + WS7）**：所有读写经 `WorkspaceScope::for_knowledge`，其 `read_only` 来自 `resolve_kb_dir` 返回的 `KbRoot{is_external,read_only}`——**外部绑定 root 默认 `read_only=true` 拒一切写（桌面也拒）；KB 级 `allow_external_writes` opt-in（owner GUI）后 `read_only=false` 解锁外部写**（冲突中止靠既有 stale-write guard 比磁盘 raw BLAKE3）。HTTP 写仍叠加 `allow_remote_writes`。**后台自主维护 `scheduler.rs` 按 `KbRoot.is_external` 跳过所有外部 root（无视 opt-in）——仅 GUI / agent 按需写外部**
...
- **块级引用（Batch G，仅 Obsidian `^block-id`）**：`parser` 扫块产 `ParsedBlock`（行尾 `^id`=`[A-Za-z0-9-]+` 附到所在叶块 / 独占行附上一块，首 id 胜），**不落表**——transclusion 切片 `note_read_ref::slice_by_anchor` 重解析、块反链 `db::block_backlinks` 查 `note_link.anchor`。`![[Note#^id]]` 切块、`![[Note#Heading]]` 切标题段（前端 transclusion 改传**全 ref**，不再 `cleanEmbedRef`；anchor 未命中降级整篇）；`[[ ]]` 提及注入（桥①）按 Obsidian 语义仍**整篇**——切片只对 `![[ ]]`。写 `note_assign_block`（`resolve_anchor_placement` 唯一文本命中后**解析整个叶块**：段落多行落块尾行不截块、列表/标题单行块、整块幂等检测、拒 frontmatter/代码围栏；`blake3(block_text)` 确定性生成 id 无 RNG，防重集走原始行扫）走与所有 KB 写**同款三闸门**：`writable_scope`（外部只读 / WS7）+ `guard_stale`（磁盘 raw BLAKE3）+ 唯一命中。**Logseq `((uuid))` / `id::` 不做**（大纲优先模型与文档优先底座冲突，`logseq/` 已在 `IGNORE_DIRS`）
- **原生大纲只读视图（Batch G，D8 可选层）**：`NoteEditorMode` 第 5 模式 `outline`，`outline.ts::buildOutline` 纯派生标题树（不改 `.md`）→ `OutlineView` 可折叠只读渲染，点标题经 `onOutlineJump` 由 KnowledgeView 切回 `source` 再 `setRevealTarget`。**红线：只读、永不替代 CM6 底座 / 不破坏性转写**（`NoteEditor` 在该模式于全部 hooks 之后 early-return，不挂 CM6 host；AI 改写 / outline 弹层在该模式隐藏）
- **图谱布局持久化（Batch J）**：拖节点钉住（`fx/fy`）+ 整体存 `sessions.db.kb_graph_layout` 表（registry.rs，**按 `rel_path` 键不用 index.db id**——id 随重建漂移；真相源 D9 非可重建缓存，`ON DELETE CASCADE` 随 KB 删）。owner 平面 `kb_graph_layout_{get,save}_cmd`（HTTP `GET|POST /api/knowledge/{kb}/graph/layout`，save body `{positions:[{relPath,x,y}]}` 整体替换、空数组=重置）。**纯 owner 平面**（GUI 画布操作，不经 `effective_kb_access`，无 agent 工具）
...
- **embedding 独立 selector（D7）**：知识空间有**自己的 `knowledge_embedding` selector**（`AppConfig.knowledge_embedding: EmbeddingSelection`——独立 enable / model / signature / reembed），与 `memory_embedding` 物理隔离、**不寄生不回退**（记忆没配 ≠ 知识空间没配，记忆关了知识空间向量检索照常）。两者**共享 `embedding_models` 命名库**（apiKey 配一次两处可选）+ 复用 `create_embedding_provider` 工厂 + signature 算法（知识索引经 `IndexDb` 裸调 `EmbeddingProvider`，**不复用** memory 的 `embedding_cache`——该表是 memory backend 内部的）。索引/检索读 `knowledge::knowledge_active_embedding_signature()`（**不读** `memory::active_embedding_signature`）；切模型走 owner 命令 `knowledge_embedding_{get,set_default,disable}`（后台 `KnowledgeReembed` job 全 KB full reindex）。`embedding_models` CRUD 对 memory **与** knowledge 的 active model 双向交叉保护。GUI 在「设置 → 知识空间」（`KnowledgePanel`，知识空间视图标题栏的**向量模型徽章** `KnowledgeEmbeddingBadge` 显示当前模型 / 「未开启」并可跳转；标题栏另置一个**齿轮按钮**作为「设置 → 知识空间」总入口——徽章与齿轮都调 `onOpenSettings` 跳同处，徽章专示向量状态、齿轮覆盖含精灵等其余设置，两者并存）；与 `memory_embedding` 一致**不进 `ha-settings`**（模型选择 + reembed 副作用，类比 `active_model` 的 GUI-only 豁免）
...
- **会话感知 KB 访问单一入口**：`Agent::resolve_kb_access()`（`agent/mod.rs`）是 agent 侧「本会话能碰哪些 KB」的唯一入口——复刻 source/origin/channel + WS8 fail-closed（共用 `note.rs::im_kb_context_from_session`，不再手抄）+ project_id 查询 + `effective_kb_access`（与 `note.rs::access_map` 同集，incognito/IM-opt-in/archived/external-cap 全应用）。`refresh_related_notes_suffix`（桥③）、no-KB 工具门控、`# Knowledge Bases` 系统提示段三处共用它，不得各自重写解析链。**按回合 memoize**（`kb_access_cache`，`reset_chat_flags`+`set_session_id` 失效）塌成单次解析；**只服务 schema/prompt/召回，绝不 gate 执行**（执行走 live `access_map`）
- **无 KB 不注入笔记工具（UX/省 token）**：`tools::is_kb_scoped_tool`（`note_*` + `session_to_note`，**不含 `knowledge_recall`**——跨 store 仍可用）在 `build_tool_schemas` + `tool_search` 后置过滤——`resolve_kb_access()` 为空则从 schema 剔除。纯 UX 优化，**不是安全边界**：执行层仍由 `note.rs::access_map` 兜底，门控必须与执行的访问集一致（同一 `resolve` 路径）
- **系统提示「# Knowledge Bases」段**：`build_full_system_prompt` 末尾按 `build_attached_knowledge_section` 追加**静态段**（cache-safe，仅 attach/detach 变动），列每库 `display_label` + 读/写 + 外部标记，库名转义防注入；`resolve_kb_access()` 为空则整段省略——绝不广告 `note_*` 会拒的库
- **检索结果来源（多库可辨）**：`NoteSearchHit` 带 `kb_name`/`kb_emoji`，`search::enrich_kb_names` 经 registry（D9 真相源，index.db 无名，缺失回退 `kb_id`）在 `search_notes`/`similar_notes` 后一次性填充；桥① 信封加 `kb="名"`（owner 名转义），桥③ **多库时**每行补 ` · 库名`（单库省，库名已在系统提示段）；前端 `KnowledgeResultCard` 把 `note_search`/`knowledge_recall`/`note_similar` 结果按库分组渲染（recall 的 memory/notes 仍两段不混排）
- **读取桥①**：用户消息 `[[note]]` 经 `knowledge::inject` 确定性注入，套 `<untrusted_external_data>` 信封 + 来源（`kb=` 友好名 + `source` 机读 id/path）+ 截断，受 `effective_kb_access` 约束，**永不提升为 system 指令**
- **读取桥③ 被动相关笔记（Batch H，D7，opt-in 默认关）**：`AppConfig.knowledge_passive_recall`（MEDIUM，三件套齐全）。`agent/related_notes.rs` 镜像 active_memory（slot + TtlCache，但**无 LLM**）：每轮 `refresh_related_notes_suffix` 经 agent 线接的 source/origin/channel 重建 `KnowledgeAccessContext` → `effective_kb_access` → `search_notes` top-N → 渲染「## Related Notes」**只给标题**套 untrusted 信封 → `related_notes_suffix` slot → `RoundRequest` → 四 adapter（**Anthropic 走 plain block 无 cache_control，4-breakpoint 已满**；OpenAI*/Codex 加 system message）。incognito 零召回、IM 未 opt-in 零访问（access 链同 `note_*`）
- **`knowledge_recall` 工具（Batch H，D7）**：一次查 memory + 笔记两 store，**两段独立排序不归一化混排**，薄编排器**绝不折进/改动 `recall_memory`**；KB 段过 `accessible_kbs`、memory 段 incognito 跳过；`Standard{default_deferred:true}` 经 `tool_search` 发现
- **外部 vault 实时同步（D6）**：`notify` watcher（per-KB 线程 + debounce）+ bind/启动 reconcile（mtime 增量 + prune）。内部 `notes/` 写时同步索引
- **侧边栏 AI 对话面板（知识空间内嵌对话）**：知识空间右栏「反向链接 ↔ AI 对话」分段切换。对话 = `kind='knowledge'` 的会话（`SessionKind::Knowledge`，`sessions.kind` 列），**从主会话列表 / `/sessions` picker / 全局 FTS 隐藏**（后端 `list_sessions_paged_inner` + `search_messages` 过滤 + `is_regular_chat()`）；锚定信息落 `knowledge_chat_threads(session_id,kb_id,anchor_note_path,created_at)`（sessions.db 真相源 D9，随 session/KB 级联删）。**懒创建走 chat 命令 auto-create 分支**：首条消息带 `tool_scope="knowledge"` + 单条 draft `kbAttachments`(write) + `kb_anchor_note` → `service::mark_session_as_kb_thread` 设 kind + 建 thread（无空会话、无闭包竞态）。owner 命令 `kb_chat_thread_get_cmd`(默认加载某笔记最近对话) / `kb_chat_threads_list_cmd`(历史 + FTS 搜索)。前端复用主对话 `useChatStream`（新增 `toolScope`/`getExtraAttachments`/`draftKbAnchorNote` 三个可选 prop，主对话不传=行为不变）+ `MessageList`/`ChatInput`/`ApprovalDialog`；`useKnowledgeChat` 管 thread 生命周期 + 模型/Agent 态。当前文档每轮经 `getExtraAttachments` 作为 `source:"quote"` 注入（cache-safe，绝不进 system 静态前缀）。
- **工具精简 `ToolScope`（与 source/D10 正交）**：`ChatEngineParams.tool_scope: Option<ToolScope>`（`tools/mod.rs`）；`Knowledge` 在 `build_tool_schemas` 收尾 + `build_full_system_prompt` 的 eager/hints 块按 `is_knowledge_scope_tool` 白名单 `retain`（全 `note_*` + `knowledge_recall` + 记忆 + 框架基础 `skill`/`tool_search`/`ask_user_question`/`runtime_cancel`/`job_status`）。**纯 schema/prompt 可见性收窄，绝不动 KB 访问**——source 仍是 Desktop/Http，访问仍由 `effective_kb_access` 单点裁决（incognito/attach/D11 全照旧）。`configure_agent` 透传 → `agent.set_tool_scope`。
- **选区针对性编辑（替代旧 AI 改写弹窗，`AiRewriteDialog` 已删）两路并存**：(1) 选区→输入框上方可删除 quote chip（走 `useChatStream.pendingQuotes`）→ 进对话由 AI 用 `note_patch`/`note_update` 改写，工具结果带 `FileChangeMetadata`（`tools/note.rs::emit_note_diff`，`language:"markdown"`）→ `ToolCallBlock` 内联 diff；落盘后 `knowledge:changed` 触发编辑器重载当前笔记（脏态跳过）。(2) 浮动 `QuickRewriteBar`：一次性、不进对话历史，走重做后的 `kb_ai_rewrite_cmd`（接 `model_override`，默认跟随对话模型、可单选）→ diff 预览 → 应用；每次结果经 `kb_rewrite_log_cmd` 落 `learning_events`(`kind="kb_quick_rewrite"`) 做统计。
- **精灵 / 灵感模式（Phase 2，`sprite/`，默认关）**：知识空间专属的**主动型**陪伴。**不是** dreaming/maintenance 那种后台 app-idle 轮询循环——精灵反应的是「用户当前正在编辑的那篇文档」，只有前端知道，故走**前端编辑空闲触发 → `kb_sprite_observe_cmd` → `sprite::observe_and_maybe_speak`（节流 + side_query + emit）→ `sprite:suggestion` 事件 → 前端瞬态气泡**。只复用 dreaming/maintenance 的**串行锁 + side_query 范式**，无 cron/idle ticker/app_init 接线。三层节流：`SPRITE_RUNNING` CAS 串行锁 + 每 key（sessionId 否则 notePath）`cooldown_secs` + 文档 hash 去重 + `max_per_session_per_hour`（节流闸先于 LLM）。上下文融合（各 `senses.*` 开关）：当前文档 + 最近编辑（前端粗 diff）+ 对话上下文（前端送最近消息）+ 记忆召回（`active_memory::shortlist_candidates`，scope=Agent+Global）+ 跨会话感知（`awareness::collect::collect_entries`）→ `build_analysis_agent` 的 `side_query`（不复用主对话 cache）→ 解析 `{category,text}`（`none`/空=沉默）。**提示词（persona + 指令）内置英文常量 `context.rs::PERSONA`，不外露配置**。**incognito 零精灵**两道关卡：后端 `observe_and_maybe_speak` 首查 `is_session_incognito` 短路；前端 incognito 同样不触发。气泡**瞬态、不进消息流**（`SpriteBubble` 渲染于 composer 上方，「回应」把内容塞进输入框）。**开关两处并存**：对话面板顶 ✨ 按钮 + 设置面板（`SpriteSection`）的「启用精灵模式」开关都直接翻 `SpriteConfig.enabled`，**即时持久化**（乐观更新 + `sprite_config_set_cmd`，失败回滚）。两处经 `config:changed` 互相回流——**注意 EventBus 的 `config:changed` payload 的 `category` 不可靠（主写路径 `save_config` 恒 `"app"`，真实 category 只进 hooks 子系统；user-config / rollback 路径另有 `"user"`/`"__rollback__"` 值），故监听一律不得按 category 过滤**，收到任意 config 变更即重拉 `sprite_config_get_cmd` 同步。enabled **不进** `SpriteSection` 的 Save/dirty 流程（dirty 经 `tuningJson` 排除 enabled，调参 Save 独立）；设置面板 `toggleEnabled` 在**磁盘最新值**之上只改 enabled 持久化，不把未保存调参冲刷落盘。owner 命令 `kb_sprite_observe_cmd`（fire-and-forget，spawn）/ `sprite_config_{get,set}_cmd`，**非 agent 工具**。`SpriteConfig` 挂 `AppConfig.sprite`，设置三件套（`SpriteSection` GUI + `tools/settings.rs` 的 `sprite` 分支 MEDIUM + SKILL.md），`clamped()` 钳值。
