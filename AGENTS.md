...
- **异步 Tool 执行**：`exec` / `web_search` / `image_generate` 标 `async_capable=true`；落 `~/.hope-agent/async_jobs.db` + spool；`job_status` 多作业管理面（`action ∈ status|list|wait|cancel|result`，`tool_job_status(args, session_id)`）——`list` 枚举本会话在途、`wait{ids?,mode,timeout}` 短便利同步（clamp ≤10s，超时回 `still_running` 不长阻塞）、`cancel(id)` 跨进程取消。**长 fan-out 等齐的正道仍是注入而非 `wait`**（Group 合并注入待 R1）
