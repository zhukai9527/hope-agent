...
- **28 事件**：24 个真触发（阻断 / 决策型 `UserPromptSubmit`/`PreToolUse`/`PreCompact`/`PermissionRequest` + 20 个观察型），4 个协议保留（`WorktreeCreate`/`WorktreeRemove`/`TeammateIdle`/`InstructionsLoaded`——无对应概念，可配置不 dispatch）。`is_observation_only` 列表里的事件 `block` 决策降级为非阻断 + log。**`PermissionRequest` 决策型**：hook 返回 `hookSpecificOutput.decision.behavior`（allow/deny）经幂等 `submit_approval_response` 接管审批，与 GUI/IM 竞争——单一入口 `hooks::dispatch_permission_request`（`mod.rs`，由 `tools::approval` spawn，deadline 钳到 `approval_timeout_secs`）
