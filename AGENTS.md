...
- **后台任务统一模型（Background Job，R1）**：异步工具子系统升格为统一后台任务模型——表/文件 `background_jobs`（`JobKind = Tool | Subagent | Group`，今日仅 `Tool` 落地，`Subagent`/`Group` 是 R6/R5 填充的类型化接缝），**单一入口 `async_jobs::JobManager`** front 全部 spawn / cancel / list / replay / schedule（`spawn_explicit_job` 等收敛为其 `pub(crate)` Tool executor 内部，新增后台单元一律加 `JobManager` 方法而非平行 API）。模块名 `async_jobs/` 与 log category `"async_jobs"` 按「沿用血脉演进」保留；`RuntimeTaskKind::AsyncJob` / `async_tool_job:*` 事件不变（R3/R4 再统一前缀）；新列（group_id / subagent_run_id / progress 等）待对应 slice 消费再加（纯缓存 drop-rebuild，延后零成本）
- **异步 Tool 执行**：`exec` / `web_search` / `image_generate` 标 `async_capable=true`；落 `~/.hope-agent/background_jobs.db` + spool；`job_status` 多作业管理面（`action ∈ status|list|wait|cancel|result`，`tool_job_status(args, session_id)`）——`list` 枚举本会话在途、`wait{ids?,mode,timeout}` 短便利同步（clamp ≤10s，超时回 `still_running` 不长阻塞）、`cancel(id)` 跨进程取消。**长 fan-out 等齐的正道仍是注入而非 `wait`**（Group 合并注入待 R5）
- **运行中输出尾巴 `output_tail`（R3 ①）**：后台 `exec` job 的 stdout/stderr 实时 tee 进进程本地有界 ring（`async_jobs::output_tail`，默认 8KB），`job_status(status)` 对 running job 返回 `output_tail`。**加法式**：仅 `ctx.output_tail_job_id` 为 Some（后台 + 非 incognito exec）才走 `wait_with_output_teed`，**前台同步 exec 不动**；ring 在 finalize / 队列取消 / 会话删三处清，incognito 永不注册
