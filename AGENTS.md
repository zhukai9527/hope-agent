...
- **下一代 Dreaming Context Pack 注入**：高 salience 的 active claim 作为 `## Pinned Memory` 段并入 system_prompt **静态 prefix**（query-independent，复用首个 cache breakpoint——不新开动态 cache block，Anthropic 4 breakpoint 已满）；query-dependent 的动态 claim 召回交给 Active Memory v2，故本期**不引入 provider `dynamic_context_blocks`**。单一来源 dedup `covered_by_active_claim_memory_ids`（`hidden` 的正向镜像、与之互斥）把被 active managed claim 覆盖的 legacy memory 从 `# Memory` 段排除——**去重阈值必须对齐 Pinned 注入阈值 `PINNED_MIN_SALIENCE`（`context_pack.rs` 单一来源常量）：dedup 永不比注入更激进，否则中等 salience（`[0.5,0.7)`）claim 的影子记忆既被 dedup 出 legacy、又够不到 Pinned = 无 prompt 出口**；`user_pinned` link / `memories.pinned=1` 豁免；claim 内容（LLM 派生）进 prefix 前必过 `sanitize_for_prompt`。Profile / Pinned 段纳入 `effective_memory_budget` 同一预算池（Core > Pinned > Profile+legacy）。Active Memory v2（`ActiveMemoryConfig.include_claims`，per-agent / 走 agent.json 不进 `ha-settings`，默认关）把当前 turn 召回候选扩到 effective-active claim（过期 / superseded claim 不经此回灌）。incognito 全链归零
- **下一代 Dreaming 用户纠错闭环（Lucid Review）**：唯一编排入口 `claims::review`——`update_claim`（PATCH 语义：edit content/triple/tags、status approve→active / reject→archived / mark-outdated→expired / flag→needs_review、move scope、pin/unpin 经 salience 越过 `PINNED_MIN_SALIENCE`）+ `forget_claim`（archive / permanent）。**decision-type 由 `resolve_update` 纯函数从 diff 派生（status > scope > pin > edit 优先级），无 DB 可穷举单测**；底层原语在 `claims::store`（`apply_claim_fields` **any→any 状态**，非 resolver 的 active-gated `set_claim_status`）。**红线**：① 每个用户操作落 `dreaming::record_user_action`（`trigger=user_correction` 完成态 run + 单 decision，「所有用户操作有 decision log」，FK 进 `dreaming_decisions`）；② approve/edit/reject/expire/move_scope 写最高权重 `manual_correction` evidence（approve 用 `user_confirmed`、`raw_allowed` 不脱敏），pin/unpin/flag 不写；③ content 变更后 `reembed_claim`（须先 drop write_conn——`embed_and_index_claim` 内部再取写锁）使下一轮召回反映新文本；④ forget archive 翻 `archived` + 把本 claim 的 link 转 `managed` + 仅独管的 memory `pinned=0`（让 read-time hidden-set 覆盖），permanent 删 claim 图谱（claim+evidence+link+vec0 手删 rowid，FTS 由 `memory_claims_ad` 触发器维护）+ 仅独管的 orphan memory；⑤ 发 `memory:claim_changed`（+ `memory:review_required` flag 时）。owner 平面 `claim_update` / `claim_forget`（Tauri 扁平参数对齐 HTTP `PATCH /api/claims/{id}` body / `POST /api/claims/{id}/forget`，API key / 本机信任，**无 agent 工具面**——claim 纠错只对用户开放，模型不能自改）；GUI 走可复用 `ClaimReviewActions`（Dashboard Needs Review 队列 + Settings ClaimsBetaView 详情）
- **下一代 Dreaming 确定性评测（deterministic layer）**：`memory/dreaming/eval.rs` golden-fixture 调度器 + `crates/ha-core/tests/fixtures/dreaming/*.json`（7 个，每个限定唯一 scope 命名空间避免共享 DB 串扰）+ `tests/dreaming_eval.rs` 集成测试（进程隔离独占 claim store global，跑默认 CI）。**无 LLM**——只测安全红线（作用域隔离 / 过期抑制 / 证据可追溯 / 冲突进待审 / legacy-sync 隐藏 / 证据 fail-closed），LLM 抽取 / profile 合成属「Golden LLM fixtures」层走 nightly 不进此处。**契约**：改动 claim 读路径 / effective-status / hidden-set / scope 过滤 / evidence 授权等安全红线时，须在 fixtures 加 case 或保既有 fixture 绿（`evaluate` 经公共 API 跑真实读路径，非重写逻辑）
...
- **审批授权来源审计**：每个后台 job 的 `background_jobs.approval_origin` 列记录授权方式（`ApprovalOrigin`：user / timeout_proceed / yolo / auto_approve / external_pre_approved / policy_allow），由审批闸单点算出写入 spawn ctx（exec + 非 exec 全覆盖）
...
- **后台任务统一模型（Background Job，R1）**：异步工具子系统升格为统一后台任务模型——表/文件 `background_jobs`（`JobKind = Tool | Subagent | Group`，三类均已落地），**单一入口 `async_jobs::JobManager`** front 全部 spawn / cancel / list / replay / schedule（`spawn_explicit_job` 等收敛为其 `pub(crate)` Tool executor 内部，新增后台单元一律加 `JobManager` 方法而非平行 API）。模块名 `async_jobs/` 与 log category `"async_jobs"` 按「沿用血脉演进」保留（**诊断按 `category='async_jobs'` grep,DB 却是 `background_jobs`**）；EventBus 事件已统一 `job:*` 前缀（R3，见下）；`RuntimeTaskKind::AsyncJob` 内部枚举名不变；`progress` / `priority` / `attempt` 等列待对应 slice 消费再加（纯缓存 drop-rebuild，延后零成本）
- **异步 Tool 执行**：`exec` / `web_search` / `image_generate` 标 `async_capable=true`；落 `~/.hope-agent/background_jobs.db` + spool；`job_status` 多作业管理面（`action ∈ status|list|wait|cancel|result`，`tool_job_status(args, session_id)`）——`list` 枚举本会话在途、`wait{ids?,mode,timeout}` 短便利同步（clamp ≤10s，超时回 `still_running` 不长阻塞）、`cancel(id)` 跨进程取消。**长 fan-out 等齐的正道仍是注入而非 `wait`**（`batch_spawn` 的 Group 等齐后合并注入一轮，R5）
- **后台工具并发两层硬配额（R7.1，`async_jobs::slots`）**：全局 `async_tools.max_concurrent_jobs`（默认按核数 `clamp(cores-2,4,16)`）+ 每会话 `max_concurrent_jobs_per_session`（**默认按硬件取值 ≈ 全局 cap 的 3/4、band [3,12]、恒 < 全局**——固定值会在低核机上 ≥ 全局 cap 而 no-op,`0` = 不限）。`SlotManager::reserve_inner` 两层都要有空位才发 `SlotReservation`，否则**入队**（status `Queued`,非拒绝);调度器 `pick_fair_index` 在已达每会话上限的会话上**跳过**(防 head-of-line),RAII Drop 释放槽 + 唤醒调度器。**auto-background detach 占槽**靠 `slots::reserve_forced`（任务已在跑、不能排队/拒绝,只强制计数,可短暂超全局 **及每会话** cap——per-session cap 约束的是新后台任务的**准入**,不是已在跑的 auto-detach 任务数),经 `Arc<Mutex<Option<SlotReservation>>>` 随 worker 线程结束释放。**调度器/队列 per-process**（队列持不可持久化 live ctx,每进程一个 tier-agnostic 调度器,与 `replay_pending_jobs` 的 Primary-only 相反)。**per-kind 双域分治不进单一 struct**:tool 池在此,subagent 池在 `subagent::queue`（per-session **排队**,默认 8 `subagents.maxConcurrent`,R7.2),同轮安全工具走信号量 8（R7.3),结构类（depth/batch/turn）拒绝不排队——勿合并成一个 per-kind 配额表（speculative generality）
- **后台 subagent reject→queue 真统一（R7.2,`subagent::queue`）**：subagent 命中 per-session 并发上限不再 `Err`,而是 **PARK 为 `SubagentStatus::Queued`**(新变体,**非终态、被 `count_active_subagent_runs` 的 `IN('spawning','running')` 排除**——若计入会 deadlock 自身提升),由独立**焦点队列**(非 generify tool-job `SlotManager`——其 `PreparedJob` 死耦 `ToolExecContext`,subagent 限额模型不同:per-parent-session、无全局池、`tokio::spawn` 跑 `run_chat_engine`)承载 live `SpawnParams`(`MAX_QUEUED_SUBAGENTS=256` 上限,满则硬拒)。**结构类(depth/agent-exists/batch/session/capability)仍硬拒**(`structural_limit_tests` 守),只资源类(过并发)排队。`spawn_subagent` 拆 validate→(enqueue|`launch_subagent_run`);**per-process 调度器** `run_subagent_scheduler`(幂等,镜像 `run_scheduler`:`Notify`+5s tick)在 `update_subagent_status` 终态 choke point 唤醒(slot 可能释放)→ 每会话低于 `max_concurrent_for_agent` 则 `Queued→Spawning`+launch,promote 前重查仍 `Queued`(输给并发 cancel)。**取消** parked run:`request_cancel_run` 先 `queue::remove_for_run`(丢 live params)再 stamp Killed;**会话删/焚** `cleanup_session` 调 `queue::purge_for_session`(projected 经 `cancel_for_session` 已 dequeue,这步兜底 incognito/未投影的 + 防 promote 进已删会话);**重启** `cleanup_orphan_subagent_runs` 扫 `'queued'`→Orphaned(内存队列丢失,镜像 tool-job Queued→Interrupted)。**R5 Group 零特例**:queued grouped child 投影 `Queued`(非终态)带 group_id,`try_complete_group` 自然等它;promote 后跑→结算→重查 group。**queued 不发 subagent_event**(纯 DB+投影,经 `job_status list`/面板 refetch 可见;launch 时才 emit `spawned`)——避免前端新增 queued 渲染态
- **后台 job 重试（R7.4,`async_jobs::retry`）**：瞬时失败自动退避重投,**默认关闭(opt-in)**——纯策略 `decide(tool,attempt,error,cfg)` 仅对 `JobError::Failed`（干净 dispatch 错误、job token 未动）且**幂等可安全重跑工具**（`is_retry_eligible` 代码级白名单 = `web_search`/`web_fetch`,**非用户旋钮**）重投;`exec`/`image_generate`（副作用/非确定性）、`Cancelled`/`DeniedByUser`/`TimedOut` 永不重投。**默认 false**:eligible 多为**计费**供应商(`web_search`),重投会重计费,故交用户按需开启。`run_tool_once`（单次 dispatch+timeout+cancel,explicit + auto-bg 共用）+ `run_tool_with_retry`（loop + 可取消退避,job-level cancel 期间停重投;`max_attempts` 钳到 10;eligible 工具不得带 output_tail ring,debug_assert 守）。`max_job_secs` 是 **per-attempt** 预算(重投各一份)。配置 `async_tools.{retryEnabled,maxRetryAttempts}`（MEDIUM）。**新增 async_capable 工具若有副作用或计费,务必不要进 `is_retry_eligible`**
- **统一 `job:*` 事件命名空间（R3）**：所有后台任务生命周期走 `async_jobs::events` 发 `job:{created,updated,progress,completed}` + `job:mark_injected_failed`（替代旧 `async_tool_job:*`，破坏性 drop），kind-tagged（`tool` / `group`）+ `session_id`。`progress` 目前 Group 报 N/M 子完成。**`subagent` kind 沿用更丰富的 `subagent:*` 流不双发**；R4 面板合并两路 + `job_status list`。新增后台任务事件一律走 `events` helper，勿散落 `bus.emit`
- **后台 exec 命中审批点 park 为 `AwaitingApproval`（R8，`async_jobs::approval_bridge`）**：显式 `run_in_background` 的 exec **不再脱钩前同步审批**（`should_run_exec_reorder_gate` 仅 `AutoBackgroundEligible` 跑——保 ASYNC-2「审批不计入 `auto_background_secs`/`max_job_secs`」预算）——其命令 gate 落到后台 job 线程,attended 审批经 **thread-local 桥**翻转 job 行 `Running⇄AwaitingApproval`：`tools::approval` 定义 `BackgroundApprovalScope`/`BackgroundApprovalBridge`（**tools 零依赖 async_jobs**——runner 在 `run_job_to_completion` 注入闭包回调 job DB）,`check_and_request_approval` attended `rx.await` 前 `notify_bg_park`、`BgResumeGuard` Drop 兜底 resume（覆盖 resolve/timeout/**cancel-drop**)。批准续跑、拒绝/超时-deny **终态 `DeniedByUser`**（折进 `Failed` 状态,STOP 文本经注入保留)——**R8 验收据此 revise ASYNC-1「拒绝的 exec 不成 job」**(显式后台 exec 拒绝现成一条终态 job)。unattended/strict 仍 fail-closed deny（内层 gate 在 `rx.await` 前返回,**不 park**)。审批纯内存:**重启 parked job → `Interrupted`**（`list_running` active 集已含 `awaiting_approval`,无需新持久化）;**parked job 持槽不释放**（避免 resume 无空槽死锁,审批 `approval_timeout_secs` 兜底释放——**若用户同时关掉 approval-timeout 且 `max_job_secs=0`,parked job 会一直占槽直到答复/取消,属显式「全 timeout 关」的取舍**);**job 预算 timer 排除 parked 时长**（`run_tool_once` 经 `approval_bridge::parked_budget_extension` 把审批等待移出 `max_job_secs`,守 ASYNC-2:审批不计入执行预算);cancel parked job → cancel_job 即时 `dismiss_parked_job_approval`（掉 sender→唤醒 rx,命令 gate 见 cancellation 不批准、弹窗立消、不死等 5s grace,闭合「grace 内点 Allow 跑已取消命令」窗口）+ token trip 使 job `Cancelled`;cross-process cancel 走 `on_resume` 的 `dismiss_parked_job_approval` 拆孤儿审批弹窗（**remove-if-present 才 emit**,在 future 已 drop 后跑、不竞 `select!`);`set_approval_origin` 修正 spawn 时占位 origin(F6 审计)。**后台 subagent 内层审批的投影 label**（子 session 跑、不经 thread-local 桥）由 EventBus watcher 补齐(见下)。`mark_cancelling`/`update_terminal` WHERE 已含 `awaiting_approval`,新增非终态状态须同步这些 active filter
- **后台 subagent 内层审批投影 watcher（R8 follow-up，`async_jobs::approval_projection_watcher`）**：后台 subagent 在子 session 跑自己的回合,内层工具审批**不经** R8 的 thread-local 桥(桥只覆盖 `run_job_to_completion` 的 `kind=Tool` job)。改由 EventBus watcher 补投影 label:订阅 `approval_required`(park)/`approval:resolved`(resume,**两事件 session 字段大小写不同**:前者 `session_id` snake、后者 `sessionId` camel,watcher 两者都认),经 `JobManager::reflect_subagent_inner_approval(child_session, parked)` 把审批的 child session → `SessionDB::find_active_run_by_child_session`(active run)→ `JobsDB::get_subagent_projection`(投影行)→ 复用 R8 的 kind 无关 `mark_awaiting_approval`/`resume_from_awaiting_approval` 翻 `running⇄awaiting_approval` + emit `job:updated`。**纯投影 label、绝不 gate 执行**(内层审批照旧 block-and-wait);非 subagent 审批(前台 / R8 后台 exec——其审批带的是**父** session)、未投影的 internal/incognito run 全部 fall-through no-op;status WHERE 守卫使终态投影 / 重复事件安全。app_init 两路各 spawn 一次(进程内幂等)
- **运行中输出尾巴 `output_tail`（R3 ①）**：后台 `exec` job（**显式 `run_in_background` 与 auto-background 两路**）的 stdout/stderr 实时 tee 进进程本地有界 ring（`async_jobs::output_tail`，大小 = `async_tools.output_tail_bytes`，默认 8KB，R9；**ring cap 在 job 起跑时按 `configured_bytes()`（读时钳 `[256,1MB]`）快照,mid-run 改配置不影响在跑 ring**），`job_status(status)` 对 running job 返回 `output_tail`。**加法式**：仅 `ctx.output_tail_job_id` 为 Some（exec + 非 incognito）才走 tee，**前台同步 exec 不动**；显式路 ring 在 finalize / 队列取消 / 会话删清，auto-bg 路在 worker 内非 detach 终局（`next.is_none()`）清、detach 走 finalize 清；incognito 永不注册
- **后台任务面板 + 完成合并窗口（R4）**：owner-plane（host-trusted）读 `JobManager::list_session_snapshots(session)` / `get_job_snapshot(id)` → `BackgroundJobSnapshot`（camelCase 展示向,与 model-facing `job_status` JSON 物理分离,**Group 子投影折叠进 Group 行**）；Tauri `list_background_jobs` / `get_background_job` + HTTP `GET /api/sessions/{id}/background-jobs`、`GET /api/background-jobs/{id}`（Bearer 同 session 端点；owner 平面看全部不经 agent-scope,与 KB owner 平面一致）；前端镜像类型 `src/types/background-jobs.ts` + `useBackgroundJobs` 单订阅喂头部徽标 / 独立面板 / 工作台区块。**取消统一复用 `cancel_runtime_task(kind=async_job)`,不新增取消端点**。**完成注入合并窗口**：`async_tools.completionMergeWindowSecs`（默认 3,`0` 关）—— `finalize_job` 改走 `injection::enqueue_injection` 缓冲,同会话窗口内完成的多 tool job 合并一条 `<task-notification-batch>` 一轮注入（省计费 turn）。纯内存 live-path：崩溃则 terminal-but-uninjected,重启 `replay_pending_jobs` 各自补投（不丢、不合并）；Group(R5) 是预合并特例绕过；沿用 ghost-turn 闸 + 逐 job claim/release + on_injected 逐行恰好一次。完成桌面通知 `notification.notifyOnBackgroundJobComplete`（默认开,仅 `job:completed` 的 completed/failed/timed_out + 仅后台）
- **R9 设置三件套（资源旁钮配置化）**：四个曾为硬编码常量的资源旁钮收进 `AsyncToolsConfig`（`async_tools` category，走泛型 merge → `get/update_settings` 自动接通、GUI 走专用 `save_async_tools_config`，MEDIUM）：`output_tail_bytes`（默认 8KB）、`max_queued_jobs`（默认 256，`async_jobs::slots`）、`wakeup_max_delay_secs`（默认 24h）、`wakeup_max_pending_per_session`（默认 5）。**契约：默认值单一来源在 `AsyncToolsConfig`（模块内不再留默认常量），读时一律钳到安全 band（队列 `[1,4096]`、tail `[256,1MB]`、wakeup delay `[10s,7d]`、pending `[1,100]`）——`0` 对这四个 bounded-resource 旁钮一律钳到地板,绝非「无限」（区别于 `max_concurrent_jobs` 的 `0`=不限）**；`wakeup` 下限 10s 固定不可配（busy-poll 地板）。**进度上报节流（R9 清单项）目前 N/A**——尚无高频进度源（`job:progress` 仅 Group 每子结算 N/M，天然有界），待字节增量进度（消费者驱动，R3/R4 延后项）落地再配置化,**不为不存在的 emit 路径预置死配置**
...
- **后台 subagent 投影进 Background Job（R6，单向）**：用户委派的后台 subagent run（`!skip_parent_injection`——排除 plan/team/hook 内部 spawn；且 `!parent_incognito`）在 `spawn_subagent` 建一条 `background_jobs`（`kind=Subagent`、`subagent_run_id` FK）投影,与后台工具 job 共享 `job_status` list/status/cancel + 未来面板。**`subagent_runs` 是执行真相源,投影只承载 status/生命周期、绝不持有 run 正文(task/result/error)、绝不反写**。同步走单一 choke point `SessionDB::update_subagent_status`→`JobManager::sync_subagent_projection`（覆盖 run 生命周期 + 三处 kill fallback；terminal 冻结）；投影 `injected=true` **永不进工具注入/replay 路径**（subagent 自有 `inject_and_run_parent`，防双注入）；取消经 `cancel_job` kind=Subagent 分支路由到 `subagent::request_cancel_run`（注册表 cancel + DB-Killed 兜底，与 `kill` 工具同源），**不跑工具 job 的 hook/注入**。新增后台单元一律加 `JobManager` 方法,勿另起平行 API
- **`batch_spawn` = Group fan-out（R5，合并注入一轮）**：`batch_spawn` 建一条 `kind=Group` 协调行（`group_id` 列关联子投影；`args_json={"sealed":bool}`；`injected=true` 自发合并注入不进工具 replay），N 个子 subagent 携 `group_id`、**抑制个体注入**;全部子到终态时单赢 CAS（`claim_group_completion`，`Running→Completed`）认领并发**一条**合并 `<task-notification>`（**join-all-settle**:部分成功 + 各自终态,含 task/label;复用既有 `subagent_result` 前端 pill）。红线：**建 group 前预校验全部 task**（任一非法整体拒——否则已建子代理永不 seal → 漏交付）;`seal_group` 在 spawn 完成后标 sealed 并兜底完成检查;**取消 kind=Group 先标 group 终态再取消子 run**（无 flag 兜底同步标子 Killed 会同线程触发 join,group 须先终态使 CAS 落败）;投影插入失败的子 **ungroup 回退个体注入**（不丢结果）;incognito / jobs DB 未初始化不建 group（回退个体注入）;join 真相读 `subagent_runs`,group 行**绝不持有 run 正文**。重启 in-flight group 同 R6 标 Interrupted（子结果仍在 `subagent_runs`,合并注入不补发）
...
- **GUI ↔ IM live 流式镜像**：desktop / HTTP 触发的 turn 通过 [`chat_engine/im_mirror.rs`](crates/ha-core/src/chat_engine/im_mirror.rs) `attach_im_live_mirror` 把 `ChannelStreamSink` 注册到 [`SinkRegistry`](crates/ha-core/src/chat_engine/sink_registry.rs)，引擎 `emit_stream_event` 末尾的 fan-out hook 在每帧把 streaming event 转发到 IM 流式预览任务。turn 收尾走 `finalize_im_live_mirror`：drop SinkHandle → drain `RoundTextAccumulator` → 复用 dispatcher 的 `deliver_split` / `deliver_final_only` / `deliver_preview_merged`，按 IM account 的 `ImReplyMode`（`split / preview / final`）渲染——与 IM 入站 turn 完全对称。**两个通道独立走自己的发送通路**：GUI 永远走 Tauri IPC stream / HTTP `chat:stream_delta` 广播，不受 `imReplyMode` 影响；`imReplyMode` 仅决定 IM 端的呈现形态。错误 / 取消路径走 RAII drop，IM 端保留半截 preview，与入站 cancel 行为一致。引擎入口的自动 attach 对 `source ∈ {Subagent, ParentInjection, Channel, Cron}` 直接 no-op（IM 入站自己有完整流式管线，subagent 不应外溢到 IM）。**例外——后台完成注入回投 IM（G1）**：后台 job / subagent / group 完成的注入 turn（`ParentInjection`）由 [`subagent::injection::inject_and_run_parent`](crates/ha-core/src/subagent/injection.rs) **自行 attach 注入镜像**（`im_mirror::attach_im_injection_mirror`，无 user-quote）并**在同一 future 内 `await` finalize**（注入跑在短命 current-thread runtime 上，引擎那条 `tokio::spawn(finalize)` 会随 runtime drop 被腰斩，故必须 await 而非 spawn）——父会话若 attach 了 IM chat，后台完成结果按 `imReplyMode` 回投 IM；cron 会话则经 `cron::delivery::deliver_injection_for_session`（按会话反查 `cron_run_logs` → job）下发到 `delivery_targets`（G2）。注入空闲门超时不再丢弃，而是重排队进 `PENDING_INJECTIONS` 待会话空闲重试（G3/G5，group 合并注入因此不再永久丢失）。
...
- **统一日志**：前后端走 [`logging.rs`](crates/ha-core/src/logging.rs)（SQLite + 文本双写），API 请求体 `redact_sensitive` + 32KB 截断；agent 自主排查入口见 [`skills/ha-logs/SKILL.md`](skills/ha-logs/SKILL.md)（用 `exec` + `sqlite3 -readonly` 直查 `~/.hope-agent/{logs,sessions,background_jobs}.db`）
...
- **项目会话懒创建（desktop / HTTP 交互入口）**：进项目「新建对话」**不预先 `create_session_cmd` 落库**，停在草稿态（`currentSessionId=null`），前端用 `draftProjectId` 记项目（仿 `draftWorkingDir`），首条消息经 `chat` 命令的 `projectId` 走 `create_session_with_project` 才落库——与普通对话对称，不再产生未发消息的空会话行，草稿态走同一套模型 / 权限模式 seeding。`chat` 在 `agent_id` 缺省时按 `project.default_agent_id` 解析（对齐 `create_session_cmd`），`project_id` 与 `incognito` 互斥（后端强制 off）。**仅交互入口懒创建**——IM 入站 / cron / subagent 仍 eager。前端 `effectiveProjectId = 会话 meta.projectId ?? draftProjectId` 是「当前项目」单一来源（草稿 + 落库过渡窗口，防 badge 闪烁与切普通会话的陈旧泄漏）。**进项目入口不得再 `create_session_cmd` 预建**
