...
use ha_core::media_gen::types::AudioKind;
...
async fn generate_impl(params: AudioGenParams<'_>) -> Result<AudioGenResult> {
    ...
    ha_core::security::ssrf::check_url(&submit_url, params.ssrf, &cfg.ssrf.trusted_hosts).await?;
    ...
    if !status.is_success() {
        anyhow::bail!(
            ...
            status.as_u16(),
            ha_core::truncate_utf8(&text, 300)
        );
    }
    ...
    ha_core::security::ssrf::check_url(&poll_url, params.ssrf, &cfg.ssrf.trusted_hosts).await?;
    ...
    loop {
        ...
        if !poll_status.is_success() {
            ...
            anyhow::bail!(
                ...
                poll_status.as_u16(),
                task_id,
                ha_core::truncate_utf8(&poll_text, 300)
            );
        }
            ...
            TaskState::Failed => {
                anyhow::bail!(
                    ...
                    data.task_status_msg
                        .as_deref()
                        .map(|m| ha_core::truncate_utf8(m, 300).to_string())
