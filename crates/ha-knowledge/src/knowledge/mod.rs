//! Knowledge Space 机器层（阶段 5 第六刀，自 `ha_core::knowledge` 迁出）。
//!
//! 台账 / 契约 / 裁决留 kernel（见 [`ha_core::knowledge`]），本模块是围着它们
//! 转的机器：`index.db` 笔记缓存与检索、Markdown 解析与来源编译、embedding 与
//! 重建、`[[note]]` 注入与被动召回、Layer-2 自主维护流水线、外部 vault 监听。
//!
//! # 路径兼容
//!
//! kernel 留存的四个子模块在这里原名再导出（[`access`] / [`registry`] /
//! [`types`] / [`maintenance_defs`]），所以本 crate 内 `super::types::…` /
//! `crate::knowledge::registry::…` 一类的写法与迁出前逐字相同，`maintenance`
//! 子模块内的 `super::types` 同样指向下沉后的 [`maintenance_defs`]。

// —— kernel 留存部分：原名再导出，保住迁出前的模块路径 ——
pub use ha_core::knowledge::{access, maintenance_defs, registry, types};
...
pub fn delete_kb_cascade(kb_id: &str) -> Result<bool> {
    ...
    if let Ok(dir) = ha_core::paths::knowledge_dir().map(|d| d.join(sanitize(kb_id))) {
        ...
        if target.exists() {
            ...
            if let (Ok(canon), Ok(root)) = (target.canonicalize(), ha_core::paths::knowledge_dir())
            {
