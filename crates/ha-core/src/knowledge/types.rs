...
// 配置类型已下沉 ha-config-schema，此处原地再导出保持 `crate::knowledge::types::*`
// 路径不变（`default_passive_*` 三个 helper 供本文件测试使用，schema 侧已做
// 可见性升级为 pub）。
pub use ha_config_schema::knowledge::types::{
    default_passive_cache_ttl_secs, default_passive_max_chars, default_passive_top_n,
    KnowledgeCompileConfig, KnowledgeMediaRetentionConfig, KnowledgeSourceLimitsConfig,
    KnowledgeVisionConfig, NoteToolsConfig, PassiveRecallConfig, DEFAULT_MAX_BINARY_SOURCE_MB,
    DEFAULT_MAX_TEXT_SOURCE_MB, DEFAULT_MAX_URL_RESPONSE_MB, MAX_MAX_BINARY_SOURCE_MB,
    MAX_MAX_TEXT_SOURCE_MB, MAX_MAX_URL_RESPONSE_MB, MIN_MAX_BINARY_SOURCE_MB,
    MIN_MAX_TEXT_SOURCE_MB, MIN_MAX_URL_RESPONSE_MB,
};
...
// KnowledgeMediaRetentionConfig / KnowledgeSourceLimitsConfig（含 MiB 限额
// 常量）已下沉 ha-config-schema，见文件顶部 pub use。
...
// KnowledgeCompileConfig / KnowledgeVisionConfig / NoteToolsConfig 已下沉
// ha-config-schema，见文件顶部 pub use。
...
// PassiveRecallConfig 已下沉 ha-config-schema，见文件顶部 pub use。
