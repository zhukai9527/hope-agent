...
pub fn get_available_tools() -> Vec<ToolDefinition> {
    let mut tools = vec![
        ToolDefinition {
            name: TOOL_EXEC.into(),
            description: "Execute a shell command. Returns stdout/stderr. For ordinary long-running commands, use `run_in_background: true` so the async job layer owns status, cancellation, output tail, and `<task-notification>` completion. The legacy exec-native `background`/`yield_ms` process session is reserved for cases that truly need the `process` tool's session surface; legacy flags are migrated to async jobs when async tools are enabled. In desktop sessions `target=auto` (the default) auto-selects the environment: it probes the host PATH for the command's leading tool first, falls back to the default WSL distribution and then every installed WSL distribution on Windows when the tool is missing, then to Docker — so `docker --version`, `git --version`, and `node --version` work even when the tool is only installed inside WSL. Do not set `target=host` just to look on the host: that skips the WSL fallback. Leave `target` unset for ordinary environment/tool queries; pass it explicitly only when the command must run in a specific environment. Container-only commands and `Isolated` mode always use Docker, so you normally do not need to set `target`.".into(),
            ...
            parameters: json!({
                ...
                "properties": {
                    ...
                    "target": {
                        ...
                        "description": "Execution target. auto (default) probes the desktop host PATH first for the command's leading tool and runs there; falls back to the default WSL distribution, then every installed WSL distribution, then Docker when the tool is missing — on Windows this automatically finds tools installed inside WSL (e.g. Docker Engine, git, node installed in WSL). Prefer auto for normal tool queries (docker version, git --version, node --version): it will run wherever the tool actually exists. host runs on the desktop host (Windows/macOS/Linux) regardless of host probe; wsl is Windows-only and uses the default WSL distribution (or the explicit `distro`); docker forces the configured Docker sandbox. Container-only commands and Isolated mode always use docker."
                    },
                    "distro": {
                        "type": "string",
