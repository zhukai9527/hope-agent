...
impl SessionDB {
    ...
    /// Return compact recent activation facts for usage dashboards.
    /// The activation table is generated by Hope sessions, so the current app
    /// source is always `hope`; external app log parsing is intentionally not
    /// inferred here.
    pub fn recent_skill_activations(&self, limit: usize) -> Result<Vec<(String, String, String)>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| anyhow::anyhow!("Lock error: {}", e))?;
        ...
        let mut stmt = conn.prepare(
            "SELECT skill_name, session_id, activated_at
             FROM session_skill_activation
             ORDER BY activated_at DESC, skill_name ASC
             LIMIT ?1",
        )?;
        let rows = stmt.query_map([bounded_limit], |row| {
            Ok((
                row.get::<_, String>(0)?,
                row.get::<_, String>(1)?,
                row.get::<_, String>(2)?,
