...
use ha_core::goal::{
    CloseGoalInput, GoalClosureDecision, GoalCriterionCheckKind, GoalCriterionKind,
    GoalCriterionSpecInput, GoalSemanticCriterionGrade, GoalSemanticCriterionVerdict,
    GoalSemanticGrade, GoalSemanticGradeStart, GoalSemanticOverallVerdict, GoalSnapshot, GoalState,
};
use ha_core::session::SessionDB;

use ha_core::tools::ToolExecContext;

use crate::policy::build_completion_report;
...
pub async fn tool_goal_status(_args: &Value, ctx: &ToolExecContext) -> String {
...
pub async fn tool_goal_prepare_contract(args: &Value, ctx: &ToolExecContext) -> String {
    let (session_id, db, snapshot) = match active_goal(ctx).await {
        Ok(value) => value,
        Err(err) => return error_json(err),
    };
    ...
    let permission_surface = if requires_approval {
        match ha_core::permission::evaluate_approval_surface(Some(&session_id)) {
            ha_core::permission::ApprovalSurface::Attended => json!({
                "status": "available"
            }),
            ha_core::permission::ApprovalSurface::Unattended(reason) => json!({
                "status": "unavailable",
...
pub async fn tool_goal_checkpoint(args: &Value, ctx: &ToolExecContext) -> String {
...
pub async fn tool_goal_record_evidence(args: &Value, ctx: &ToolExecContext) -> String {
...
async fn run_goal_semantic_grade(
    ...
    match started {
        ...
        GoalSemanticGradeStart::Started {
            ...
            let chain = ha_core::automation::effective_chain(&config, legacy_chain);
            if chain.is_empty() {
            ...
            let mut model = ha_core::automation::model_label(&config, &chain[0]);
            ...
            for parse_attempt in 0..GOAL_SEMANTIC_GRADER_PARSE_ATTEMPTS {
                ...
                model = ha_core::automation::model_label(&config, &result.model);
                ...
                let parsed = ha_core::extract_json_span(&result.text, Some('{'))
...
pub async fn tool_goal_evaluate(args: &Value, ctx: &ToolExecContext) -> String {
    ...
    json_string(json!({
        ...
        "report": build_completion_report(&evaluated, None),
...
pub async fn tool_goal_finish_request(args: &Value, ctx: &ToolExecContext) -> String {
    ...
    let mut report = build_completion_report(&closed, summary.as_deref());
...
pub async fn tool_goal_block_request(args: &Value, ctx: &ToolExecContext) -> String {
...
mod tests {
    ...
    use ha_core::tools::SessionDbHandle;
