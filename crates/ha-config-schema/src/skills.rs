//! Skills configuration (`AppConfig.skills` / `AppConfig.skill_prompt_budget`).
//!
//! - [`SkillsConfig`] — root `AppConfig.skills` section.
//! - [`SkillPromptBudget`] — configurable limits for skill prompt generation.
//! - [`SkillsAutoReviewConfig`] / [`AutoReviewPromotion`] — five-gate
//!   auto-review waterfall config (`AppConfig.skills.auto_review`); gate
//!   enforcement lives in ha-core (`triggers.rs` gate 1, `heuristics.rs`
//!   gates 2 & 5, `pipeline.rs` gates 3 & 4). Defaults skew strict — surface
//!   false-negatives in the UI rather than ship false-positive drafts.
...
pub struct SkillsConfig {
    #[serde(default)]
    pub auto_review: SkillsAutoReviewConfig,
...
pub const DEFAULT_MAX_SKILLS_IN_PROMPT: usize = 150;
pub const DEFAULT_MAX_SKILLS_PROMPT_CHARS: usize = 30_000;
pub const DEFAULT_MAX_SKILL_FILE_BYTES: u64 = 256 * 1024;
pub const DEFAULT_MAX_CANDIDATES_PER_ROOT: usize = 300;
...
// ── Auto-review (`AppConfig.skills.auto_review`) ─────────────────
