...
//!
//! 本文件只有 wire 类型与其自包含 impl / serde helper / 纯类型测试；读写 contract
//! （`cached_config` / `mutate_config` / persistence）全部留在 ha-core `config/`。
//! 文末测试随类型下沉（只测默认值 / 钳制 / serde 兼容，无 ha-core 依赖）——
//! 它们大量使用模块私有 helper，留在 ha-core 会强迫这些 helper 全部转 pub。
...
/// 默认 Agent id 单一来源（红线：禁止裸字面量 agent id）。原定义在 ha-core
/// `agent_loader.rs`，随 `AppConfig.default_agent_id` 的 serde default 一起下沉；
/// ha-core 侧 `agent_loader::DEFAULT_AGENT_ID` 经再导出保持既有路径，前端走
/// `@/types/tools` 同名常量。
...
fn default_ask_user_question_timeout() -> u64 {
    0
}
