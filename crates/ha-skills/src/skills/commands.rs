...
fn probe_skill_apps() -> Vec<SkillAppProbe> {
    ...
    let hope_root = ha_core::paths::skills_dir().ok();
    ...
    if !status.is_success() {
        ...
        return Ok(SkillPublishPushResult {
            ...
            error: Some(format!(
                "Hub returned HTTP {}: {}",
                status,
                ha_core::truncate_utf8(message.trim(), 512)
            )),
            skill_name: draft.skill_name,
            source_id: hub.id,
            registry_url,
...
fn skill_market_hubs_path() -> Result<PathBuf> {
    Ok(ha_core::paths::root_dir()?.join("skill-market-hubs.json"))
}
...
fn skill_market_hub_token_path(token_ref: &str) -> Result<PathBuf> {
    ...
    Ok(ha_core::paths::credentials_dir()?
        .join("skill-market")
...
fn write_skill_market_hub_token_by_ref(token_ref: &str, token: &str) -> Result<()> {
    ...
    ha_core::platform::write_secure_file(&path, &payload)
...
fn skill_market_sources_path() -> Result<PathBuf> {
    Ok(ha_core::paths::root_dir()?.join("skill-market-sources.json"))
}
...
fn first_markdown_paragraph(content: &str) -> Option<String> {
    content
        ...
        .map(|line| ha_core::truncate_utf8(line, 240).to_string())
}
...
pub fn list_skill_backups(name: &str) -> Vec<SkillBackupEntry> {
    let root = match ha_core::paths::skills_dir() {
        Ok(dir) => dir,
        Err(_) => return vec![],
    };
...
pub fn rollback_skill(name: &str, backup_timestamp: &str) -> Result<()> {
    ...
    let root = ha_core::paths::skills_dir()
...
pub fn scan_skill_usage(db: &ha_core::session::SessionDB) -> Result<SkillUsageScanReport> {
...
fn external_app_skills_root(app: &str) -> Option<PathBuf> {
    ...
    match app {
        ...
        "hope" => ha_core::paths::skills_dir().ok(),
        _ => None,
    }
}
