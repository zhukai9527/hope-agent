...
use ha_core::permission::{AskReason, Decision};
use ha_core::tools;
use ha_core::workflow::{
    spawn_agent_tool_args, WorkflowPermissionPreview, WorkflowPermissionPreviewCall,
    WorkflowPermissionPreviewSummary, WorkflowSessionContext,
};

const MAX_PREVIEW_CALLS: usize = 128;

pub fn preview_workflow_script(
    script: &str,
    session_id: &str,
...
fn preview_raw_call(
    ...
    match raw.api.as_str() {
        ...
        "workflow.read" => preview_tool_call(
            ...
            normalize_tool_args(ha_core::tool_defs::TOOL_READ, value.clone()),
        ...
        "workflow.grep" => preview_tool_call(
            ...
            normalize_tool_args(ha_core::tool_defs::TOOL_GREP, value.clone()),
            optional_string(&value, "label"),
            session_context,
            calls,
        ),
...
fn preview_ask_user(
    ...
    match ha_core::permission::evaluate_approval_surface(Some(session_id)) {
        ha_core::permission::ApprovalSurface::Attended => calls.push(allow_call(
            api,
            line,
            None,
            label,
        ...
        ha_core::permission::ApprovalSurface::Unattended(reason) => {
            let action = ha_core::config::cached_config()
                .permission
                .unattended_approval_action;
            let (decision, detail) = match action {
                ha_core::permission::UnattendedApprovalAction::Proceed => ("allow", "proceed"),
                ha_core::permission::UnattendedApprovalAction::Deny => ("deny", "deny"),
            };
...
fn resolve_tool_preview(
    ...
    let resolve_ctx = ha_core::permission::engine::ResolveContext {
        tool_name,
        args,
        session_mode: session_context.session_mode,
        sandbox_mode: ha_core::permission::SandboxMode::Off,
        global_yolo: ha_core::security::dangerous::is_dangerous_skip_active(),
        plan_mode: false,
    ...
    ha_core::permission::engine::resolve(&resolve_ctx)
}
...
fn normalize_tool_args(tool_name: &str, mut args: Value) -> Value {
    if tool_name == ha_core::tool_defs::TOOL_EXEC {
