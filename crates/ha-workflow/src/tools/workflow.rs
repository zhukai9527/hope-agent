...
pub async fn tool_workflow(args: &Value, ctx: &ToolExecContext) -> Result<String> {
    ...
    let output = match input.action {
        ...
        _ => {
            ...
            ha_core::blocking::run_blocking(move || -> Result<Value> {
                match input.action {
...
fn workflow_authoring_guide(workflow_mode: ha_core::workflow_mode::WorkflowMode) -> Value {
    json!({
        "apiVersion": 5,
...
/// worker (see `ha_core::blocking`).
fn resolve_workflow_ctx(
    ctx: &ToolExecContext,
) -> Result<(String, std::sync::Arc<ha_core::session::SessionDB>)> {
    let session_id = ctx
        .session_id
        .as_deref()
...
fn create_workflow(
    ...
    session_id: &str,
    workflow_mode: ha_core::workflow_mode::WorkflowMode,
) -> Result<Value> {
...
fn create_workflow_run_from_script(
    ...
    if start_now {
        ha_core::workflow::ensure_workflow_launcher_primary().map_err(|e| {
            anyhow!(
    ...
    if matches!(
        execution_mode,
        ha_core::execution_mode::ExecutionMode::Autonomous
    ...
    let preview = ha_core::workflow::ensure_workflow_script_can_create(
        &db,
        session_id,
        &script_source,
...
fn resolve_workflow_size_guideline<'a>(
    ...
    Ok(match workflow_mode {
        ha_core::workflow_mode::WorkflowMode::Ultracode => "large",
        ha_core::workflow_mode::WorkflowMode::Off | ha_core::workflow_mode::WorkflowMode::On => {
            "medium"
        }
    })
}
    ...
    let run = match command {
        ...
        WorkflowControlCommand::Resume => {
            ...
            let run = db
                ...
                .await?;
            let _ = ha_core::workflow::spawn_workflow_run_if_primary(
                db.clone(),
                run.id.clone(),
        ...
        WorkflowControlCommand::Cancel => {
            ...
            ha_core::workflow::cancel_workflow_run_with_children(db.clone(), &run_id).await?
        }
    };
...
fn resolve_execution_mode(
    db: &ha_core::session::SessionDB,
    session_id: &str,
    requested: Option<&str>,
) -> Result<ha_core::execution_mode::ExecutionMode> {
    if let Some(raw) = requested {
        return ha_core::execution_mode::ExecutionMode::from_str(raw)
    ...
    Ok(match session_mode {
        ha_core::execution_mode::ExecutionMode::Off => {
            ha_core::execution_mode::ExecutionMode::Guarded
        }
        mode => mode,
    })
}
...
fn visible_workflow_run(
    ...
    session_id: &str,
    run_id: &str,
) -> Result<ha_core::workflow::WorkflowRun> {
    let run = db
...
fn select_relevant_workflow_run(
    db: &ha_core::session::SessionDB,
    session_id: &str,
) -> Result<Option<ha_core::workflow::WorkflowRun>> {
...
fn is_visible_active_state(state: ha_core::workflow::WorkflowRunState) -> bool {
    matches!(
        state,
        ha_core::workflow::WorkflowRunState::Draft
            | ha_core::workflow::WorkflowRunState::AwaitingApproval
            | ha_core::workflow::WorkflowRunState::Running
            | ha_core::workflow::WorkflowRunState::AwaitingUser
            | ha_core::workflow::WorkflowRunState::Paused
            | ha_core::workflow::WorkflowRunState::Recovering
    )
}

fn run_summary_json(run: &ha_core::workflow::WorkflowRun) -> Value {
    json!({
        "runId": run.id,
...
fn ops_summary_json(ops: &[ha_core::workflow::WorkflowOp]) -> Value {
    ...
    for op in ops {
        match op.state {
            ha_core::workflow::WorkflowOpState::Pending => pending += 1,
            ha_core::workflow::WorkflowOpState::Started => started += 1,
            ha_core::workflow::WorkflowOpState::Completed => completed += 1,
            ha_core::workflow::WorkflowOpState::Failed => failed += 1,
        }
    }
    json!({
        "total": ops.len(),
...
fn pending_actions_json(snapshot: &ha_core::workflow::WorkflowRunSnapshot) -> Vec<Value> {
    let mut actions = Vec::new();
    match snapshot.run.state {
        ha_core::workflow::WorkflowRunState::AwaitingApproval => actions.push(json!({
            "kind": "user_approval",
            "severity": "blocking",
            "message": "Workflow is waiting for explicit user approval. The model cannot approve it."
        })),
        ha_core::workflow::WorkflowRunState::AwaitingUser => actions.push(json!({
            "kind": "user_input",
            "severity": "blocking",
            "message": "Workflow is waiting for user input."
        })),
        ha_core::workflow::WorkflowRunState::Paused => actions.push(json!({
            "kind": "paused",
            "severity": "waiting",
            "message": "Workflow is paused and can be resumed or cancelled."
        })),
        ha_core::workflow::WorkflowRunState::Blocked => actions.push(json!({
            "kind": "blocked",
            "severity": "blocking",
            "message": snapshot.run.blocked_reason.clone().unwrap_or_else(|| "Workflow is blocked.".to_string())
        })),
        ha_core::workflow::WorkflowRunState::Failed => actions.push(json!({
            "kind": "failed",
            "severity": "blocking",
    ...
    let failed_ops = snapshot
        .ops
        .iter()
        .filter(|op| op.state == ha_core::workflow::WorkflowOpState::Failed)
        .take(5)
        .map(|op| {
            json!({
                "kind": "failed_op",
...
fn event_summary_json(event: &ha_core::workflow::WorkflowEvent) -> Value {
    json!({
        "seq": event.seq,
        "type": event.event_type,
        "createdAt": event.created_at,
...
fn latest_checkpoint_json(events: &[ha_core::workflow::WorkflowEvent]) -> Option<Value> {
    events
        .iter()
        .rev()
        .find(|event| {
...
fn model_next_action_for_run(
    ...
    match run.state {
        ha_core::workflow::WorkflowRunState::Completed => "summarize_workflow_result_to_user",
        ha_core::workflow::WorkflowRunState::Failed => {
            "inspect_trace_then_explain_failure_or_create_followup"
        }
        ha_core::workflow::WorkflowRunState::Cancelled => "tell_user_workflow_was_cancelled",
        ha_core::workflow::WorkflowRunState::Blocked => "explain_blocker_and_recovery_options",
        ha_core::workflow::WorkflowRunState::AwaitingApproval => {
            "ask_user_to_review_workflow_approval"
        }
        ha_core::workflow::WorkflowRunState::AwaitingUser => "ask_user_for_required_input",
        ha_core::workflow::WorkflowRunState::Paused => "resume_or_cancel_when_user_confirms",
        ha_core::workflow::WorkflowRunState::Running
        | ha_core::workflow::WorkflowRunState::Recovering => {
            if has_checkpoint {
        ...
        ha_core::workflow::WorkflowRunState::Draft => "start_or_explain_draft_to_user",
    }
}
