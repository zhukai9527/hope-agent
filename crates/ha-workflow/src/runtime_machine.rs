//! Feature-owned QuickJS interpreter, host object, and step machine.
...
use ha_core::async_jobs::{BackgroundJob, JobManager, JobOrigin, JobStatus};
use ha_core::domain_workflow::{DomainEvidenceItem, RecordDomainEvidenceInput};
use ha_core::review::{self, ReviewFindingStatus, RunReviewInput};
use ha_core::session::{SessionDB, SessionIdeContext, Task, TaskStatus};
use ha_core::tool_defs::ToolExecContext;
use ha_core::tools;
use ha_core::verification::{self, PlanVerificationInput};
use ha_core::workflow::runtime::*;
...
pub fn execute_script(
    db: Arc<SessionDB>,
    run: ha_core::workflow::WorkflowRun,
...
fn script_timeout(run: &ha_core::workflow::WorkflowRun) -> Duration {
    let secs = run
        .budget
        .get("maxScriptSecs")
...
fn has_runtime_budget(run: &ha_core::workflow::WorkflowRun) -> bool {
    optional_u64_any(
        &run.budget,
        &[
            "maxScriptSecs",
...
pub fn has_required_autonomous_budget(run: &ha_core::workflow::WorkflowRun) -> bool {
...
struct WorkflowRuntimeHost {
    ...
    control: Option<ha_core::workflow::WorkflowRunControl>,
...
impl WorkflowRuntimeHost {
    ...
    fn task_create(&mut self, args: Value) -> Result<Value> {
        ...
        self.execute_op(
            ...
            |host| {
                ...
                ha_core::session::emit_task_snapshot(&host.session_id, &tasks);
                ha_core::hooks::fire_task_created(&host.session_id, &title, None, "");
        ...
        self.execute_op(
            ...
            |host| {
                ...
                if status == Some(TaskStatus::Completed) {
                    ha_core::hooks::fire_task_completed(&host.session_id, id, &updated.content);
                }
    ...
    fn file_search(&mut self, args: Value) -> Result<Value> {
        ...
        self.execute_op("fileSearch", WorkflowEffectClass::Pure, input, |_host| {
            let response = ha_core::filesystem::search_files(&root, &query, limit)
    ...
    fn spawn_agent(&mut self, args: Value) -> Result<Value> {
        ...
        self.execute_op_with_child_handle(
            ...
            |host, child_handle| {
                ...
                let output = host.dispatch_tool(ha_core::tool_defs::TOOL_SUBAGENT, &dispatch_args)?;
    ...
    fn recover_spawn_agent_child(
        ...
        let result_only = run.owner_kind != ha_core::subagent::SubagentOwnerKind::Workflow
    ...
    fn resume_agent(&mut self, args: Value) -> Result<Value> {
        ...
        if matches!(source.status, ha_core::subagent::SubagentStatus::Killed)
            || matches!(
                source.terminal_reason,
                Some(
                    ha_core::subagent::SubagentTerminalReason::UserKilled
                        | ha_core::subagent::SubagentTerminalReason::ApprovalDenied
                        | ha_core::subagent::SubagentTerminalReason::WorkflowCancelled
                        | ha_core::subagent::SubagentTerminalReason::ParentCancelled
                )
            )
        {
            return Err(anyhow!(
        ...
        self.execute_op_with_child_handle(
            ...
            |host, child_handle| {
                ...
                if run.continuation_of_run_id.as_deref() != Some(source_run_id.as_str())
                    || run.owner_kind != ha_core::subagent::SubagentOwnerKind::Workflow
            ...
            |host, child_handle| {
                ...
                dispatch_args.insert(
                    tools::WORKFLOW_PREALLOCATED_RUN_ID_ARG.to_string(),
                    Value::String(child_handle.to_string()),
                );
                dispatch_args.insert(
                    tools::WORKFLOW_RUN_ID_ARG.to_string(),
                    Value::String(host.run_id.clone()),
                );
                dispatch_args.insert(
                    tools::WORKFLOW_DISPATCH_ID_ARG.to_string(),
                    Value::String(format!("workflow-resume:{child_handle}")),
                );
                dispatch_args.insert(
                    tools::WORKFLOW_ISOLATION_ARG.to_string(),
                    Value::String(isolation.clone()),
                );
                let output = host.dispatch_tool(
                    ha_core::tool_defs::TOOL_SUBAGENT,
            ...
            |host| {
                let output = host.dispatch_tool(
                    ...
                    &json!({
                        ...
                        "message": message,
                        "mode": "steer_only",
                        (tools::WORKFLOW_RUN_ID_ARG): host.run_id.clone(),
                    }),
                )?;
    ...
    fn record_consumed_agent_results(&self, api: &str, output: &Value) -> Result<()> {
        ...
        for run_id in &run_ids {
            ha_core::subagent::mark_run_fetched(run_id);
            for injection_run_id in self
                .db
                .workflow_agent_checkpoint_injection_run_ids(&self.run_id, run_id)?
            {
                ha_core::subagent::mark_run_fetched(&injection_run_id);
    ...
    fn read(&mut self, args: Value) -> Result<Value> {
        ...
        self.execute_op("read", WorkflowEffectClass::Pure, args, |host| {
            host.dispatch_tool(ha_core::tool_defs::TOOL_READ, &tool_args)
                .map(Value::String)
        })
    }
    ...
    fn grep(&mut self, args: Value) -> Result<Value> {
        ...
        self.execute_op("grep", WorkflowEffectClass::Pure, args, |host| {
            host.dispatch_tool(ha_core::tool_defs::TOOL_GREP, &tool_args)
                .map(Value::String)
        })
    }
    ...
    fn spawn_validation_exec_job(&self, job_ref: &ValidationJobRef) -> Result<()> {
        let mut ctx = self.tool_exec_context();
        ctx.async_tool_policy = ha_core::agent_config::AsyncToolPolicy::NeverBackground;
    ...
    fn diff(&mut self, args: Value) -> Result<Value> {
        ...
        self.execute_op("diff", WorkflowEffectClass::Pure, input, |host| {
            ...
            let diff = ha_core::session::load_git_diff_for_root(std::path::Path::new(root))
    ...
    fn previous_repair_validation_event(
        &self,
        op_key: &str,
    ) -> Result<Option<ha_core::workflow::WorkflowEvent>> {
        Ok(self
            .db
            .list_workflow_events(&self.run_id, 500)?
            .into_iter()
    ...
    fn current_diff_hash(&self) -> (Option<String>, Option<String>) {
        ...
        match ha_core::session::load_git_diff_for_root(std::path::Path::new(root))
        ...
        F: FnOnce(&mut WorkflowRuntimeHost, &str) -> Result<Value>,
    {
            ...
            .is_some_and(|op| op.state == WorkflowOpState::Started)
        {
            match self.db.started_op_recovery_action(&self.run_id, &op_key)? {
                Some(ha_core::workflow::StartedOpRecoveryAction::BlockNonIdempotent)
                | Some(ha_core::workflow::StartedOpRecoveryAction::AttachChildHandle(_)) => {
                    let _ = self
                        .db
                ...
                Some(ha_core::workflow::StartedOpRecoveryAction::RerunPure)
                | Some(ha_core::workflow::StartedOpRecoveryAction::RecheckIdempotent)
                | None => {}
            }
        }
        ...
        R: FnOnce(&mut WorkflowRuntimeHost, &str) -> Result<Option<Value>>,
    {
        ...
        {
            match self.db.started_op_recovery_action(&self.run_id, &op_key)? {
                Some(ha_core::workflow::StartedOpRecoveryAction::AttachChildHandle(handle)) => {
    ...
    fn dispatch_ask_user(&self, args: &Value) -> Result<Value> {
        if let ha_core::permission::ApprovalSurface::Unattended(reason) =
            ha_core::permission::evaluate_approval_surface(Some(&self.session_id))
        {
    ...
    fn resolve_unattended_ask_user(
        &self,
        reason: ha_core::permission::UnattendedReason,
    ) -> Result<Value> {
        let action = ha_core::config::cached_config()
            .permission
        ...
        if let Some(bus) = ha_core::globals::get_event_bus() {
            bus.emit(
                ...
                json!({
                    ...
                    "action": match action {
                        ha_core::permission::UnattendedApprovalAction::Proceed => "proceed",
                        ha_core::permission::UnattendedApprovalAction::Deny => "deny",
                    },
                    "strict": false,
        ...
        match action {
            ha_core::permission::UnattendedApprovalAction::Deny => Err(anyhow!(
                "workflow.askUser unattended surface ({}): {}",
                reason.as_str(),
                reason.explain()
            )),
            ha_core::permission::UnattendedApprovalAction::Proceed => {
                ha_core::app_warn!(
                    "workflow",
                    "ask_user",
    ...
    fn tool_exec_context(&self) -> ToolExecContext {
        ToolExecContext {
            ...
            session_db: Some(ha_core::tool_defs::SessionDbHandle(self.db.clone())),
...
fn inject_workflow_preallocated_run_id(
    ...
    map.insert(
        tools::WORKFLOW_SKIP_PARENT_INJECTION_ARG.to_string(),
        Value::Bool(true),
    );
    map.insert(
        tools::WORKFLOW_ISOLATION_ARG.to_string(),
...
fn subagent_run_handle(
    run: &ha_core::subagent::SubagentRun,
    label: Option<&str>,
    task: Option<&str>,
    inject_policy: &str,
    result_mode: &str,
...
pub fn wait_all_tool_args(args: &Value) -> Result<Value> {
    let handles = args
        .get("handles")
...
pub fn ensure_workflow_visible_agent_run_ids(
    db: &SessionDB,
    workflow_run_id: &str,
    run_ids: &[String],
    api: &str,
...
pub fn ensure_workflow_controlled_agent_run_ids(
    db: &SessionDB,
    workflow_run_id: &str,
    run_ids: &[String],
    api: &str,
...
fn subagent_recovery_metadata(
    ...
    let effective_reason = run
        .terminal_reason
        .unwrap_or(ha_core::subagent::SubagentTerminalReason::Unknown);
    let resume_allowed = run.status.is_terminal()
        && run.status != ha_core::subagent::SubagentStatus::Killed
...
fn workflow_agent_run_status(run: &ha_core::subagent::SubagentRun) -> Value {
    let (terminal_reason, resume_allowed, resume_recommended) = subagent_recovery_metadata(run);
    json!({
        "runId": run.run_id,
        "run_id": run.run_id,
    ...
    loop {
        ...
        for run_id in run_ids {
            ...
            if run.status.is_terminal() {
                if run.status == ha_core::subagent::SubagentStatus::Completed {
                    completed += 1;
                } else {
                    failed += 1;
                }
...
pub fn wait_all_output_consumes_results(output: &Value) -> bool {
    output
        .get("resultMode")
                ...
                if let Value::Object(run) = run {
                    ...
                    if !run.contains_key("resumeAllowed") || !run.contains_key("resumeRecommended")
                    {
                        ...
                        let reason = run
                            .get("terminalReason")
                            .and_then(Value::as_str)
                            .map(ha_core::subagent::SubagentTerminalReason::from_str);
...
pub fn ask_user_tool_args(args: &Value) -> Result<Value> {
...
fn workflow_tool_uses_generic_job_child(name: &str, args: &Value) -> bool {
    tools::is_generic_job_capable(name)
        && ha_core::config::cached_config().async_tools.enabled
        && args
...
fn async_tool_started_output(name: &str, job_id: &str) -> String {
    ha_core::async_jobs::synthetic_started_result(job_id, name, JobOrigin::Explicit)
}
...
impl ValidationJobRef {
    ...
    fn exec_args(&self) -> Value {
        ...
        if let Value::Object(map) = &mut args {
            if let Some(timeout) = self.timeout {
                map.insert(
                    ha_core::tool_defs::ASYNC_JOB_TIMEOUT_ARG.to_string(),
...
fn normalized_validation_output(result: &Value) -> String {
    ...
    ha_core::truncate_utf8(normalized.trim(), VALIDATION_FINGERPRINT_OUTPUT_BYTES).to_string()
}
...
pub fn validation_exit_code(output: &str) -> i64 {
...
fn tool_effect_class(name: &str) -> WorkflowEffectClass {
    match name {
        ha_core::tool_defs::TOOL_READ
        | "read_file"
        | ha_core::tool_defs::TOOL_GREP
        | ha_core::tool_defs::TOOL_FIND
        | ha_core::tool_defs::TOOL_LS
        | "list_dir"
        | ha_core::tool_defs::TOOL_TOOL_SEARCH
        | ha_core::tool_defs::TOOL_GET_SETTINGS
        | ha_core::tool_defs::TOOL_AGENTS_LIST
        | ha_core::tool_defs::TOOL_RECALL_MEMORY
        | ha_core::tool_defs::TOOL_MEMORY_GET
        | ha_core::tool_defs::TOOL_JOB_STATUS
        | ha_core::tool_defs::TOOL_SESSIONS_LIST
        | ha_core::tool_defs::TOOL_SESSION_STATUS
        | ha_core::tool_defs::TOOL_SESSIONS_SEARCH
        | ha_core::tool_defs::TOOL_SESSIONS_HISTORY
        | ha_core::tool_defs::TOOL_PEEK_SESSIONS => WorkflowEffectClass::Pure,
...
/// Narrow recovery projection used by the kernel cancellation path.
pub fn validation_child_job_ids(child_handle: &str) -> Result<Vec<String>> {
    Ok(parse_validation_child_handle(child_handle)?
        .jobs
        .into_iter()
        .map(|job| job.job_id)
        .collect())
