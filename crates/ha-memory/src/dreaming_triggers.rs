//! Feature-owned Dreaming trigger state and idle/manual dispatch.

use std::sync::atomic::{AtomicBool, AtomicI64, Ordering};

use crate::dreaming_pipeline;
pub use ha_core::memory::dreaming::DreamTrigger;
use ha_core::memory::dreaming::{DreamReport, DreamingConfig};
...
pub async fn manual_run(trigger: DreamTrigger) -> DreamReport {
    ...
    if matches!(trigger, DreamTrigger::Idle) {
        ha_core::hooks::fire_notification(
            "",
            "idle_prompt",
            "App idle — starting a background memory cycle.",
        );
    }
    dreaming_pipeline::run_cycle(trigger).await
}

pub fn manual_run_boxed(trigger: DreamTrigger) -> ha_core::memory::dreaming::DreamingTriggerFuture {
    Box::pin(manual_run(trigger))
}
