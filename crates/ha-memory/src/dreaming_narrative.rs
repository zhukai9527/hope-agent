...
use ha_core::memory::MemoryEntry;
use ha_core::provider::ActiveModel;

use std::collections::HashMap;

use ha_core::memory::dreaming::narrative::{NarrativeFuture, NarrativeOutput};
use ha_core::memory::dreaming::{DreamingConfig, EvidenceRef, PromotionRecord};

// Go through the kernel-owned typed facades. Besides keeping the feature
// boundary explicit, this lets core tests compile this exact production file
// back into `ha-core` without inventing a second implementation.
use ha_core::memory::dreaming::scanner::{evidence_for_candidate, render_candidates_for_prompt};
use ha_core::memory::dreaming::scoring::{filter_and_rank, parse_nominations};
...
pub fn run_side_query_boxed<'a>(
    ...
    cfg: &'a DreamingConfig,
) -> NarrativeFuture<'a> {
    Box::pin(run_side_query(chain, candidates, cfg))
}
...
pub fn write_diary(md: &str) -> Result<std::path::PathBuf> {
    let dir = ha_core::paths::dreams_dir()?;
    ...
    ha_core::platform::write_atomic(&path, md.as_bytes()).context("writing diary markdown")?;
    Ok(path)
}
...
mod tests {
    ...
    fn candidate(id: i64, session: Option<&str>) -> MemoryEntry {
        MemoryEntry {
            id,
            memory_type: ha_core::memory::MemoryType::User,
            scope: ha_core::memory::MemoryScope::Global,
