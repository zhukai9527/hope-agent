//! Feature-owned memory profile synthesis machine.
...
use ha_core::memory::claims::{self, EvidenceRecord, ResolveClaim};
use ha_core::memory::dreaming::{
    self, DreamPhase, DreamRunStatus, DreamTrigger, ProfileReport, ProfileSnapshotSourceRecord,
};
...
const PROFILE_REWRITE_PROMPT: &str = "You are consolidating a long-term memory profile from already-extracted facts. Rewrite the draft below into a concise, de-duplicated Markdown bullet list of stable, useful facts about the user or project. Merge near-duplicates, drop trivia, keep each bullet to one short line. CRITICAL: do not invent anything not supported by the draft — you may only condense and reword existing facts. Output ONLY the bullet list (lines starting with '- '), no headings, no preamble.";
    ...
    detail.evidence.is_empty()
        || detail.evidence.iter().any(|evidence| {
            evidence
                .session_id
                .as_deref()
                .map(ha_core::memory::session_contribution_source_allowed)
                .unwrap_or(true)
        })
}
...
fn claim_scope_memory_enabled(claim: &ResolveClaim) -> bool {
    ...
    ha_core::agent_loader::load_agent(agent_id)
...
pub fn run_profile_synthesis_cycle_boxed(
    trigger: DreamTrigger,
) -> ha_core::memory::dreaming::ProfileRuntimeFuture {
    Box::pin(run_profile_synthesis_cycle(trigger))
}
