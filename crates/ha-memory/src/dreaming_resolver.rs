//! Feature-owned Deep Resolver execution machine.
...
use ha_core::memory::claims::{self, ResolveClaim};
use ha_core::memory::dreaming::{
    self, AutoResolverPlanning, DreamPhase, DreamRunStatus, DreamTrigger, ResolverDecision,
    ResolverDecisionType, ResolverPreflightBlockReason, ResolverPreflightReport, ResolverReport,
};
...
pub fn resolver_preflight_from_claims(
    cfg: &ha_core::memory::dreaming::DreamingConfig,
...
pub fn plan_auto_expiration_sweep(
    claims: &[ResolveClaim],
    now: &str,
    ...
    for claim in all_claims.iter().filter(|claim| same_scope(anchor, claim)) {
                ...
                || objects.contains(&object))
        {
            neighboring_edges.push(format!(
                "{} --{}--> {}",
                ha_core::memory::sqlite::sanitize_for_prompt(&claim.subject),
                ha_core::memory::sqlite::sanitize_for_prompt(&claim.predicate),
                ha_core::memory::sqlite::sanitize_for_prompt(&claim.object)
            ));
        }
    }
...
pub fn plan_auto_resolution_groups(
    claims: &[ResolveClaim],
    expiring: &HashSet<String>,
    group_cap: usize,
    ...
    let rows = group
        ...
        .map(|claim| {
            json!({
                ...
                "object": ha_core::memory::sqlite::sanitize_for_prompt(&claim.object),
                "content": ha_core::memory::sqlite::sanitize_for_prompt(&claim.content),
            })
        })
        .collect::<Vec<_>>();
...
fn parse_verdict(resp: &str) -> Option<GroupVerdict> {
    let span = ha_core::extract_json_span(resp.trim(), Some('{'))?;
...
fn bounded_rationale(value: &str) -> String {
    ha_core::logging::redact_sensitive(value)
        .split_whitespace()
        .collect::<Vec<_>>()
        .join(" ")
        .chars()
...
fn map_auto_verdict_to_decisions(
    ...
    signals: &GraphGroupSignals,
    cfg: &ha_core::memory::dreaming::DeepResolverConfig,
...
pub async fn run_auto_resolver_sweep(trigger: DreamTrigger) -> ResolverReport {
    let started = Instant::now();
    let app_cfg = ha_core::config::cached_config();
    let cfg = app_cfg.dreaming.clone();
    if !cfg.enabled {
...
pub fn run_resolver_cycle_boxed(
    trigger: DreamTrigger,
) -> ha_core::memory::dreaming::ResolverRuntimeFuture {
    Box::pin(run_resolver_cycle(trigger))
}

#[cfg(test)]
mod tests {
    use super::*;
    use ha_core::memory::dreaming::DreamingConfig;
    ...
    fn automatic_conflicts_require_high_confidence_and_only_route_to_review() {
        ...
        let cfg = ha_core::memory::dreaming::DeepResolverConfig::default();
