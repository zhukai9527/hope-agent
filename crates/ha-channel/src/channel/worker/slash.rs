...
use ha_core::channel::db::{ChannelDB, ATTACH_SOURCE_ATTACH};
use ha_core::channel::traits::ChannelPlugin;
use ha_core::channel::types::{ChannelAccountConfig, ChatType, InlineButton};
...
pub(super) enum ChannelSlashOutcome {
    ...
    Reply {
        ...
        buttons: Vec<Vec<ha_core::channel::types::InlineButton>>,
    },
...
pub(super) async fn dispatch_slash_for_channel(
    ...
    match result.action {
        ...
        Some(CommandAction::SwitchModel {
            ...
            } else if let Some(bus) = ha_core::get_event_bus() {
                bus.emit(
        ...
        Some(CommandAction::StopStream) => {
            ...
            let stop = match ha_core::get_session_db() {
                Some(db) => {
                    ha_core::chat_engine::stop::stop_session(db.clone(), session_id, None, false)
                        .await
                }
                None => ha_core::chat_engine::stop::StopSessionOutcome {
                    stopped: ha_core::globals::get_channel_cancels()
        ...
        Some(CommandAction::ExportFile { content, filename }) => {
            let msg = match ha_core::paths::root_dir() {
                Ok(root) => {
        ...
        Some(CommandAction::SetToolPermission { mode }) => {
            let resolved = ha_core::permission::SessionMode::parse_or_default(&mode);
...
async fn set_session_model_core(
    ...
    let session_db = ha_core::require_session_db().map_err(|e| e.to_string())?;
    session_db
...
async fn compact_context_now_core(
    session_id: &str,
    agent_id: &str,
) -> Result<ha_core::context_compact::CompactResult, String> {
    let session_db = ha_core::require_session_db().map_err(|e| e.to_string())?;
    let store = ha_core::config::cached_config();
    ...
    let result =
        ha_core::chat_engine::compact_session_now(ha_core::chat_engine::CompactSessionParams {
            ...
            source: ha_core::chat_engine::ChatSource::Channel,
...
fn is_model_active(
    item: &ha_core::slash_defs::types::ModelPickerItem,
...
pub(super) fn build_model_buttons_from_items(
    ...
    active_provider_id: &Option<String>,
    active_model_id: &Option<String>,
) -> Vec<Vec<ha_core::channel::types::InlineButton>> {
    ...
    for m in models.iter().take(20) {
        ...
        row.push(ha_core::channel::types::InlineButton {
            text: label,
...
fn render_session_picker_buttons_body(
    sessions: &[ha_core::slash_defs::types::SessionPickerItem],
) -> String {
    ...
    for s in sessions.iter().take(SESSION_PICKER_BUTTONS_BODY_LIMIT) {
        lines.push(ha_core::slash_defs::format_session_picker_line(s));
    }
...
fn lookup_command_help(name: &str) -> Option<CommandHelp> {
    use ha_core::slash_defs::{canonical_builtin_command_name, registry};
    ...
    let skill = ha_core::slash_hooks::skill_command_help(name)?;
    Some(CommandHelp {
...
fn render_project_picker_text(
    projects: &[ha_core::slash_defs::types::ProjectPickerItem],
) -> String {
