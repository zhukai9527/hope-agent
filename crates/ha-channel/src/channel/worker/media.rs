...
pub(super) fn convert_inbound_media_to_attachments(
    media: &[InboundMedia],
    session_id: &str,
) -> Vec<ha_core::agent::Attachment> {
    let mut attachments = Vec::new();
    let session_att_dir = ha_core::paths::attachments_dir(session_id).ok();
...
fn sanitize_file_id(raw: &str) -> String {
    ...
    if out.is_empty() {
        ...
        ha_core::truncate_utf8(&out, 64).to_string()
    }
}
...
fn persist_channel_media_to_session(
    ...
    // the same rule.
    let channels_root = match ha_core::paths::channels_dir() {
        Ok(root) => root,
        Err(err) => {
    ...
    match std::fs::rename(&canonical_src, &dest) {
        ...
        Err(err) if ha_core::platform::is_cross_device_rename(&err) => {
            app_debug!(
                "channel",
                "worker",
...
pub(super) async fn transcribe_inbound_voice_attachments(
    ...
    let audio_atts: Vec<&ha_core::agent::Attachment> = attachments
        .iter()
    ...
    let (primary, fallback) = ha_core::stt::current_im_chain();
    if primary.is_none() {
        app_warn!(
            "channel",
            "stt",
    ...
    let futures = audio_atts.into_iter().map(|att| {
        ...
        async move {
            let payload = ha_core::stt::AudioPayload::File { path, mime_type };
            let result = ha_core::stt::failover_transcribe_batch(
                primary,
                fallback,
                payload,
                &ha_core::stt::TranscriptOptions::default(),
                None,
            )
            .await;
            (name, result)
    ...
    for (name, result) in results {
        match result {
            Ok(transcript) if !transcript.text.trim().is_empty() => {
                prefix.push_str(&ha_core::stt::voice_prefix_for_locale(
                    cfg_language,
                    &transcript.text,
                ));
                app_info!(
...
mod tests {
    ...
    fn persist_moves_source_into_session_dir() {
        ...
        ha_core::test_support::with_env_vars(&[("HA_DATA_DIR", data_root.path())], || {
            let inbound_dir = ha_core::paths::channels_dir()
                .unwrap()
                .join("feishu")
                .join("inbound-temp");
