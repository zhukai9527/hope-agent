...
use ha_core::channel::types::{InlineButton, ReplyPayload};
use ha_core::tools::{
...
fn hint_throttle_duration() -> Duration {
    let secs = ha_core::config::cached_config()
        .permission
...
fn snippet_detail(detail: &Option<String>) -> Option<&str> {
    ...
    Some(ha_core::truncate_utf8(trimmed, 280))
}
...
fn format_approval_text_for_locale(
    ...
    let preview = ha_core::truncate_utf8(command, 500);
    format!(
        "{}\n\n{}{}",
...
fn id_tag(request_id: &str) -> &str {
    ha_core::truncate_utf8(request_id, 6)
}
...
pub fn spawn_channel_approval_listener(channel_db: Arc<ChannelDB>, registry: Arc<ChannelRegistry>) {
    ...
    tokio::spawn(async move {
        loop {
            ...
            let request: ha_core::tools::ApprovalRequest =
            ...
            let payload = if supports_buttons {
                ...
                ReplyPayload {
                    text: Some(format_text_approval(
                        ...
                        &request.request_id,
                        stack_depth,
                        ha_core::tools::approval_timeout_secs(),
                    )),
                    thread_id: conversation.thread_id.clone(),
                    ..ReplyPayload::text("")
                }
...
struct ApprovalTimedOut {
    ...
    #[serde(default)]
    timeout_action: ha_core::config::ApprovalTimeoutAction,
}
...
fn approval_timeout_notice(
    ...
    let template = match action {
        ha_core::config::ApprovalTimeoutAction::Deny => tr(
            locale,
            [
        ...
        ha_core::config::ApprovalTimeoutAction::Proceed => tr(
            locale,
            [
...
pub async fn try_handle_approval_reply(msg: &ha_core::channel::types::MsgContext) -> bool {
...
async fn send_source_mismatch_notice(msg: &ha_core::channel::types::MsgContext, request_id: &str) {
    let store = ha_core::config::cached_config();
    let locale = ha_core::i18n::effective_ui_locale(&store);
    let Some(account_config) = store.channels.find_account(&msg.account_id).cloned() else {
        return;
...
async fn send_allow_always_unavailable_notice(
    msg: &ha_core::channel::types::MsgContext,
    tag: &str,
) {
    let store = ha_core::config::cached_config();
