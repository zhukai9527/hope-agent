...
use ha_core::channel::types::{MediaData, MediaType, OutboundMedia};
...
async fn upload_media_to_wechat(
    ...
    for attempt in 0..3 {
        ...
        let err_msg = format!(
            ...
            status,
            ha_core::truncate_utf8(&body_preview, 300)
        );
...
fn outbound_temp_dir() -> Result<PathBuf> {
    Ok(ha_core::paths::channel_dir("wechat")?.join("outbound-temp"))
}
