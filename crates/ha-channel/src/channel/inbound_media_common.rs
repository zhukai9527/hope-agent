...
use ha_core::channel::types::{MediaType, MsgContext};
...
pub async fn inbound_temp_path(
    ...
    let dir = ha_core::paths::channel_dir(channel_id)?.join("inbound-temp");
...
/// [`ha_core::security::ssrf::check_url`] (or be downloading from a known
...
pub async fn stream_to_disk(
    ...
    if !status.is_success() {
        ...
        return Err(anyhow!(
            "HTTP {}: {}",
            status,
            ha_core::truncate_utf8(&body_text, 512)
        ));
    }
...
mod tests {
    use super::*;
    use ha_core::channel::types::{ChannelId, ChatType};
    ...
    async fn inbound_temp_path_sanitizes_separators() {
        ...
        let path =
            ha_core::test_support::with_env_vars_async(&[("HA_DATA_DIR", root.path())], || async {
