...
fn load_session(account_id: &str, shard_id: u64, shard_count: u64) -> Option<SessionFile> {
    let dir = ha_core::paths::channel_dir("qqbot").ok()?;
...
fn save_session(account_id: &str, shard_id: u64, shard_count: u64, session_id: &str, seq: u64) {
    if let Ok(dir) = ha_core::paths::channel_dir("qqbot") {
                        ...
                        if let Some(session_id) = d.get("session_id").and_then(|v| v.as_str()) {
                            *saved_session_id = Some(session_id.to_string());
                            app_info!(
                                ...
                                account_id,
                                ha_core::truncate_utf8(session_id, 30)
                            );
                        }
                            ...
                            tokio::spawn(async move {
                                if let Err(e) = api_clone.ack_interaction(&interaction_id).await {
                                    app_warn!(
                                        ...
                                        ha_core::truncate_utf8(&interaction_id, 30),
                                        e
                                    );
                                }
                            });
