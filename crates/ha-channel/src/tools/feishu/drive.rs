...
use ha_core::tool_defs::feishu_names::{
    TOOL_DRIVE_DOWNLOAD_MEDIA, TOOL_DRIVE_LIST_FILES, TOOL_DRIVE_UPLOAD_MEDIA,
};
...
pub fn list_files_tool() -> ToolDefinition {
    ToolDefinition {
        ...
        background_policy: ha_core::tool_defs::BackgroundPolicy::ForegroundOnly,
        parameters: json!({
            "type": "object",
            "properties": {
                "folder_token": {
...
fn require_absolute_path(raw: &str) -> Result<PathBuf> {
    let expanded = ha_core::tools::expand_tilde(raw);
