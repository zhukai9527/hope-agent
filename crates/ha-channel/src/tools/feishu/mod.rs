...
// 名字常量已下沉 kernel 契约层，由上方 `pub use ha_core::tool_defs::feishu_names::*;`
// 统一再导出——此处不再逐个子模块 re-export。
...
use ha_core::channel::types::{ChannelAccountConfig, ChannelId};
use ha_core::config::cached_config;
use ha_core::tool_defs::{ToolDefinition, ToolTier};

// 名字常量下沉 kernel 契约层（见 `ha_core::tool_defs::feishu_names` 模块文档）；
// 此处 glob 再导出，`feishu::TOOL_*` 既有路径逐字不变。
pub use ha_core::tool_defs::feishu_names::*;
