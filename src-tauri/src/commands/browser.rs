...
) -> Result<ha_browser::browser::BrowserExtensionStatus, CmdError> {
    Ok(browser_ui::extension_status())
}

#[tauri::command]
pub async fn browser_install_native_host_manifest(
    request: ha_browser::browser::NativeHostInstallRequest,
) -> Result<ha_browser::browser::NativeHostInstallResult, CmdError> {
    browser_ui::install_native_host_manifest(request).map_err(Into::into)
}

#[tauri::command]
pub async fn browser_extension_stop_control(
) -> Result<ha_browser::browser::BrowserExtensionStopResult, CmdError> {
...
) -> Result<Option<ha_browser::browser::frame::BrowserFramePayload>, CmdError> {
    ha_browser::browser::frame::capture_frame(session_id.as_deref())
        .await
        .map_err(Into::into)
}
...
pub async fn browser_panel_navigate(
    ...
    ha_browser::browser_ui::panel_navigate(&op, url.as_deref(), session_id.as_deref())
        .await
        .map_err(Into::into)
}
...
/// `browser_connect`. See [`ha_browser::browser::user_attach`].
#[tauri::command]
pub async fn browser_spawn_user_chrome(
    args: ha_browser::browser::user_attach::SpawnUserChromeArgs,
) -> Result<ha_browser::browser::user_attach::SpawnUserChromeResult, CmdError> {
...
) -> Result<ha_browser::browser::user_attach::BrowserDoctorReport, CmdError> {
    Ok(ha_browser::browser::user_attach::browser_doctor().await)
}

/// Read `AppConfig.browser` for the settings panel.
#[tauri::command]
pub async fn browser_get_config() -> Result<ha_browser::browser::BrowserConfig, CmdError> {
...
pub async fn browser_set_config(
    config: ha_browser::browser::BrowserConfig,
) -> Result<(), CmdError> {
