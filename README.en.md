...
<tr><td><b>🔧 Toolbox</b></td><td>Controllable browser (8 high-level actions with a <b>live mirror panel</b> on the chat sidebar — Chrome stays visible as the agent drives it; direct CDP via chromiumoxide, zero runtime dependencies), Canvas, AI image generation (7 providers), web search (8 providers with failover), bash (optional Docker sandbox), file read/grep/find, URL preview, crash journal, self-diagnosis.</td></tr>
