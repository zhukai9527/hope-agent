...
> **已实现（与上表的偏差，见 §10）**：上面的 `per_kind` 三层表是**设计愿景**;实际落地为**双域分治**而非单一 struct——SlotManager 只管 Tool kind（全局 `max_concurrent_jobs` + 每会话 `max_concurrent_jobs_per_session`,auto-bg 计入）;subagent 的 per-kind 配额留在 subagent 域（R7.2 的 per-session 拒绝)。把异质的 tool 排队 / subagent 拒绝 / 信号量塞进一个 per-kind 结构属 speculative generality（YAGNI),且 subagent reject→queue 的真统一是高风险大改（§8.2),与 R7.4 一并推迟。
...
- ✅ **R3 ① 运行中输出尾巴 `output_tail`（本期子集）**：后台 `exec` job 的 stdout/stderr 实时 tee 进有界 ring（`async_jobs::output_tail`，默认尾部 8KB，`DEFAULT_TAIL_BYTES`），`job_status(action:status)` 对 running/cancelling job 返回 `output_tail`（BashOutput 对齐——判断「在跑还是卡住」不必等完成）。**加法式设计**：`spawn_exec_waiter` 仅当 `ctx.output_tail_job_id` 为 Some（后台 + 非 incognito exec）时走 `wait_with_output_teed`（并发读双管 + tee，返回完整 `Output` 不变）；**前台同步 exec 走原 `wait_with_output` 完全不动**。ring 在 spawn 时按 `tool_name==exec && !incognito` 注册，finalize_job / 队列取消 / 会话删（经 `cancel_job`）三处清；incognito 永不注册（关闭即焚）。`ToolExecContext` 加 `output_tail_job_id`。含 5 单测。
  - **R3 对抗审查（22 agent，17 候选去重 3 类全处理，commit bde71c7c）**：(A) ring 泄漏 3 早退路径（record_running_job 失败 / 队列满回滚 / start_runner runtime-build 失败均绕过 finalize_job）→ 补 `output_tail::remove`（5 条终态路径全配对）；(B) `drain` 把管道读错误当 EOF 吞掉 → 改 `io::Result` + `try_join!` 传播（对齐 stock `wait_with_output`）；(C) 自动后台化 exec 不注册 ring（无 output_tail）→ 判定一致性缺口非 bug，auto-bg 属独立关注点（待 R1），加注释 + 本记录，本期不接。
  - **未做（待 R4/R1）**：`job:*` 事件命名空间 + 进度事件（`job:progress` stdout 字节增量 / subagent 轮次 / Group N/M）——需 R4 前端面板消费 + R1 的 Group；`output_tail` 尾巴大小配置化待 R9；auto-background exec 的 output_tail（C）待 R1 统一模型。
- ✅ **R3 事件命名空间统一 + auto-bg output_tail（commit 655a610b + 审查加固）**：P1「看得见」后端地基。**统一 `job:*` 事件**（`async_jobs/events.rs`）：`job:{created,updated,progress,completed}` + 告警 `job:mark_injected_failed`，替代旧 `async_tool_job:*`（破坏性 drop，前端 `AsyncJobCancelCard` 两 listener 同步改），kind-tagged（`tool` / `group`）+ `session_id`。生命周期埋点：tool `created`（显式 spawn + auto-bg detach，一次）/ `completed`（finalize）/ `updated|completed`（cancel）；group `created`（spawn_group）/ `progress`（每子结算按 N/M，复用 group_children 一次加载）/ `completed`（join CAS 认领后）。`subagent` kind **沿用 `subagent:*` 流不双发**（避免双发 + sync 加载成本），R4 面板合并两路 + `job_status list`。取消 kind=Group 仅在 `update_terminal` 真翻转时发 `completed`（防与 join CAS 竞争产生重复终态）。**auto-background exec output_tail（收口 R3 ① 的 C，R1 后解锁）**：auto-detach 的长 exec 现也 tee 运行中输出进 ring，`job_status` 可查（与显式 `run_in_background` 对齐）。ring 在 worker 内 runtime 起来后注册（runtime-build 失败先于注册无泄漏），清理统一——非 detach 终局 `next.is_none()`（同步完成 inline / awaiter bail）worker 内清、detach 走 finalize 清、DB-not-init detached 早退补一处清。**对抗审查（ring 生命周期 + 事件双发/漏发/kind 两视角）0 CONFIRMED**，修 1 个自查的重复终态（group 取消加 `cancelled_now` 守卫）+ 1 处 ring-leak 加固。门禁 ha-core 2070 + clippy + typecheck 全绿。
  - **未做（待 R4）**：tool stdout 字节增量进度 / subagent 轮次进度（消费者驱动，R4 面板定义所需粒度时再加）；`output_tail` 尾巴大小 + 进度节流配置化待 R9。
- 🔬 **对抗审查修复（R2+R10+R5，3 CONFIRMED 全修）**：(1) wakeup `mark_fired` 吞错 → 改 `mark_fired_with_retry`（镜像 `mark_injected_with_retry`，retry+log），防瞬时 SQLite 失败留 `fired=0` 导致重启重投重复计费 turn；(2) `job_status` `action_wait` 静默截断 `ids` 到 32 → 加 `dropped`+`truncated` 字段，模型不再把被丢弃的 id 误当完成；(3) wakeup 复用注入管线被打 `subagent_result` 标记 → 前端误渲染成绿色「已完成」pill 且 note 丢失：改写 `wakeup_trigger` 专属 meta（`injection.rs` 按 `WAKEUP_CHILD_AGENT_ID` 分支，镜像 cron `cron_trigger`）+ 前端 `WakeupTriggerBubble`（紫色「定时唤醒」chip，可展开）+ `isWakeupTrigger` 解析 + i18n 12 语。
- ✅ **R1 统一 Background Job 模型 + JobManager 地基（commit 2b1af486 + 73dac2be）**：异步工具子系统升格为统一后台任务模型,解锁后续 R5-Group / R6 / R7 per-kind。
  - **领域模型 + 持久化（1/2）**：`AsyncJob`→`BackgroundJob`、`AsyncJobStatus`→`JobStatus`、`AsyncJobsDB`→`JobsDB`;新增 `JobKind { Tool | Subagent | Group }` 枚举 + `BackgroundJob.kind` 字段（Tool 今日唯一落地,Subagent/Group 是 R6/R5 的类型化接缝）。表 `async_tool_jobs`→`background_jobs`、DB 文件 `async_jobs.db`→`background_jobs.db`、spool 目录同步;`kind` 列(末列,索引不重排,默认 'tool')+ stale-schema 探针改 `SELECT kind`(升级即 drop-rebuild,纯缓存无迁移);未知 kind 回退 Tool;启动期 best-effort 清理旧 `async_jobs.db`/`async_jobs/`。
  - **JobManager 单一入口（2/2）**：`async_jobs::JobManager` front 全部 spawn_tool / dispatch / get / list / cancel / cancel_for_session / purge_for_session / replay / run_scheduler / retention（薄委托）。全部生产调用方（execution / runtime_tasks / job_status / streaming_loop / cleanup_watcher / project::files / app_init）迁移到 JobManager;`spawn_explicit_job` 等收敛为 `pub(crate)` Tool executor 内部 + `get/set_async_jobs_db` 降 pub(crate)(白盒读)。R6/R5 在此并列加 `spawn_subagent`/`spawn_group`。
  - **刻意延后（YAGNI + drop-rebuild 零成本）**：`group_id`/`subagent_run_id`/`spec_json`/`progress_json`/`priority`/`attempt` 列 + `queued_at`/`started_at` 拆分 + `JobExecutor` trait(待第二个 executor 到场,避免 speculative generality)。模块名 `async_jobs/` + log category `"async_jobs"` 按 §4.3 保留;`RuntimeTaskKind::AsyncJob`/`async_tool_job:*` 事件不变(R3/R4 再统一前缀)。全 workspace 编译 + 44 async_jobs + 19 job_status 单测全过。
- ✅ **R6 后台 subagent 统一为 Background Job（commit a668c96e）**：把用户委派的后台 subagent run 投影进统一 background_jobs 面(kind=Subagent),与后台工具 job 共享 job_status list/status/cancel + 未来 R4 面板。**严格单向投影**:subagent_runs 是执行真相源,background_jobs 只承载调度/生命周期、绝不持有 run 正文(task/result/error 只在 subagent_runs)、绝不反写。
  - **模型/持久化**:BackgroundJob + `subagent_run_id` 字段 + 同名列(末列 index 19)+ 索引;探针改 `SELECT subagent_run_id`(drop-rebuild)。`update_subagent_projection_status`(按 run_id 同步 status+completed_at,scoped `kind='subagent'`,terminal 冻结防 late-sync 重开,无投影即 no-op)。
  - **JobManager 投影 API**:`project_subagent_spawn`(建投影,**injected=true 永不进工具注入/replay 路径**——subagent 有自己的 inject_and_run_parent;args_json 空)+ `sync_subagent_projection`(SubagentStatus→JobStatus 映射)。
  - **三接线**:**建** 在 spawn_subagent 插 run 后 gate `!skip_parent_injection`(排除 plan/team/hook 内部 spawn)`&& !parent_incognito`(关闭即焚);**同步** 在单一 choke point update_subagent_status 后(覆盖 run 生命周期 + 三处 kill fallback,先释放 SessionDB 锁再跨库);**取消** cancel_job 对 kind=Subagent 路由到 `subagent::request_cancel_run`(注册表 cancel + DB-Killed 兜底,与 kill 工具同源),不跑工具 job 的 hook/注入。**顺带修** cancel_jobs_for_session 现在也取消会话删除时的后台 subagent(此前缺口)。
  - **job_status**:响应增 `kind`;subagent 投影补 `subagent_run_id` + 终态提示走 `subagent(action='result')` 取正文。
  - **刻意延后**:cleanup_orphan_subagent_runs 的 raw-SQL 路径不同步投影(重启由 replay 标 Interrupted 兜底);Group 子 job 用 group_id 关联(R5)。
- ✅ **R5 Group fan-out（commit 9933ed0f + 审查修复 770446b0）**：`batch_spawn` 升格为统一 **Group**——一次 fan-out N 个后台 subagent,全部到终态时**合并注入一轮**(非 N 轮),收口 PRD P2 核心差距「委派并发后台工作并等齐收集结果」。**join-all-settle**:等所有子到终态(不 fail-fast),一并返回部分成功 + 各自终态。
  - **模型/持久化**:BackgroundJob + `group_id` 字段 + 同名列(末列 index 20)+ 索引;探针改 `SELECT group_id`(drop-rebuild)。新 DB 方法 `group_children` / `group_id_for_subagent_run`(scoped `kind='subagent'`)/ `mark_group_sealed`(写 `args_json={"sealed":true}`)/ `claim_group_completion`(**单赢 CAS**:`Running→Completed` only if 非终态)。
  - **JobManager**:`spawn_group`(建 group 行 `Running`+`{"sealed":false}`+**injected=true**——group 自己发合并注入,不进工具 replay)+ `seal_group`(标 sealed 后跑一次完成检查,兜底「spawn 期间子已全完成」)+ `try_complete_group`(sealed + 全子终态 → CAS 认领 → 一条合并注入)+ `build_group_push_message`(单 `<subagent-result>` 封套,`<result>` body 枚举每子 status/result/error + **task/label**,XML-escape;复用既有前端 pill,零前端/零文案)+ `group_progress`(N-of-M)。**取消** cancel_job kind=Group **先标 group Cancelled 再取消子 run**(顺序 load-bearing:无 flag 兜底会同步标子 Killed → 同线程触发 try_complete_group,group 须已终态使 CAS 落败)。
  - **subagent**:`SpawnParams.group_id`;批量子投影携带 group_id,**投影失败则 ungroup 回退个体注入**(不丢结果);grouped 子代理**抑制个体注入**,Group 统一交付(覆盖全终态含 Killed,个体路径只发 Completed/Error/Timeout)。
  - **batch_spawn**:**建 group 前预校验全部 task**(防第 k>0 个非法 `?` 早退 → 已建 group + 子代理永不 seal → 漏交付);非 incognito 建 Group → 子代理打 group_id → 收尾 seal_group;响应返回 `group_id` + delivery 提示。**fetched-all 跳过冗余注入**(`take_runs_fetched` drain,顺带清抑制注入留下的 FETCHED 泄漏)。incognito / jobs DB 未初始化 → 不建 group,回退个体注入(pre-R5 行为)。
  - **job_status**:kind=Group surfacing N-of-M(child_count / terminal / completed / failed)+ 合并交付提示。
  - **刻意延后 / narrow gap**:重启 in-flight group + 子(`injected=true`)只标 Interrupted(不进 replay 注入路径),合并注入不补发(子结果仍在 subagent_runs 可查)——与 R6 对称。`Group` 暂只 `batch_spawn` 触发(单 `spawn` / `spawn_and_wait` 仍个体投影 + 个体注入)。
- ✅ **R4 前端后台任务面板 + 徽标 + 完成通知 + 注入合并窗口（commit c1405f81 后端 + 192ee055 前端）**：P1「看得见」前端收尾,消费 R3 的 `job:*` 事件。
  - **owner-plane 端点**：`list_background_jobs(session)` / `get_background_job(id)`（Tauri + HTTP `/api/sessions/{id}/background-jobs`、`/api/background-jobs/{id}`,Transport COMMAND_MAP 双适配,Bearer 鉴权同 session 端点）。`BackgroundJobSnapshot`（camelCase 展示向,无 agent 引导 hint,与 model-facing `job_status` JSON 分离）由 `JobManager::list_session_snapshots`/`get_job_snapshot` 构建；**Group 子投影折叠进 Group 行**（面板看 N/M,不看 N 子行）,exec 取命令首行为标签,running exec 带 `output_tail`（仅单查）。`db.list_for_session`（活跃优先 + 最近终态,cap 50）。取消复用 `cancel_runtime_task(kind=async_job)`,不新增端点。
  - **完成注入合并窗口（控成本)**：`async_tools.completionMergeWindowSecs`（默认 3,`0` 关）。同会话多个 tool job 在窗口内完成 → 合并一条 `<task-notification-batch>` 一轮注入而非 N 轮计费。首个完成开窗 + 起定时器,窗口内入批,flush 原子取空（后到开新窗）。纯内存 live-path 优化:崩溃则行 terminal-but-uninjected,重启 `replay_pending_jobs` 各自补投（不丢、不合并）。Group(R5) 是预合并特例绕过。沿用 `dispatch_injection` 的 ghost-turn 闸 + 逐 job claim/release + on_injected 逐行恰好一次。`finalize_job` + 构建失败路径均改走 `enqueue_injection`。
  - **前端**：`useBackgroundJobs(sessionId)`（list 播种 + 订阅 `job:{created,updated,progress,completed}` 按 session 过滤 + `subagent_event` 按 parentSessionId,防抖 refetch,常驻 ChatScreen 级喂徽标 / 面板 / 工作台同一份)；独立「后台任务」右侧互斥面板（本会话区可取消 + Group N/M 进度条 + 本地模型只读镜像,复用 `local_model_job:*` 不动现有 4 处入口)；工作台简化区块（速览 + 跳独立面板,label/chip/icon 经 `jobDisplay.tsx` 共享）；ChatTitleBar 在跑计数琥珀徽标 + 开关按钮（单会话作用域）；`useDesktopAlerts` 加 `job:completed`（completed/failed/timed_out 才弹,跳过 cancelled/interrupted,受 `notifyOnBackgroundJobComplete` + `notifyIfBackground` 双门)。
  - **设置三件套**：`notification.notifyOnBackgroundJobComplete`（默认开,LOW,NotificationPanel + serde read/update）+ `async_tools.completionMergeWindowSecs`（MEDIUM,AsyncToolsPanel + serde read/update);均登记 `ha-settings` SKILL.md。i18n 新增 `backgroundJobs` 命名空间 + notification/settings 键,全 12 语齐全。
- ✅ **R7.1 分层配额闭环 — per-session 强约束 + auto-bg 计入**：把「单一全局数」升级为**全局 + 每会话**两层硬配额,并修补 auto-background 不占槽的漏。
  - **每会话硬上限**:`async_tools.maxConcurrentJobsPerSession`（**默认按硬件取值 ≈ 全局 cap 的 3/4、恒 < 全局、band [3,12]**,`0` = 不限）。此前 per-session 计数**仅用于排队公平排序**,单会话仍能占满全局池饿死其他会话;现 `SlotManager::reserve_inner` 同时校验全局 + 每会话两层,任一满即入队。调度器 `pick_fair_index` 跳过已达每会话上限的会话（防 head-of-line:capped 会话的队头不阻塞其他会话的提升）,空位再起靠 RAII `SlotReservation` Drop 唤醒。
  - **auto-bg 计入配额**:慢同步工具超 `auto_background_secs` detach 后,此前**不占任何槽**（漏）。现 detach 点（主线程 Phase→DetachedRunning,持 phase 锁内）取 `reserve_forced`（任务已在跑,不能排队/拒绝,只能**强制计数**,可短暂超全局 cap),reservation 经 `Arc<Mutex<Option<SlotReservation>>>` 与 worker 线程共享 → worker 线程结束（所有终态路径）时自然 Drop 释放。auto-bg 也计入每会话配额（防绕过)。
  - **per-kind 按双域分治落地（非单一结构）**:SlotManager 今天只跑 Tool kind,故 tool 池由「全局 + 每会话」治理;**subagent 池在 subagent 域**（`subagent::spawn` 的 `count_active_subagent_runs >= max_concurrent_for_agent` 每会话拒绝,默认 8,R7.2 已接死配置 `subagents.maxConcurrent`);**同轮安全工具**走信号量 8（R7.3）;结构类（depth/batch/turn）仍拒绝不排队。把这三类塞进一个 per-kind struct 属 speculative generality（YAGNI）——subagent reject→queue 的真统一是高风险大改（§8.2「需先勘探」),与 R7.4 一并刻意推迟。
  - **三件套**:`maxConcurrentJobsPerSession`（MEDIUM,AsyncToolsPanel + 既有 `async_tools` category 泛型 merge read/update + SKILL.md 登记)。i18n `settings.asyncToolsMaxConcurrentPerSession{,Desc}` 全 12 语。slots 单测覆盖每会话拒绝 / 跳过 capped 会话 / forced 越 cap;config 单测覆盖默认恒 < 全局 + floor 2 + `0` 不限。
  - **对抗审查（4 角度 finder × 逐候选 verify,13 候选 → 5 CONFIRMED + 7 PLAUSIBLE）抓 1 真缺陷全修 + 3 文档精确化**:(CONFIRMED 真缺陷) per-session 默认硬编码 `6` ≥ 全局 cap(`clamp(cores-2,4,16)`)在 ≤8 逻辑核机器上(8 线程笔记本→全局 6)**fairness 整层 no-op**,单会话先撞全局 cap → 改默认为**硬件推导 ≈ 全局 3/4、恒 < 全局**;(文档) module-doc「`max_concurrent_jobs==0` = no queueing」漏了 per-session cap 仍排队、调度器 fallback-tick 注释漏了 per-session 旋钮也是 no-notify 事件、auto-bg `reserve_forced` 文档精确化为「计数但不拒绝、可短暂超全局**及每会话** cap」(per-session cap 约束**准入**非已在跑的 auto-detach 数)。**刻意不修**(PLAUSIBLE,无 live trigger / 边界自愈):session_id=None 共享空串桶(当前无路径以 None 起后台 job)、FIFO 门让 under-cap 会话排一跳(≤5s 自愈)、cancel 撞调度器 dequeue(token 兜底无 live bug、mod.rs 不在本 diff)、remove_queued 不动计数(今日正确、latent)。另 1 个 CONFIRMED 判为**预存正交问题、加注释不修**:会话在 detach 窗口(行 insert 前 sub-ms 间隙)被删 → `cancel_for_session` 按 DB 行查不到该 job 而漏取消,worker 跑成 orphan——这是**预存的取消子系统缺口**(R7.1 前 worker 也先于行 insert spawn),R7.1 的 forced reservation 只是**正确地**计入这条 live 线程并在其结束时释放(非 leak),真修属取消正确性、不在本刀范围。
- ✅ **R7.4 重试（per-tool,安全默认）**：后台 job 因瞬时错误失败时自动退避重投,关闭 R7 验收的「重试单测」项。**纯策略层 `async_jobs/retry.rs`**(`decide(tool,attempt,error,cfg)->Stop|Retry{backoff_ms}`,6 个纯单测):**仅 `Failed` 类重投**(干净 dispatch 错误、job token 未动)且仅对**幂等 / 无副作用工具**(`web_search`/`web_fetch`,代码级 `is_retry_eligible` 白名单、**非用户旋钮**——防 `exec` 被配出自动重投重复副作用);`exec`/`image_generate`(副作用/计费)永不重投;`Cancelled`/`DeniedByUser` 永不重投。**`TimedOut` 刻意排除**(per-job 超时会 cancel 共享 job token,无 per-attempt token 无法干净重起;且超预算的工具大概率再超时)——退避指数(500ms 起、shift cap 32s)。**接线**:抽共享 `run_tool_once`(原 dispatch+timeout+cancel select 逐字保留)+ `run_tool_with_retry`(loop + `retry::decide` + **可取消退避**:退避期 job-level cancel/跨进程 watcher 翻 token → 停重投settle Cancelled),explicit (`run_job_to_completion`) + auto-bg worker 两路共用(auto-bg 重投只是更久 → 触发主线程 detach,正确地变后台 job 续重投)。三件套:`retryEnabled`(默认 true)+ `maxRetryAttempts`(默认 3,含首次,1=不重投)MEDIUM,AsyncToolsPanel + 泛型 merge + SKILL.md;i18n 全 12 语。
- ✅ **R7.0/R7.4 结构类拒绝断言用例(验收项)**：`subagent::spawn::structural_limit_tests` 断言结构类限制**拒绝不排队**——`subagent_depth_overflow_rejects_not_queues`(depth 99 > 默认 3 → Err「depth limit」,不入 SlotManager 队列)+ `batch_size_cap_is_a_fixed_structural_limit`(batch 默认 10 是固定结构帽非可调队列深度)。守 R7.1 的「reject→queue」改动不漏进结构路径。
- ⏳ **待续**：R7.4 共享 executor（Send 工具用 tokio 任务 + 信号量替代 thread-per-job,§8.2 标「需先勘探不可一刀切」——独立一刀)、subagent reject→queue 真统一（高风险大改、不在验收清单）、R8（AwaitingApproval 落地,需审批引擎,#318 已合 main）、R9（设置三件套横切剩余:wakeup clamp / 队列上限 / output_tail 尺寸 / 进度节流)。**P0 地基 + P1（看得见,后端事件 + 前端面板）+ P2 编排 + R7 调度健壮性（分层配额 + 重试 + 结构类拒绝验收）已全部完成；R7 剩共享 executor 这一可选重构。**
