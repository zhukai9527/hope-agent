...
- **跨进程队列归属（契约）**：桌面 + 自托管 server 同享 `background_jobs.db`，但**调度器/队列是 Primary 进程单点**（对齐 `replay_pending_jobs` 的 Primary-only 模式），Secondary 不自起调度器、不消费队列——否则两个内存调度器 = 容量翻倍或双执行。配额计数以 Primary 的内存态为准；跨进程取消仍走既有 DB flag。**`queued` 行的 owner = Primary**，Primary 切换时由新 Primary 接管未起跑队列。
