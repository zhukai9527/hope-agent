...
| [真实模型与复杂任务评测](architecture/live-model-evaluation.md) | 与确定性证据物理分轨的真实 Provider Campaign：不可变计划、重复/统计、任务终态 verifier、Goal/Workflow/Async/Subagent 归因、专用受保护 Runner 与独立 release preflight | `ha-eval-spec::model`, `hope-agent-eval model`, `evals/live/`, `model-campaign.yml` |
