...
| [Coding Eval 控制面评测](architecture/coding-eval.md) | Phase 3.7-3.11 确定性 fixture harness：临时 git repo + 真实 session/goal/task/workflow/IDE context/improvement state，回归 Review / Smart Verification / Context Retrieval / Improvement Loop 协同质量 | `coding_eval.rs`, `tests/fixtures/coding_eval/` |
| [Coding Improvement Loop](architecture/coding-improvement-loop.md) | Phase 3.11 质量趋势与改进候选：基于 durable Goal/Workflow/Review/Verification/Eval 数据生成 trend report、failure taxonomy 和 draft-only proposal 队列 | `coding_improvement.rs`, `coding_eval.rs`, `components/chat/workspace/` |
