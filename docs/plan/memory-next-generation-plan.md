...
398. 已完成：同步主分支的统一后台模型调用底座与统一 token usage ledger。Dreaming Narrative、Profile Rewrite、手动 Deep Resolver、自动 graph-first Resolver 不再各自构造单模型 Agent，而是统一解析 `modelOverride → deprecated narrativeModel → function_models.automation → chat active/fallback`，经 `automation::run` 获得跨模型降级；旧配置无需物理迁移。四类调用分别以 `dreaming.narrative`、`dreaming.profile_rewrite`、`dreaming.resolver.manual`、`dreaming.resolver.auto` 写入 `model_usage_events.operation`，Dashboard 可按 operation 精确看输入 / 输出 token、调用次数和模型，也可按 `dreaming` domain 汇总。Retrieval Planner / graph-first 确定性规划不调用 LLM，外部 memory adapter 的 HTTP 用量也不伪装成模型 token；本期新增模型成本只来自 narrative/profile 和有界 resolver 分组，其中自动 resolver 默认最多 8 组、单次输出预算 512 tokens，失败与降级尝试仍沿统一 side-query ledger 记账。同步同时保留本期 claim/evidence/source 筛选、owner 变更事件、错误可见与保存失败回滚，并纳入主分支的 blocking DB owner handler、模型链 GUI、Knowledge OCR / note-tools 路由和 Recall Summary 配置。
399. 已完成：主分支同步后的非 GUI 回归再次全绿。所有新增 memory owner handler（claim / audit / Episode / Procedure / backup / restore / health / repair）与主分支 `blocking::run_blocking` 契约对齐，避免同步 SQLite、ZIP 和文件 IO 占住 async runtime worker；统一模型链组件对混合版本 transport 返回的 `null` 模型/配置 fail-soft 归一为空列表或安全默认值，Source-to-note 模型链保存失败仍回滚到上次确认值并显示脱敏 detail。最终 `cargo fmt --all --check`、`cargo clippy -p ha-core -p ha-server --all-targets --locked -- -D warnings`、`cargo test -p ha-core -p ha-server --locked`、`pnpm typecheck`、`pnpm lint`、`pnpm test` 全部通过：ha-core 2751 passed / 1 ignored scale benchmark，ha-server 36 passed，前端 143 files / 665 tests，ESLint 0 errors（11 个既有非阻断 hook warnings）。UI 实机验收继续由用户自行完成，不计入本期剩余工作。
400. 已完成：同步到当时最新 `origin/main@f00af8c1a` 的知识空间导入 / 同步进度体验。主分支把旧 `KnowledgeJobsButton` 合并为统一 `KnowledgeActivityButton`，同时展示 reindex/re-embed 与 source import 活动；本期的“任务列表失败不可伪装成空态、cancel/retry/clear 错误按动作提示、错误 detail 脱敏、job error 不裸显”契约已迁入新面板，旧入口按上游设计删除。`KnowledgeSourcesPanel` 同时保留运行中 import run 自动恢复追踪和详情读取失败反馈。增量复验通过三包 `cargo check`、12 语种 6338 keys / 缺失 0、`pnpm typecheck`、143 files / 665 tests 与 ESLint 0 errors（11 个既有 warning）。
401. 已完成：Memory 设置页多语言裸 key 与英文 fallback 回归修复。`settings.tabOverview` 的根因是文案已存在于 `project.tabOverview`，但 Memory Settings tab 读取 `settings.tabOverview`；旧 `sync-i18n --check` 只比较非英文 locale 相对 `en.json` 的缺失，所以当 12 种语言与英文基准一起漏同一个路径时会全部通过。现在 12 个 locale 均补齐 `settings.tabOverview`，Memory 面板里另外 3 处无 fallback 的 `common.copy/details` 调用改为已有的 `chat.copy/details` 基准 key；`scripts/sync-i18n.mjs --check` 额外扫描源码裸 `t("...")` 和 `t("...", "defaultValue")` 调用并要求它们必须进入 `en.json`，避免未来再次出现所有语言共同漏 key，或靠代码内英文 defaultValue 漏到非英文界面。后续验收又发现 Overview 卡片 `Recent memory sources`、外部记忆 Provider 摘要、Experience filter、backup / restore、Answer Memory Chips 与 Workspace 记忆诊断里还有代码 fallback 文案；已把这些 key 正式补入 12 个 locale，简中 / 繁中补齐中文，其他语种先沿用英文基准。增量复验通过：12 语种 6488 keys / 缺失 0 / 裸 source key 缺失 0 / fallback source key 缺失 0、`pnpm typecheck`。该步骤不改变记忆运行时、prompt 注入、召回排序、外部 provider、Deep Resolver 或模型 token 统计，只把记忆相关 UI 的多语言体验和 i18n 基线检查收口到可防回归状态。
402. 已完成：多语言内容质量审计工具落地，并完成 Memory 面板 11 个非英文 locale 的可见英文清零。新增 `scripts/audit-i18n-quality.mjs` 与 `pnpm i18n:audit`，可按 locale 与源码目录扫描“源码实际引用的 key 是否仍等于英文基准”，并支持 `--fail-on-same` 作为局部门禁；同时 `sync-i18n` 已覆盖源码裸 key 与 defaultValue key 缺基准两类问题。先前验收发现简中 / 繁中已清零但 ar/es/ja/ko/ms/pt/ru/tr/vi 在 Memory 面板仍各有 476-517 条英文兜底；现在已按语种批量补齐这些 Memory 面板文案，并对少量占位符异常做人工修正。当前证据：`pnpm i18n:audit -- --locales ar,es,ja,ko,ms,pt,ru,tr,vi,zh,zh-TW --source src/components/settings/memory-panel --fail-on-same` 全部为 0，Memory 面板占位符一致性问题 0，`pnpm i18n:check` 通过 12 语种 6488 keys / 缺失 0，`pnpm typecheck` 与 `git diff --check` 通过。该步骤不改变记忆运行时、prompt 注入、召回排序、外部 provider、Deep Resolver 或模型 token 统计，只把本次记忆相关 UI 的多语言体验从结构完整推进到内容审计可验证。全仓范围仍可继续用 `pnpm i18n:audit -- --locales zh,zh-TW --source src` 追踪历史英文同值项；这些集中在审批、Workspace 环境、MCP、本地模型等非 Memory 面板旧区域，不属于本次截图暴露的记忆页修复闭环。
403. 已完成：本次多语言适配从 Memory 面板扩大到全前端源码实际引用面。基于 `scripts/audit-i18n-quality.mjs` 对 `src` 下所有非测试 TS/TSX 的字面量 `t("...")` 调用做内容审计，批量补齐 ar / es / ja / ko / ms / pt / ru / tr / vi / zh / zh-TW 中仍等于英文基准的源码引用文案；品牌名、协议名、文件名、单位、URL、占位符格式和技术缩写通过精确白名单保留英文。翻译过程对 `{{...}}` 插值变量做保护，少量机器翻译破坏变量的条目已回退并手工修正。最终证据：`pnpm i18n:audit -- --locales ar,es,ja,ko,ms,pt,ru,tr,vi,zh,zh-TW --source src --fail-on-same` 全部为 0；源码引用 key 的占位符一致性问题 0；`pnpm i18n:check` 通过 12 语种 6488 keys / 缺失 0；`pnpm typecheck` 与 `git diff --check` 通过。该步骤仍不改变运行时逻辑，只把本次多语言适配的验收从“记忆页无英文”推进到“前端源码实际引用文案无非白名单英文兜底、变量不丢失、结构不缺 key”。
