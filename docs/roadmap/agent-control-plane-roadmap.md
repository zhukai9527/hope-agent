...
| Coding Eval | coding-first 质量闸，harness 可复用于通用控制面 | 已实现 Phase 6.3 | 控制面协同是否可回归，关键上下文是否被召回，focused action 是否真实收窄，Agent 是否能从 prompt 生成候选结果，候选 diff 是否满足任务级成功标准；20 个 active gold tasks 是否可批量回放；mock tool-call 是否真实调用写文件工具；策略改动前后是否真的改善质量；持久化历史是否满足发布质量门禁；真实 provider 是否能在受控 Gold Pack 中从 prompt 产出可评分候选 diff；promoted learning 是否有跨项目泛化证据；Dashboard 是否能以 Benchmark Run Center / Campaign Runner / Leaderboard 形式展示、运行、取消、重试、对标和审计当前 benchmark readiness。 |
...
- 当前 runner 是 owner-plane 后台 task + durable item 状态；跨模型 leaderboard 已在 P6.3 补齐，跨进程恢复、成本/延迟聚合和更强 gate policy 留给 P6.4-P6.6 继续补齐。
...
- 设计 task pack manifest 和 versioning：任务来源、repo template、难度、语言/框架、成功标准、验证命令、允许/禁止改动、人工校准记录。
- 支持本地真实 repo / 外部 fixture repo 显式导入；记录 license / privacy note / redaction 状态。
- 扩展任务类型：bugfix、feature、refactor、test repair、i18n、docs/design、multi-file、tool-call-heavy、review-seeded、verification-heavy、context-heavy。
- Dashboard / owner API 增加 corpus health：active/draft、覆盖、过期、重复、失败分布、容易被 fixture-gaming 的任务。

### Phase 6.5 Benchmark Report Export（待做）

- 从 campaign / comparison / release gate 生成可复盘报告：执行摘要、scope、模型矩阵、关键指标、失败分类、代表 case、成本/耗时、风险和结论。
- 支持 Markdown / JSON / HTML snapshot；报告数字引用稳定 run id，不依赖 live DB 变化。
- 报告链接回 campaign、pack run、case report、diff summary、review findings、verification evidence、workflow / goal evidence。
- Dashboard 增加 report history，可查看、重新生成、复制路径、标记为 release evidence。
...
- 建立 domain workflow manifest、registry、版本和启用范围。
- 支持 Research、Writing、Data Analysis、Meeting Prep、Knowledge Curation、Inbox、Project Ops 等首批任务类型。
- Workflow 创建入口可从 domain template 生成 `workflow.js` draft，继续走 Script Gate、permission preview 和用户审批。

### Phase 7.2 General Evidence Model（待做）

- 扩展 Goal evidence 到通用证据：source citation、claim check、user decision、artifact、data quality、citation audit、message approval。
- Evidence 记录来源 metadata、access scope、retrieved_at、confidence 和 redaction 状态。
- 非 coding workflow 不再用 validation/diff/file 伪装所有证据。
