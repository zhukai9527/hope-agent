...
> 更新时间：2026-07-01
>
> 状态：路线调整与方案设计。`/goal` 第一版已落地并沉淀到 [Goal 控制平面](../architecture/goal.md)；`/loop` 第一版已落地并沉淀到 [Loop 控制平面](../architecture/loop.md)；本文继续记录 `/worktree` 等后续推进顺序。
...
- 后续 worktree、LSP/review 仍先在 roadmap 迭代，稳定后再沉淀到 `docs/architecture/`；Loop 第一版已沉淀到 [Loop 控制平面](../architecture/loop.md)。
