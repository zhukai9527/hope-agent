...
4. **embedded runtime MVP**：脚本 spawn 2 个只读 subagent 并汇总 / 跑一次 targeted validate / 重启后不重复已完成 op。
5. **eval 回归闸**：接 Phase 0 coding-eval baseline，每个里程碑跑一遍要求不回归（见上层方案验收）。
