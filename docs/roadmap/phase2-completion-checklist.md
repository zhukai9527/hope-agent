...
> 更新时间：2026-06-30
>
> 状态：实现收口清单。自动化证据坚持无 LLM；真实模型回复型 fan-out 作为人工 smoke/eval gate 记录。
...
| debug with failing test | `workflow.validate` 结构化结果 + repair events | 构造一个失败测试，要求 agent 最小修复、单点验证、失败时写 repair feedback |
| feature implementation | `/loop` prompt 注入 + workflow host API | 让 agent 生成 feature script，必须有 task、diff、validate、finish，并能通过审批后执行 |
...
| user approval / cancel / resume | permission preview / owner action API / Workspace actions | Draft script 触发 approval，用户 approve 后运行；运行中 pause/resume/cancel 能反映到 run state |
