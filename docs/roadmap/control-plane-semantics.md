...
| Coding Eval | 已实现 Phase 3.7 | 测试/CI 质量闸 | `ha-core::coding_eval`，`cargo test -p ha-core --test coding_eval --locked` | 确定性 fixture harness，回归 Review / Smart Verification / Context Retrieval / Goal / Task / Workflow 协同；完整实现见 [Coding Eval 控制面评测](../architecture/coding-eval.md)。 |
