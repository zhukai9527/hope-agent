...
- 工作台新增「真实样本验收」卡片：从当前 session 的 domain evidence、Operational Gate、Soak Report、Artifact Export Guard 与 Connector Action Guard 派生覆盖领域数、控制面记录数、已排空样本、Connector E2E evidence 与事故/缺口；验收缺口按 `阻塞` / `待补` / `扩展` 标记并排序，让 critical soak incident、failed E2E gate 等先于普通补样本展示；每个验收缺口都可显式「转任务」，也可一键创建包含当前指标、带严重度标签的验收缺口和采样动作的「采样清单」任务，避免“真实样本还没跑够”只停留在文档提醒。
