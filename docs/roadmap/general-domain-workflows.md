...
- 通用任务工作台后续仍可继续接入更多 owner action，例如产物版本对比和更细的人工确认；当前已支持从交付守门对具体 artifact 发起领域复核，并在 evidence 已带 artifact 线索时按目标产物收窄复核范围；artifact-scoped 复核完成后可一键写回 `artifact_reviewed` evidence，但不伪造 `exportReview` / `exportReady` / `redactionChecked`；来源一键加入 evidence、从证据缺口/守门 check/需复核 evidence/长跑事故创建 task 也已落地，不再只依赖 Workspace「领域复核」和 Dashboard Learning。
