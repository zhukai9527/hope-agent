...
状态：详细方案见 [Phase 2 Coding Mode 与 Script-first Dynamic Workflow 方案](phase2-coding-mode-dynamic-workflow.md)。2026-06-30 已完成 durable store/state machine 与 QuickJS runtime foundation；同步首批 host API（`task.create/update`、`fileSearch`、`trace`、`finish`）已可通过 Script Gate 后执行并 durable replay。`spawnAgent` / `waitAll` / `validate` / 工具桥 / 控制面仍待接入。
