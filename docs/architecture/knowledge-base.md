...
**上下文融合（`context::build_instruction`，各 `senses.*` 开关）**：内置英文 persona（**两档** `PERSONA_PROACTIVE` / `PERSONA_RESTRAINED`，由 `SpriteConfig.proactive` 选，默认主动档；**不外露自由文本配置**）+ 当前文档(截断) + 最近编辑 + 对话上下文(前端送最近 6 条) + 记忆召回(`active_memory::shortlist_candidates`，scope = Agent + 可选 Global) + 跨会话感知(`awareness::collect::collect_entries`)，各段 `truncate_utf8` 预算裁剪。整条指令为英文、要求模型用文档语言作答。经 [`crate::automation::run`](automation-model.md)（purpose `sprite.observe`，`SpriteConfig.model_override` 未设落全局默认链，真跨模型降级；`timeout_secs` 现语义为全部候选尝试总预算）→ 解析 `{category,text}`，`none`/空 = 沉默不发。`category ∈ writing|review|encourage|remind|connect`。

**incognito 零精灵（两道关卡）**：后端 `observe_and_maybe_speak` 首行 `is_session_incognito` 短路（零召回 / 零调用 / 零 emit）；前端 incognito 同样不触发（知识会话本就非 incognito，防御性）。

**呈现**：`SpriteBubble` 渲染于 `MessageList` 与 composer 之间（**整体紫色 + 模糊光晕呼吸的扩散效果** + Agent 头像 + 分类角标），**瞬态、不进对话历史**；「回应」把建议塞进输入框顺势聊；新建议替换旧、发消息清空。前端按 `notePath`(+`sessionId`) 过滤 `sprite:suggestion`。**施法光效**：后端在整个 `automation::run` 降级循环前后（不是单次尝试前后）emit `sprite:casting {active}`（仅在通过节流闸门、真正发起 LLM 调用时），前端同样按 `notePath`(+`sessionId`) 过滤后驱动标题栏猫咪图标的「施法」态光环（比启用态更快更亮的品红涟漪 + 脉冲），LLM 返回即熄；前端兜底 30s 自动清除以防「结束」事件丢失。

**配置**：`SpriteConfig`(`config.rs`，`clamped()` 钳值，**无 persona 字段**，`model_override` 未设落 `function_models.automation`) 挂 `AppConfig.sprite`；设置三件套 = `SpriteSection` GUI（设置 → 知识空间，**只放调参不放 enabled**；保存时 merge 最新 `enabled` 防覆盖对话栏开关）+ `tools/settings.rs` 的 `sprite` 分支（MEDIUM）+ `ha-settings/SKILL.md`。owner 命令 `sprite_config_{get,set}_cmd`（GUI + 对话栏开关）+ `kb_sprite_observe_cmd`（触发），**均非 agent 工具**。
...
纯图片扫描版 PDF（无文本层）今天之前会直接导入失败——`file_extract.rs::extract_pdf` 早就无条件把 PDF 页面光栅化成 PNG，只是 `extract_uploaded_document` 只读文本、丢弃图片。这一节把图片接上 `automation::run_vision`，并按**逐页**而非整份文档粒度追踪成败，使一份 40 页扫描书里 7/8 页成功、1 页失败不必整份重来。

**触发**：`import_file_snapshot` 的 `Pdf` 分支先走纯文本抽取（`file_extract::extract_pdf_text`，不加载 pdfium）；有文本 → 行为与今天字节对字节一致，同步返回。文本为空 → 转入本节的 OCR 兜底路径；`Docx` 没有这条兜底，文本为空仍直接 `bail!`。

**数据模型（`knowledge_source_ocr_pages` 表，`sessions.db`，`ON DELETE CASCADE`）**：一行一页，`(source_id, page_number)` 唯一。字段：`status`（`pending|running|succeeded|failed`）、`stage`（`render|vision|timeout`，仅 `failed` 非空——区分"这一页连图都没渲染出来"和"渲染出来了但 vision 调用失败/超时"两种失败）、`error`、`model_label`（实际胜出的候选，`automation::model_label`）、`attempt_count`。**不存 OCR 文本本身**——文本唯一真相源仍是 `.md` 快照，这张表只管状态/错误/指针，与 `KnowledgeSourceImportItem` 不存正文的原则一致。粒度上这是比 `KnowledgeSourceImportItem`（文件级）更细一级的新表，不是它的复用——把 N 页塞进一个"一个 item 产出至多一个 source"的表会破坏其去重/计数不变式。

`KnowledgeSourceStatus` 新增 `PartiallyExtracted`（`partially_extracted`）第三态，与既有 `Ready`/`Failed` 并列；全仓库未对该枚举做穷举 match，新增变体零改动风险。

**原始 PDF 字节保留（专为重试，不受 `knowledge_media_retention.enabled` 门控）**：写入 `assets/{source_id}/ocr-original.pdf`（`source_asset_path` 同一目录约定），不经过图片 / 音视频那套可选留存 + 配额系统——那个开关管的是"要不要留一份原件方便回看"的隐私/存储偏好，这里的字节是重试功能能否工作的**必要条件**，默认关会让大多数用户的重试形同虚设。全部页面成功后尽力删除，回收磁盘。

**永远异步**：`run_vision` 的 `timeout_secs` 是**每页**的预算，几页也可能要几分钟，同步单文件导入路径又完全没有进度 UI；转入 OCR 兜底后立即持久化一个 `PartiallyExtracted` 占位 `KnowledgeSource`（正文"OCR 进行中"）并**立刻返回**给调用方，真正的逐页 OCR 由 `tokio::spawn` 的后台 round 完成，经 `JobManager::spawn_knowledge_ocr`/`finish_knowledge_ocr`（镜像 `spawn_knowledge_import`/`finish_knowledge_import`，`tool_name="knowledge_source_ocr"`）接入统一后台任务面板。占位创建走 `persist_source_draft(kb_id, draft, dedupe=false, None)`——不做 `extracted_text_hash` 去重（占位内容本来就不该按去重语义处理）。

**逐页并发与隔离**：`file_extract::render_pdf_bytes_isolated`（本次新增，与既有 `render_pdf_pages`/`render_pdf_bytes` 的"一页失败整批失败"不同，单页 `Result` 隔离）渲染出的页面图片，经 `knowledge/source.rs::ocr_pages_with`（有界并发，`futures::stream::buffer_unordered`，复用 `recap/facets.rs` 已有的同一并发模式）逐页独立调用 `run_vision`——**一次调用打包全部页面附件**与**逐页独立重试**根本矛盾（一次调用要么整体成功要么整体失败），这里选择支持独立重试，所以必须逐页单独调用。`KnowledgeVisionConfig` 新增 `ocr_concurrency`（默认 3，硬顶 8）、`max_ocr_pages`（默认 40，硬顶 120——比聊天内联场景的 8/10 高不少是刻意的：知识空间导入是一次性主动归档动作，不占对话轮次延迟预算）。`ocr_pages_with` 的 vision 调用是**注入的闭包参数**，是这条链路上唯一的确定性单测入口（`knowledge::source::tests::ocr_pages_with_isolates_per_page_failure`，零网络/零 provider）。

**Markdown 约定**：header 新增 `OCR-Pages: {succeeded}/{total}`、`OCR-Status: processing|complete|partial|failed`；正文按 `## Page N` 分节，每节复用单图 OCR 提示词的 `### OCR Text`/`### Structured Notes`/`### Tables`/`### Uncertain Reads` 四段（标题降一级）+ 一行 `_OCR: {model_label}_` 溯源；失败页写 `_OCR failed ({stage}, attempt {n}): {error}_` 占位，末尾汇总一段 `## Failed Pages`。`Source-Type` 仍是 `pdf`（kind 在抽取前就按文件名/MIME 定死，不因内容变化）。

**重试与去重语义（红线）**：重试/(再)finalize 走 `update_source_snapshot_in_place`（`knowledge/source.rs`，镜像既有 `reextract_source` 的原地 `replace_source_chunks`），**不走 `persist_source_draft` 的去重/版本链**——重试成功页数变化会让内容 hash 跟着变，硬套去重要么误判成第二份新资料，硬塞进版本链又会让版本历史里出现"7/8 页"这种没人想 diff 的中间态。每轮 finalize（`finalize_pdf_ocr_source`）从当前快照正文里解析出之前已成功页的文本（`parse_pdf_ocr_page_sections`，按 `_OCR failed` 前缀排除失败占位，绝不把失败占位当成功文本带入下一轮）与本轮新结果合并重建整份文档——这是"文本只活在 `.md` 快照里、账本不存正文"这条设计约束换来的必要复杂度。若第一轮全部页面失败，占位 source 仍保留（`status=Failed`）、逐页可重试，不像既有单图 `ocr_image_bytes` 那样直接整体 bail 掉。

**启动恢复**：`interrupt_running_source_ocr_pages`（镜像 `interrupt_running_source_import_runs`，同一 `registry.rs` 初始化点接入）把异常退出遗留的 `pending`/`running` 行标 `failed`，避免永久卡在"进行中"。

**owner 面**：`kb_source_ocr_pages_cmd`（列出某 source 的逐页状态）/ `kb_source_ocr_retry_cmd`（重试当前失败页，后台执行、立即返回），Tauri + HTTP `GET/POST /api/knowledge/{kb}/sources/{source}/ocr-pages|ocr-retry` 双适配。GUI：[`KnowledgeSourcesPanel`](../../src/components/knowledge/KnowledgeSourcesPanel.tsx) 列表按 `source.status` 显示"部分失败"/"OCR 失败"提示，右键菜单 PDF 源在非 `ready` 状态下露出「重试失败 OCR 页面」。
...
- **AI 改写（owner 平面）**：选区改写已收敛到对话面板的快捷改写浮动条（`chat/QuickRewriteBar.tsx`，详见前文「选区针对性编辑」），走 `kb_ai_rewrite_cmd`（`service::ai_rewrite` 经 [`crate::automation::run`](automation-model.md)，purpose `knowledge.ai_rewrite`；用户在浮动条显式选了模型就单模型钉死不降级，没选/解析失败才落全局默认链真降级，**不落盘**）→ `UnifiedDiffView` diff → 「应用」`replaceRange` splice 回编辑器。旧标题栏 `AiRewriteDialog` 已删。
