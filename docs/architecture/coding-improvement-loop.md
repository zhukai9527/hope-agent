...
> 返回 [技术文档索引](../README.md)
>
> 状态：Phase 6.4 已实现。本文是 `ha-core::coding_improvement`、`dashboard::coding_improvement`、Coding Trend Report、Transcript Distillation、Failure Feedback、Workflow Retro、Improvement Proposal 队列、Proposal-to-Action、Draft Promotion、Gold Pack / Strategy Effect history、External Model Baseline、Release Gate、Learning Generalization Gate、Benchmark Run Center、Benchmark Campaign Runner、Cross-model Leaderboard、Benchmark Task Corpus、owner API、Workspace 质量趋势区块与 Dashboard 全局学习视图的单一技术事实源。
...
初始化入口在 `SessionDB::open()`，由 `crate::coding_improvement::ensure_tables()` 创建七张表。
...
Dashboard Benchmark Center 在 Campaign 控制区上方展示 Model leaderboard：rank、label、baseline/execution/task pack、case pass rate、item pass 和 warning 标记。P6.3 先做可信表格与 owner API；P6.4 已补齐真实任务集 registry 与 corpus health，趋势图、成本/延迟和报告导出会在 P6.5-P6.6 继续增强。
