...
- **`config:changed`** —— 所有 `apply_*` 与 state 写入经 `save_config` 落盘后 emit（前端据此刷新缓存配置快照）

## 安全 / 红线

- **写路径走 `load_config()` + `save_config()`，不是 `mutate_config`**：`apply.rs` / `state.rs` 实测走前者。这是**历史既存路径**——`save_config` 仍会 emit `config:changed` + 落 autosave 备份，但**不具 `mutate_config` 的并发 lost-update 防护**（详见 [配置系统](config-system.md)）。改这里时别照搬误以为已走 config contract。
