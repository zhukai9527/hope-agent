...
- **Chrome profile / extension mode 识别（后续落点）**：`browser.status` 应把扩展连接拆成 `store` / `unpacked-dev` / `enterprise-policy` / `unknown` 四类，并输出当前 profile 线索（profile path 可脱敏为 basename + hash）。Settings 安装向导据此区分「未安装」「装在其他 profile」「unpacked 版本过旧」「Web Store 版本过旧」。识别只消费现有 extension hello、manifest id、native-host diagnostics 和 Chrome profile path；不新增浏览器扩权协议，不扫描用户磁盘全量 profile。
