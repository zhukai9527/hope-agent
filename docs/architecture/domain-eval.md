...
- Workspace「通用任务工作台」也复用 `evaluate_domain_operational_gate({ sessionId, windowDays: 14 })`、`generate_domain_soak_report({ sessionId, windowDays: 14, maxItems: 8 })` 与 `evaluate_domain_connector_e2e_gate({ sessionId })`，作为当前会话的运行稳定性、长跑审计和连接器端到端验收卡片；它只读并刷新状态，不自动 approve / retry / cancel / run loop，也不自动执行外部动作。
