...
| `/goal` | `[status\|pause\|resume\|evaluate\|clear]` 或 `<objective> --criteria <criteria>` | 创建、查看、暂停、恢复、审计或清除当前会话的 active Goal | `DisplayOnly` |
| `/workflow` | `[status\|trace\|approve\|pause\|resume\|cancel] [run_id]` | 查看和控制当前会话的 durable workflow runs；`run_id` 可用唯一短前缀 | `DisplayOnly` |
...
`/workflow` 是当前会话 workflow run 的轻量命令面，适合在聊天里快速查看长任务状态；完整创建、预览、审批详情和控制中心 UI 走 [Workflow 与 Execution Mode](workflow.md) 的 owner API / Workspace 面板。

| 用法 | 行为 |
|---|---|
| `/workflow` / `/workflow status` / `/workflow list` | 列出当前 session 最近 12 条 workflow run，标注 active 数量、短 id、state、kind、execution mode、更新时间、op 完成摘要 |
...
| 用法 | 行为 |
|---|---|
| `/goal <objective> --criteria <criteria>` | 创建 active Goal。completion criteria 可多行；同一 session 同时只允许一个 open Goal。 |
...
`/goal` 是通用长任务语义，不限定 coding 场景。无痕会话拒绝创建 durable Goal。
...
| `/goal` | Utility | `[status\|pause\|resume\|evaluate\|clear]` 或 `<objective>` | 是 | 创建/查看/控制当前会话 active Goal |
| `/workflow` | Utility | `[status\|trace\|approve\|pause\|resume\|cancel] [run_id]` | 是 | 查看/控制当前会话 workflow runs |
