...
```
dreaming_runs ──1:N──→ dreaming_decisions
              ──1:N──→ dreaming_locks
              ──1:1──→ memory_profile_snapshots (source_run_id)

memory_claims ──1:N──→ memory_evidence        (ON DELETE CASCADE)
              ──1:N──→ memory_claim_links ──N:1──→ memories (legacy 双轨)
```
