...
之所以只驱逐单个最旧 session 而非撞顶全清（`clear()`）：全清会一次抹掉所有 stickiness，长跑进程 prompt cache 命中率断崖式下跌。回归测试 `test_sticky_map_lru_eviction_preserves_recent` / `test_sticky_map_lru_promotes_on_get` 守住语义。
