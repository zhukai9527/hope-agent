...
1. `CronDB::list_run_timeline(batch, raw_offset)`——`cron_run_logs LEFT JOIN cron_jobs`，按 `started_at DESC, id DESC` 倒序取批次，job 被删时 `job_name` 回退 `(deleted job)`；
2. `SessionDB::cron_session_read_state(session_ids)`——按批次 session id 取 `(title, 未读会话标记 0/1, archived)`；过滤归档项后再按**可见行**做 `offset/limit`，防止归档造成短页或漏页。session 被 purge 的 id 缺席，由装配层回退 `title=job_name`、`unread_count=0`；SessionLoop 共享普通 parent session，只参与归档判定、不并入 Cron 未读。

返回 `CronTimelineRow { sessionId, jobId, jobName, status, startedAt, finishedAt, resultPreview, title, unreadCount }`。前端 `CronConversationsPanel`（`CronCalendarView` 第三模式「历史」）做 master-detail：左栏时间线列表（状态点 / 相对时间 / 未读角标 / 归档 / 翻页），右栏 `CronSessionViewer` 复用主聊天 `MessageList` + `parseSessionMessages` 只读渲染（无 ChatInput，对齐 ChatScreen 的 `isCronSession` 只读分支）。job 详情的 `cron_get_run_logs` 同样经 `visible_cron_run_logs` 按可见行分页并提供逐条归档。视图模式经 `localStorage`（`cron_view_mode`）持久化。
...
**单对话永久删除**：归档管理页可永久删除某次 Cron / SessionLoop 对话，但 `sessions.db` 的 Session CASCADE 无法触及独立 `cron.db`。Tauri / HTTP `delete_session_cmd` 因此统一经 `cron::delete_conversation_and_run_logs`：先 `CronDB::delete_run_logs_for_session(session_id)`，再 `SessionDB::delete_session(session_id)`。顺序不可反转——时间线刻意保留缺失 Session 的历史审计行，若先删 Session 而 run log 清理失败，用户刚永久删除的对话会立即以不可打开的空壳重新出现。
