...
Coding Improvement owner API 基于 durable Goal / Workflow / Review / Smart Verification / Coding Eval 数据生成 trend report、failure taxonomy 和 draft-only proposal 队列。`generate_coding_improvement_proposals` 只写 `coding_improvement_proposals(status='draft')`，不自动修改 project guidance、skills、memory 或 eval fixture；采纳/拒绝也只更新 proposal 状态。无痕会话 fail-closed。完整契约见 [Coding Improvement Loop](coding-improvement-loop.md)。
...
```bash
# 1. Tauri 命令总数（截至 2026-07-02：691）
...
# 2. HTTP 路由总数（截至 2026-07-02：650）
grep -cE '^[[:space:]]+\.route\(' crates/ha-server/src/lib.rs

# 3. COMMAND_MAP 条目数（截至 2026-07-02：678，不含闭合 `}` 的行）
