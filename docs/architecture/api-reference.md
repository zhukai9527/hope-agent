...
| 源 | 位置 | 数量 |
|---|---|---|
| Tauri 命令 | `src-tauri/src/lib.rs` 的 `tauri::generate_handler!` | **947** |
| HTTP 路由 | `crates/ha-server/src/lib.rs` 的 `.route(...)` | **881** |
| 前端 COMMAND_MAP | `src/lib/transport-http.ts::COMMAND_MAP` | **928** |
...
| 分类 | 数量 | 说明 |
|---|---|---|
| ✅ 两端完全对齐（在 COMMAND_MAP 中） | 928 | 常规请求/响应命令（所有 COMMAND_MAP 条目都有 Tauri 命令对应） |
| 🔧 特殊处理（不在 COMMAND_MAP 但 HTTP 已实现，走专用 Transport 方法） | 11 | multipart/二进制流/保存对话框类接口：avatar、filesystem、session export、Artifact export 和 memory backup archive |
...
Tauri ↔ COMMAND_MAP 差集为 19 条合法非通用映射命令：5 条 Desktop-only 系统权限命令、11 条走专用 Transport 方法的 multipart/二进制/保存路径接口，以及 `project_fs_resolve` / `kb_file_resolve_cmd` / `set_dock_badge_cmd` 3 条 Tauri-only 命令。HTTP 路由侧的非 REST endpoint（健康检查、静态文件、流式 chat、multipart 和二进制下载等）不要求映射为通用 `call()`；对齐状态在各功能域章节登记。新增 Tauri 命令时须同步补 HTTP 路由、COMMAND_MAP 或专用 Transport 说明。
...
截至 2026-07-15 三端差集为 19 条：§7.3 的 5 条 Desktop-only 系统权限命令、§7.3.1 的 11 条 HTTP 已实现但走专用 Transport 方法，以及 `project_fs_resolve` / `kb_file_resolve_cmd` / `set_dock_badge_cmd` 3 条 Tauri-only 命令。没有“HTTP 漏写 COMMAND_MAP”或“HTTP 路由缺失”的破口；COMMAND_MAP 每一条都能在 `tauri::generate_handler!` 找到对应命令。
...
### §7.3.1 不进 COMMAND_MAP 但 HTTP 已实现的合法非 REST 命令（11 条）
...
```bash
# 1. Tauri 命令总数（截至 2026-07-15：917）
...
# 2. HTTP 路由总数（截至 2026-07-15：881）
grep -cE '^[[:space:]]+\.route\(' crates/ha-server/src/lib.rs

# 3. COMMAND_MAP 条目数（截至 2026-07-15：928，不含闭合 `}` 的行）
