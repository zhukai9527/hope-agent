...
## 用户纠错闭环（Lucid Review，下一代 Dreaming Phase 6）

把整理结果的控制权交给用户（设计 §5.2 / §5.3）。唯一编排入口 `claims::review`：

- **`update_claim(ClaimUpdate)`**：PATCH 语义，每个字段可选。`resolve_update`（纯函数，无 DB，穷举单测）从「当前行 vs 请求」的 diff 派生主 decision-type，优先级 **status > scope > pin > edit**：
  - status：`active`→`approve`、`archived`→`reject`、`expired`→`expire`（mark outdated）、`needs_review`→`flag`
  - scope：move scope（global 清空 scope_id）
  - pin/unpin：经 salience 越过 / 退到 `PINNED_MIN_SALIENCE`（pin=0.95 / unpin=0.5）
  - edit：content / subject / predicate / object / tags
  - approve / edit 视为用户确认：置 `confidence_source=user_confirmed` 并把 confidence 提到 0.95
- **`forget_claim(claim_id, permanent, note)`**：`permanent=false`（默认）翻 `archived` 并停止 linked legacy memory 注入（本 claim 的 link 转 `managed` + 仅独管的 memory `pinned=0`，让 read-time hidden-set 覆盖），**保留 evidence 作审计**；`permanent=true` 硬删 claim 图谱（claim + evidence + link + vec0）+ 仅其独管的 orphan memory，**连同 evidence 一起删**。
...
- **审计**：每个用户操作经 `dreaming::record_user_action` 落一条 `trigger=user_correction`、`phase=user`、`status=completed` 的 run + 单 decision（before/after JSON 快照），FK 进 `dreaming_decisions`，在 Dashboard 运行历史里与流水线 run 并列（§5.3「所有用户操作有 decision log」）。审计是 best-effort：纠错本身已成功，审计写失败只 `app_warn!` 不回滚。
- **证据**：approve / edit / reject / expire / move_scope 写一条最高权重（1.0）`manual_correction` evidence（approve 用 `user_confirmed`、`redaction_status=raw_allowed`——用户自述无需脱敏）；pin / unpin / flag 不写。
- **重嵌**：content 变更后 `reembed_claim`（须先 drop write_conn——`embed_and_index_claim` 内部再取写锁，writer Mutex 不可重入），使下一轮 Active Memory v2 / Context Pack 召回反映新文本。
- **FTS/vec 维护**：edit content 经 `memory_claims_au` 触发器自动同步 FTS；vec0 无触发器，永久删除手删 rowid（表懒创建，缺失容错）；linked memory 的 `memories_vec` 同理。
- **事件**：`memory:claim_changed`（每次）+ `memory:review_required`（flag 时）经 EventBus 发出，Dashboard Needs Review 队列 + 运行历史据此实时刷新。

鉴权平面：纯 owner 平面（`claim_update` / `claim_forget`，API key / 本机信任），**无 agent 工具面**——claim 纠错只对用户开放，模型不能自改自己的记忆。GUI 走可复用 `ClaimReviewActions`（Dashboard Dreaming Center 的 Needs Review 队列 + Settings 记忆面板 ClaimsBetaView 详情共用）。
