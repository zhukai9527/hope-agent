...
产品级配置统一由 `AppConfig.memory: MemoryRuntimeConfig` 承载，使用、Core、快速召回、深度召回和学习是正交开关，不得再从某个旧 Active Memory 字段推断整个 Memory 的启停状态：
...
- `recall.enabled=false` 只停止动态召回；Core 仍可使用。
- `deepRecall.enabled=false` 只保证不发生额外 LLM rerank；确定性 Fast Recall 不受影响。
...
dynamic suffix: permission / Fast-or-Deep Recall / Procedure / Knowledge / reminders
...
V2 的 `MemoryRecallPlanner` 是动态召回的唯一产品编排入口。每个 user turn 先经过零 LLM 的意图 gate；空输入、寒暄、简单确认/感谢、Incognito、memory off 和 recall off 直接返回带原因的 skip。需要召回时，并行读取当前 scope 内的 legacy memory、effective-active claims、按 profile intent 展开的 Profile、Procedure 和 Graph 辅助候选，再执行：
...
Fast Recall 默认开启且不调用额外 LLM。Deep Recall 默认关闭；开启后只对已经完成实时资格过滤的 Fast shortlist 做一次 bounded side query，用于 rerank / distill，默认 timeout 4.5s、TTL 60s、摘要 220 字符、预算 512 token。无效响应、超时或模型失败回退到原 Fast Top-K；有效的空选择表示模型确认无需注入，绝不能回退为全量静态记忆。
...
默认验收边界为：Core 目标 ≤1,600 token、硬上限 ≤2,400；动态召回 ≤5 条 / ≤800 token；`hi` 为 0 条；Fast Recall P95 目标 ≤100ms；普通回合至少 90% 不产生额外记忆 LLM；同一 session 普通回合 stable fingerprint 不变；热缓存 stable read ratio 目标 ≥80%；scope leakage、Review 未批准内容可见和因记忆导致的主回答 hard failure 均为 0。
...
旧 `ActiveMemoryConfig.enabled` 是 per-agent 的 LLM 主动召回入口。V2 保留其反序列化、Agent override 和 rollback 执行链，但普通用户看到的对应概念是 Deep Recall：Fast Recall 默认开启且不调用额外 LLM；Deep Recall 默认关闭，开启会增加 token 和发送延迟，失败时只回退 Fast Top-K。关闭 Deep Recall 不会恢复 SQLite / Profile / Pinned 的默认静态注入。
...
Memory Center 的“相关时想起”主开关控制 Fast Recall，“深度召回”作为次级开关并明确显示延迟/token 成本。读取或保存配置失败必须显示动作级本地化错误、脱敏 detail 和 retry，不得把失败状态渲染成关闭或默认值。旧“回答时使用记忆”入口仍可映射兼容字段，但不能同时改 Memory master switch、其它 Agent、Incognito/IM opt-in 或 Prompt 预算。
...
flowchart LR
    ...
    Turn["current user turn"] --> Gate["deterministic intent gate"]
