# 记忆系统架构
> 返回 [文档索引](../README.md) | 更新时间：2026-07-10
...
用户或 Agent 通过 `save_memory` 工具显式保存记忆，支持 `scope` 参数指定作用域（`"global"` / `"agent"` / `"project"`）。省略 `scope` 时，项目会话默认写入 Project scope，非项目会话保持 Global scope 以兼容既有行为；`scope="project"` 时从当前会话的 `session.project_id` 自动解析项目 ID，无项目上下文则返回错误。自动执行去重检查：
...
Settings 的“记忆学习模式”是面向普通用户的 UI 预设层：

- `自动学习` 同时开启 `autoExtract` + `flushBeforeCompact`；全局模式还确保 `extractClaims=true` 且 `reviewFirst=false`，新结构化 claim 默认 `active`。
- `先审核` 是全局结构化记忆策略，写入 `memoryExtract.reviewFirst=true`，并同时开启 `autoExtract` + `flushBeforeCompact` + `extractClaims`；自动抽取的新 claim 初始状态为 `needs_review`，其 `managed` legacy shadow 会被 prompt 读路径隐藏，直到用户在 Review Inbox approve / edit。显式 `save_memory`、用户按钮、backfill、restore 和纠错流程保持各自原策略。
- `仅手动` 同时关闭 `autoExtract` + `flushBeforeCompact`，并清掉全局 `reviewFirst`；显式 `save_memory` / 用户按钮仍可写入。
- `关闭` 写入 `memoryExtract.enabled=false`，作为全局 agent-plane 长期记忆 runtime gate：系统提示不注入 Memory / Profile / Context Pack，Active Memory 动态召回和静态 used-memory trace 清空，Memory tier 工具（`save_memory` / `recall_memory` / `memory_get` / `update_core_memory`）不进 schema 且写工具 handler 级拒绝，自动提取 / idle 提取 / 压缩前 flush 全部跳过。关闭动作走二次确认，Memory Overview 置顶显示“长期记忆已暂停”横幅，并提供恢复为“仅手动”与打开设置入口。Owner 平面（Memory Center 的查看、导出、删除、导入、备份恢复、手动管理）仍可用；手动添加、AI 导入和备份恢复入口会显示 inline 提示，说明这些动作只管理本机数据，不会在 off 状态下重新启用 Agent 读取 / 召回长期记忆，避免用户关闭后无法清理已有数据或误以为导入即生效。
...
Memory Center 首页的“回答时使用记忆”推荐入口是普通用户开启 Active Memory 的主路径：读取默认 Agent 配置失败必须显示“无法检查主动召回”，应用推荐预设失败必须显示“无法更新主动召回”；两者都保留本地化标注后的错误 detail，但 detail 必须先做 token / Authorization / api_key / password / passphrase 脱敏、单行规整和 bounded 截断，空错误不展示 detail。该入口只改默认 Agent 的 memory 配置和 Active Memory 推荐档，不改变全局 memory 开关、其它 Agent 配置、incognito / IM opt-in、prompt 注入预算或召回排序。
...
- `context_pack`：Pinned claim / Context Pack 静态前缀。
- `static_memory`：legacy `# Memory` SQLite 段。
- `profile`：Profile Snapshot 段。
- `active_memory`：每轮 Active Memory side-query。
...
- External memory provider 是**可选 additive 同步层**，本地 SQLite / claim / evidence 永远是真相源；全局默认关闭、单 provider 默认关闭且 policy=`off`，外部服务失败不得阻断本地 prompt、召回、写入、Dreaming 或用户纠错。
  ...
  - 手动同步、3s 本地写 debounce 和 5min 周期 pull/reconcile 共用进程级 async mutex，避免同一 ledger 并发覆盖；debounce 使用 dirty+queued 双标志，执行期间发生的新写入必须再跑一轮，不能丢边沿。仅 Primary 启动自动任务；`manual` policy 永不被后台调度，只有 `pull_only / push_only / bidirectional` 参与自动同步。切换 endpoint/subject/protocol 会清 ledger，单纯轮换 API key 保留断点。
