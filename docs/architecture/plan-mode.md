...
**Dashboard Plan stats**（[`crates/ha-core/src/dashboard/plan_stats.rs`](../../crates/ha-core/src/dashboard/plan_stats.rs)）：复用 `list_all_plans` 输出 + `DashboardFilter` 时间窗，纯内存聚合状态分布 / 完成率 / Top Agent / Top Project / 30 天 trend / 平均执行时长（剔除 > 7 天 outlier）。
