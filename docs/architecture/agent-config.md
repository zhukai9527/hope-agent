...
| [`agent_loader.rs`](../../crates/ha-core/src/agent_loader.rs) | 磁盘装配与 CRUD：`load_agent` / `list_agents` / `ensure_default_agent` / `save_agent_*` / `delete_agent` / `get_template` / `render_persona_to_soul_md`；硬常量 `DEFAULT_AGENT_ID` |
...
1. 读 `agents/{id}/agent.json`，**缺失或解析失败回落 `AgentConfig::default()`**（load 不应因缺字段失败）
...
`AgentSummary` 是前端列表用轻量摘要（含 `has_*` 标志与 `memory_count`）。`list_agents` 扫 `agents/` 目录产 summary 列表，**按 `config.agent_order` 排序、主 Agent 置顶**，对**解析失败的目录 skip 而非报错**。`list_agent_ids` 是更轻量的变体，只返回目录名集合（跳过 JSON 解析 / 记忆计数），供 ID 冲突检测用。
...
- **`delete_agent` 拒删主 Agent**：`id == DEFAULT_AGENT_ID` 直接 `bail`
- **markdown 白名单**：`save` / `get_agent_markdown` 仅允许 `agent.md` / `persona.md` / `tools.md` / `agents.md` / `identity.md` / `soul.md`，防路径穿越
- **`AgentConfig` 全字段 `serde default`**：缺字段 / 缺文件回落 `default`，`load` / `list` 不因缺字段失败（`list_agents` 对解析失败的目录 skip）
