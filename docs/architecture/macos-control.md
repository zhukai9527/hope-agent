...
| `snapshotId` | string | `visual.point` 要解析的 snapshot id，来自 `visual.observe` 或 `snapshot includeScreenshot=true` |
...
| `MacControlVisualResult` | `op`、`snapshotId?`、`snapshot?`、`screenshot?`、`coordinateSpace?`、`imagePoint?`、`screenPoint?`、`insideFrame?`、`hitElements[]`、`nearestElements[]`、`suggestedAction?`、`warnings[]` |
...
| 分类 | action/op | 决策 |
| --- | --- | --- |
| 只读 | `status`、`permissions`、`snapshot`、`elements.find`、`wait`、`visual.observe/point`、`apps.list/frontmost/installed/search`、`windows.list`、`act.dry_run`、`menu.list`、`dialog.inspect` | Allow |
...
- 视觉定位走 `visual.observe -> 读取图片选 image pixel -> visual.point -> act.click_point -> verify`；不要把截图像素坐标直接传给 `act.click_point`。
