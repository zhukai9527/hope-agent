...
> 返回 [技术文档索引](../README.md)
>
> 状态：Phase 5.3 已实现。本文只记录已经落地的自动化评测层；人工 gold task 体系仍见 [Coding Eval 体系方案](../roadmap/coding-eval.md)。
...
- 能列出内置 Gold Task Pack v1，并把 5 个 active gold tasks 自动 materialize 成 fixture，批量运行后返回 pack-level pass/fail、skipped/error、总 checks 和逐 case report。
...
| `crates/ha-core/tests/fixtures/coding_eval/*.json` | Phase 3.7-5.2 控制面、执行与任务级 JSON fixture；Phase 5.3 Gold Task Pack v1 由 `coding_eval.rs` typed registry 生成 fixture。 |
...
| `list_coding_eval_gold_tasks` | Owner-plane Tauri command；返回内置 Gold Task Pack v1 的 case summary。 |
...
`checks.execution` 可断言 mode、status、是否必须有 turn、必须/禁止改动文件、response / error 片段。`FixtureReport.metrics` 同步暴露 `execution_status`、`execution_mode`、`execution_changed_files`。

## Gold Task Pack Runner

Phase 5.3 新增 Gold Task Pack v1，把 Phase 0 文档里的 active gold tasks 变成可批量运行的结构化 registry。它不是把 20 个草案一次性全部自动化，而是先把 5 个 `active` 任务接进可回放链路：

| ID | 类型 | 主题 |
| --- | --- | --- |
| `CE-TEST-004` | `test_gap` | workflow repair loop 连续两轮无有效 diff 后停止并 ask_user。 |
| `CE-RUST-001` | `rust_logic` | ToolDefinition v2 只读 / 写入 / 破坏性 / open-world 元数据设计，且不绕过 permission engine。 |
| `CE-REV-002` | `review` | Review verifier 三态中 `PLAUSIBLE` / `REFUTED` 的证据阈值。 |
| `CE-NAV-001` | `repo_navigation` | 新增 coding workflow 时必须接入 JobManager、HookDispatcher、permission engine、Plan/Task 等现有模块。 |
| `CE-NAV-002` | `repo_navigation` | LSP 能力与 ACP/IDE 上下文的接合点，区分 prompt tail、passive diagnostics 和 bounded tools。 |

`list_coding_eval_gold_tasks` / `GET /api/coding-eval/gold-tasks` 返回 `GoldTaskPackSummary`：

- `packId` / `sourceDoc`
- `totalCases` / `activeCases` / `automatedCases`（v1 为 20 / 5 / 5）
...
默认执行模式是 `fixture_patch`，因此不会访问外部模型；它验证的是 task schema、候选 diff、Review / Verification / Context / Goal / Task scorer 的端到端胶水。真实模型成功率、稳定模型基线和策略 A/B 仍属于后续阶段。
...
| `execution` checks | execution mode/status、turn、response/error、执行后 changed files。 |
...
Phase 5.1 在两者之间补了一层：它把“某个候选结果是否满足任务级成功标准”变成可回归的 deterministic report。Phase 5.2 再补上真实 agent execution runner，让 owner API 可以从 task prompt 开始跑一轮 agent，再把产物交给同一个 scorer。Phase 5.3 开始把 active gold tasks 结构化成可批量回放的 pack。当前仍不等同于完整大规模 benchmark：20 个任务全量自动化、稳定模型基线、跨项目统计和策略 A/B 仍在后续阶段。
...
这层不会把 proposal 自动写进项目规则或 skill；它只验证 `coding_improvement` 聚合器是否能稳定消费 durable control-plane 数据。Phase 4.2 允许 fixture 显式声明 `promoteAppliedProposal`，用于验证 promotion 路径本身，但仍然是 owner-plane 的确定性动作，不会由 proposal generation 或 apply 隐式触发。Phase 5.1/5.2/5.3 的 task-level report 也写入同一 eval run 表，因此 Improvement Loop 可以把任务级失败、执行失败或 scope creep 按既有 failure taxonomy 继续生成候选。
...
- 扩展 gold task pack，从当前 5 个 active automated cases 继续覆盖 Phase 0 的 20 个任务。
- 增加稳定模型基线 / mock tool-call fixture，覆盖真实写文件工具调用而不依赖外部服务。
- 增加策略效果 A/B：比较 workflow policy、skill/guidance、tool contract 改动前后的通过率、验证质量和 scope creep。
