...
> 返回 [技术文档索引](../README.md)
>
> 状态：Phase 5.4 已实现。本文只记录已经落地的自动化评测层；人工 gold task 体系仍见 [Coding Eval 体系方案](../roadmap/coding-eval.md)。
...
默认执行模式是 `fixture_patch`，因此不会访问外部模型；它验证的是 task schema、候选 diff、Review / Verification / Context / Goal / Task scorer 的端到端胶水。真实模型成功率和稳定模型基线仍属于后续阶段。
