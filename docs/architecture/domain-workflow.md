...
- 「运行稳定性」卡片调用 `evaluate_domain_operational_gate({ sessionId, windowDays: 14 })`，显示 workflow 完成/活跃、loop 成功/失败、campaign 活跃、阻塞 check 和 recommended next steps；失败 / 样本不足 check 行与稳定性建议均可显式「转任务」，只创建可跟踪 task，不自动 retry、cancel、approve 或修改 gate 结果。
