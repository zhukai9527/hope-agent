...
| `create_session_task` | 从 Context Retrieval 候选行的“转任务”按钮或通用任务工作台“下一步”缺口创建 session task，并通过 `task_updated` 刷新进度面板。 |
...
「真实样本验收」卡片是 Workspace 内的只读派生视图，不新增后端表：它从当前 session 的 domain evidence、Artifact Export Guard、Connector Action Guard、Operational Gate 与 Soak Report 计算覆盖领域数、控制面记录数、已排空 Workflow / Loop / Campaign 样本、Connector E2E evidence 数，以及事故、缺证据、缺排空、样本陈旧、输出预算耗尽、缺连接器 E2E、单领域覆盖等缺口。它回答“当前会话是否已经有真实场景样本可供验收”，不启动采样、不调用连接器、不修改 gate 结果；卡片会显示验收快照 ID、证据等级、来源分布、保守验收进度（必需项通过数 / 必需项总数 + 百分比，failed gate 会拉低必需项进度，守门通过要求至少有已观察 gate 且全部 passed，跨领域覆盖作为额外加分），并把领域样本、证据链、排空样本、样本新鲜、预算健康、事故清零、守门通过、连接器 E2E（仅涉及外部动作时）逐项显示为通过 / 待补 / 阻塞，让用户能直观看到“还差多少”和“差在哪里”。卡片还派生只读「验收结论」和「证据等级」：验收结论为 `不可验收` / `待补样本` / `可局部复核` / `可验收`，由 danger gap、未通过必需项、warning/info gap 和控制面样本数保守计算；证据等级为 `未采样` / `阻塞样本` / `局部样本` / `局部验收` / `非外部动作候选` / `真实 E2E 候选`，明确这份材料只能用于采样待办、问题定位、局部复核、非外部动作验收，还是已经包含连接器执行/复核 evidence 可进入最终人工复核。来源分布按 evidence 的可见 `sourceMetadata` / evidence type / access scope 统计 workflow、connector、fixture/mock、manual、public 和 restricted 数量，只作为 reviewer 判断样本成分的提示，不替代权限、gate 或真实 evidence。验收结论、证据等级和来源分布进入 UI、采样清单和复制报告，帮助人工 / Claude Code review 直接判断这份样本能不能作为最终验收证据；它们不是新后端 gate，不写库、不改变运行状态。卡片还派生「验收矩阵」，把 Workflow、Loop、Campaign、连接器 E2E、跨领域覆盖拆成独立样本跑道，逐项显示当前 evidence 数字和下一步动作；未完成跑道可显式「转任务」，矩阵用于产品验收和 review packet，不改变 readiness 百分比，也不把未涉及外部动作的会话强行判 failed。“样本新鲜”使用 Soak Report 的 `latestActivityAgeSecs` 做 GUI 侧保守验收：最近活动超过 7 天或缺少新鲜度信号时显示待补并可转任务，但不改变 Soak Report 后端“陈旧样本不自动 failed，只提示补新样本”的 gate 语义。“预算健康”使用 Soak Report 的 `workflowBudgetExhaustedEvents` 和最大 output-token 用量：出现预算耗尽时会压低验收进度、显示阻塞并可转任务，提醒用户先收窄上下文、拆分阶段或减少无效输出后再跑最小验证样本；它同样不自动改预算、不自动重跑 workflow。验收缺口会按 `阻塞` / `待补` / `扩展` 排序和标记，critical soak incident、预算耗尽、failed E2E gate 这类会先展示，避免用户把长任务事故或性能/成本失控误当成普通补样本。卡片可一键复制 Markdown 验收报告，内容包含当前指标、必需项、验收结论、证据等级、来源分布、审计索引、复核协议、验收矩阵、每条跑道的证据 checklist、缺口、守门状态、最近 evidence provenance、长跑审计事故 / timeline 和推荐下一步，供人工 / Claude Code / PR review 复核；审计索引先给出 `acc-xxxxxxxx` 形式的快照 ID，再集中列出领域、控制面样本、样本新鲜度、预算、Gate 快照、Evidence IDs 和 Soak 窗口，方便 reviewer 对齐报告与当前工作台状态。卡片、复制报告和「采样清单」任务使用同一个 review context 生成快照 ID，避免同一轮验收材料出现两个 ID。该快照 ID 是基于报告摘要、gate 状态、evidence id、来源分布与 soak 计数的非安全确定性指纹，用于比较两份验收材料是否来自同一批可见状态，不替代真实 evidence 或完整性签名。复核协议明确只有 `可验收` 才能作为最终验收证据、转任务/按钮点击不能替代真实 evidence、长任务必须复核 Gate / Soak、连接器 E2E 必须来自测试账号或沙箱数据并包含执行/复核/回滚说明。复制动作只读，不创建 evidence、不修改 gate，且不展开完整 `sourceMetadata` 或大段私密正文。每个未通过必需项、未完成样本跑道与验收缺口都可由用户显式「转任务」；只要存在验收缺口、未通过必需项或非 optional 的样本跑道未完成，即使只有一个扩展缺口，也可一键创建「采样清单」任务，清单会写入当前指标、验收结论、证据等级、来源分布、审计索引、复核协议、验收进度、最近样本年龄、输出预算状态、必需项逐项状态、验收矩阵、每条跑道的证据 checklist / 刷新目标、带严重度标签的验收缺口和采样动作，均复用 `create_session_task` 落入 TaskProgressPanel。

验收跑道的「转任务」会生成可直接执行的采样 checklist，而不是一句提醒：任务正文包含当前状态、采样动作、必须记录的证据、完成后要刷新的 Gate / Soak / 报告目标。Workflow / Loop / Campaign 跑道强调终态、trace、失败恢复与 retry/cancel 证据；连接器 E2E 跑道强调测试账号或沙箱数据、读取 -> 草稿 -> 批准 -> 执行 -> 执行后复核的完整闭环；跨领域跑道强调 Goal / Workflow / Context Retrieval / Domain Quality 都能读取通用领域模板，证明能力不只适用于 coding 场景。该任务仍只是 TaskProgressPanel 里的用户可见工作项，不自动调用连接器、不自动重跑 workflow、不改变验收百分比。
...
- 「运行稳定性」卡片调用 `evaluate_domain_operational_gate({ sessionId, windowDays: 14 })`，显示 workflow 完成/活跃、loop 成功/失败、campaign 活跃、最长未排空时长、阻塞 check 和 recommended next steps；失败 / 样本不足 check 行与稳定性建议均可显式「转任务」，只创建可跟踪 task，不自动 retry、cancel、approve 或修改 gate 结果。
- 「长跑审计」卡片调用 `generate_domain_soak_report({ sessionId, windowDays: 14, maxItems: 8 })`，显示样本数、critical/warning incidents、最长 workflow drain、已闭环/未闭环审批等待、owner control intervention、恢复次数、output-token budget 消耗/耗尽信号、最近 timeline、Soak Report recommended next steps 和最近事故建议，并可复制 Soak Markdown 报告用于复盘 / review；推荐建议与事故行的「转任务」按钮复用 `create_session_task`，把样本不足、未排空、审批悬空或具体事故处理建议落成可跟踪 task。
...
- 「连接器 E2E」卡片提供「记录执行」和「记录复核」真实样本入口：用户必须填写执行结果或执行后读回复核文本，分别写入 `connector_action_executed` / `connector_action_verified` evidence。卡片会根据 Gate summary 显示当前采样步骤：缺批准时先提示补批准；有批准但无执行时只开放「记录执行」；执行证据存在或本面板刚记录执行后，才开放「记录复核」；执行和复核都有后显示真实样本已闭环并提示处理剩余回滚 / 交付缺口。它只记录已经发生的外部动作结果，不发起连接器调用、不补造 result id、不把空样本算作通过；E2E check 行也可「转任务」，把缺执行结果、缺执行后复核、缺回滚等问题落入 TaskProgressPanel。
...
- 工作台只聚合 owner-plane 读模型和已有显式动作按钮；写路径仅限用户显式点击候选行「摘要」、「确认」、「证据」、「冲突」/「转任务」、工作台「下一步」/「转任务」、交付守门「复核产物」/「导出复核」/「可交付确认」/「脱敏复核」、外部动作守门「批准动作」/「记录回滚」、连接器 E2E「记录执行」/「记录复核」、长跑事故「转任务」、守门 check「转任务」或需复核 evidence「转任务」后记录当前 session evidence / task / quality run，不自动创建 WorkflowRun、不运行 loop、不 retry campaign、不访问连接器、不发送/分享/导出内容。
...
GUI 上，Workspace「领域复核」区块内新增「交付守门」卡片，自动随会话加载、回合结束和手动刷新更新。用户无需记命令即可看到最终产物是否已创建、是否复核、是否存在敏感来源或待脱敏证据；失败或样本不足的 check 行、以及需复核 evidence 行都可显式「转任务」，把缺复核、缺产物、敏感来源或脱敏缺口落入 TaskProgressPanel 追踪。

同一卡片提供三种 owner-side 显式确认：「导出复核」、「可交付确认」、「脱敏复核」。点击后调用既有 `record_domain_evidence` 写入 `artifact_reviewed` evidence，`sourceMetadata.sourceType="artifact_export_guard_confirmation"`，并只写入用户点击的 marker：`exportReview=true`、`exportReady=true` 或 `redactionChecked=true`。这不是新的授权平面，也不会静默修改原 evidence 的 `redactionStatus`；`pending|sensitive` 仍由 Artifact Export Guard 重新计算并保持 fail closed。
...
Workspace「通用任务工作台」也会在当前 session scope 下调用同一个 Gate，并展示「连接器 E2E」卡片。用户能直接看到连接器输入、草稿、批准、执行结果、执行后复核、回滚、下层 Action Guard / Export Guard 状态、阻塞 check 和相关 evidence；执行结果与执行后复核可由用户显式填写并写回标准 evidence，随后刷新 E2E Gate / 真实样本验收 / Soak Report 输入。Autonomous Readiness 会把该 gate 的 failed / insufficient_data 纳入整体健康状态，并提供「查看 E2E」快捷入口。普通无连接器动作的会话不会因为全局缺样本而被强行标红，只有已有连接器证据、动作、toolName、connector 或 action scope 时才把 E2E 缺口推给用户处理。
