...
Workspace「通用任务工作台」也会在当前 session scope 下调用同一个 Gate，并展示「连接器 E2E」卡片。用户能直接看到连接器输入、草稿、批准、执行结果、执行后复核、回滚、下层 Action Guard / Export Guard 状态、阻塞 check 和相关 evidence；Autonomous Readiness 会把该 gate 的 failed / insufficient_data 纳入整体健康状态，并提供「查看 E2E」快捷入口。普通无连接器动作的会话不会因为全局缺样本而被强行标红，只有已有连接器证据、动作、toolName、connector 或 action scope 时才把 E2E 缺口推给用户处理。
