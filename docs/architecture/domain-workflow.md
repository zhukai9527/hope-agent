...
- 「连接器 E2E」卡片提供「记录执行」和「记录复核」真实样本入口：用户必须填写执行结果或执行后读回复核文本，分别写入 `connector_action_executed` / `connector_action_verified` evidence。它只记录已经发生的外部动作结果，不发起连接器调用、不补造 result id、不把空样本算作通过。
