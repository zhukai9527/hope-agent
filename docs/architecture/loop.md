...
已归档的 Loop v2 设计决策只作为后续边界，不能被当成当前已实现能力：
...
- 后续可绑定 Goal criteria，但依赖 Goal v2 的 stable criteria model。Goal criteria 删除或修改后，相关 Loop 应进入 needs-rebind / blocked，而不是继续静默运行。
