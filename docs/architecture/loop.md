...
Loop 复用现有 Cron 调度器，不另起 scheduler。每个 Loop schedule 都有一个受控 Cron job；Cron 负责可靠 tick、primary-only、并发上限、启动恢复、失败退避、无人值守权限面，Loop store 负责 session 归属、Goal / Goal criteria 绑定、执行策略、预算、次数、状态和可审计 trace。
...
当前活跃的 [Loop v2 路线图](../roadmap/loop-v2-roadmap.md) 只作为后续边界，不能被当成当前已实现能力：

- Loop v2 的方向是把 Loop 从“能触发”升级为可靠、可治理、可解释的持续推进器，核心问题是“为什么继续跑、下次何时跑、有没有推进 Goal、连续无进展时如何停止或降频”。
- Loop 仍只表示持续触发器，不重新承载执行强度；执行强度继续归 `/mode` / Execution Mode，具体执行继续归 Workflow。
- 后续 Loop Center 可替代当前 inline 小区块，提供 active / paused / blocked / completed / cancelled 分组、“查看更多 Loop”、run detail、next run、last result、budget、blocked reason、pause/resume/stop/run now/edit budget。
- 已支持 Goal criteria durable 绑定和 workflow strategy 继承；后续仍需要在 Goal criteria 删除或修改后，把相关 Loop 提升为 needs-rebind / blocked，而不是继续静默运行。
- 后续 Progress Guard 应优先基于 durable evidence delta 判定 `progressed` / `weak_progress` / `no_progress` / `blocked` / `failed`，不能只靠 LLM 自评；连续无进展应 backoff、blocked 或 ask user。
- 后续 Event-triggered Loop 可先接内部 EventBus，如 workflow terminal、goal state、task state；外部 webhook、file watcher、CI provider、connector object change streaming 进入更后续。
- 后续 Workflow strategy 可观察性应让 Loop Run Detail 与 Workflow Run Detail 双向跳转，并把 workflow terminal state、approval、blocked reason 和 evidence delta 映射回 Loop run outcome。
- 成本预算精确统计仍等待 provider cost ledger；在此之前 `cost_budget_micros` 继续保持保守限制，避免给用户错误安全感。
- Condition workflow 仍等待 Workflow terminal event 能反写 condition result 后再放开，不能把 until loop 伪装成 workflow 完成。

这些边界用于保证后续增强不推翻当前已实现契约：Loop 管触发和调度治理，不拥有 Goal 完成标准，也不绕过 Workflow、权限、预算和无痕红线。
