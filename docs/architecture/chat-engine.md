...
Axum 因 HTTP 客户端断开而丢弃 handler future、导致 Chat Engine 没机会异步
finalize 时，把仍非终态的 `chat_turns` 同步标为 `interrupted/runtime_cancel` 并
广播 `chat:stream_end`。因此 owner API / Web 入口不得出现
`active=false` 但 latest `chat_turn.status=running` 的半悬挂状态；如果模型或工具
仍需继续，必须由新的 durable wakeup / background injection 明确接管，而不是依赖
已断开的 HTTP request future。
