...
`deferredTools.enabled` 默认 **true**，`toolNames` 预置低频、大 schema、场景化工具。真正进入 deferred 池的内置工具仍必须同时满足两个条件：工具定义的 `default_deferred=true`，且名称出现在 `deferredTools.toolNames`。因此 Core 文件 / shell / task 等基础工具始终 eager；用户可以在设置中移除某个工具名，让它恢复 eager 注入，也可以把 `enabled=false` 作为全局关闭。
...
| 跨会话 | `agents_list`, `sessions_list`, `session_status`, `sessions_search`, `sessions_history`, `sessions_send`, `peek_sessions` |
| 技能入口 | `skill`（`Core::Meta`，恒注入） |
| 运行时控制 | `runtime_cancel`（`Core::Meta` + `internal = true`，async_job / subagent / process / cron 统一取消入口） |

此外 `tool_search` 和 `job_status` 两个 `Core::Meta` 元工具按 feature flag 条件注入：前者只在 (`deferredTools.enabled=true` 且 `toolNames` 非空) 或存在 deferred MCP server 时注入，后者只在 `asyncTools.enabled` 打开时注入。默认配置下 `tool_search` 会注入，因为默认 deferred 清单非空。

### Deferred（按需通过 `tool_search` 发现）

支持 deferred 的内置工具由 `ToolDefinition::supports_deferred()` 标记，当前主要是低频/高 schema 成本的 Standard / Configured 工具。默认 `toolNames` 由 `DeferredToolsConfig::default()` 预置，dispatcher 只对名单内且支持 deferred 的工具返回 `InjectDeferred`。

| 子域 | 默认 deferred 工具 |
|------|------|
| 浏览器 / 桌面 | `browser`, `mac_control` |
| 多模态 | `image`, `pdf` |
| 场景化查询 / 上报 | `get_weather`, `issue_report` |
| 设置备份 | `list_settings_backups`, `restore_settings_backup`（`get_settings` / `update_settings` 不在默认 deferred 候选——为了主 agent 主动改配置时随时可用）|
| Knowledge / Memory 编排 | `knowledge_recall` |
| 多 Agent 编排 | `team`, `acp_spawn` |
| 天气 | `get_weather` |
...
返回结果包含完整 provider schema 所需的 `parameters`，同时带 `metadata`、`tier`、`internal`、`concurrent_safe`、`async_capable`、`defer_capable`、`globally_configured` 等摘要。模型通过 `tool_search` 发现 deferred 工具后，可以看到工具效果、风险、输入形态和结果形态，而不是只靠自然语言 description 猜测。
