...
| 工具 | 标记 | 说明 |
|------|------|------|
| `mac_control` | deferred | 原生 macOS 桌面控制能力的只读骨架。Phase 1 仅支持 `action=status|permissions`，返回 bridge / platform / permission readiness；截图、AX snapshot、点击、输入、窗口、App、菜单等突变能力留到后续阶段。桌面 Tauri 注册 bridge，server/headless 返回 `supported=false`。 |
