...
| 工具 | 标记 | 说明 |
|------|------|------|
| `mac_control` | deferred | 原生 macOS 桌面控制能力。当前支持 `action=status|permissions|snapshot|elements|wait|apps|windows|act|menu|clipboard|dialog`；`snapshot` 返回前台 App / 窗口 / AX 元素摘要，`includeScreenshot=true` 可落盘 display/window JPEG 并打开右侧镜像，`elements.find` 只读返回排序 AX 候选、score 和 reasons，`wait` 支持 `present|gone` 轮询 AX snapshot。`apps` 支持 list/frontmost/installed/search/activate/launch/quit；`windows` 支持 frontmost/all 窗口发现与 list/focus/move/resize/minimize/close；`act` 支持 dry_run/perform_action/click/click_point/double_click/right_click/type/paste/set_value/hotkey/scroll/drag，其中 perform_action 只允许白名单 AX action 且目标必须在 `actions[]` 声明支持；`menu` 支持 app/system 两种 scope 的 list/click；`clipboard` 支持 get/set/clear UTF-8 文本且需审批；`dialog` 支持 inspect/accept/dismiss。桌面 Tauri 注册 bridge，server/headless/HTTP 返回 `supported=false`。 |
...
- `dialog.inspect` 返回当前前台 App 的 dialog/sheet 摘要，`dialog.accept/dismiss` 可用 `buttonText` 精确按钮。
- 只读 op：`status`、`permissions`、`snapshot`、`elements.find`、`wait`、`apps.list/frontmost/installed/search`、`windows.list`、`act.dry_run`、`menu.list`、`dialog.inspect`。
- 普通突变 op 进入审批：`apps.activate/launch`、`windows.focus/move/resize/minimize`、除 `dry_run` 外的 `act.*`、`menu.click`、`dialog.dismiss`。
- 高风险突变 op 进入严格审批且禁用 Allow Always：`apps.quit`、`windows.close`、`dialog.accept`、危险菜单路径（delete / trash / reset / discard 等中英文关键词）。
