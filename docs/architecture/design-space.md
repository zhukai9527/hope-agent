...
单一 `design` 工具（`internal: true`，按 action 路由；生成类可声明 `BackgroundPolicy::GenericJob`），供模型自主创建/迭代设计。
...
- **[后台任务](background-jobs.md)**：生成/导出/提取若需后台化则标 `BackgroundPolicy::GenericJob`，走 `JobManager` 统一后台模型（不起平行 API）。
