...
> 返回 [技术文档索引](../README.md)
>
> 状态：Phase 7.4 已实现，Phase 7.5 已接入 Domain Learning。本文记录 `ha-core::domain_quality` 的最终技术事实：通用领域 review / verification run、check、event、Goal evidence 阻塞语义、Domain Learning 输入信号、owner API 与 Workspace「领域复核」交互。
...
| 表 | 说明 |
| --- | --- |
| `domain_quality_runs` | 一次领域质量复核。字段包括 session、goal、domain、template、state、summary、stats、error、created/updated/completed。 |
...
| `templateId` | 可选；指定 Domain Workflow template。 |
...
1. `templateId` 指定的 template。
2. 显式 `domain`。
3. `artifactKind` / active Goal objective / completion criteria 的关键词推断。
4. 当前 session / goal 的 domain evidence 计数。
5. fallback 到 `writing`。
...
Phase 7.5 后，`coding_improvement::generate_coding_improvement_proposals()` 和 `distill_coding_improvement_proposals()` 会读取当前 session/project scope 内的 Domain Quality snapshot，并把领域质量结果转成 draft-only improvement proposal：
...
- 展示通过、缺失、需确认、建议四类计数。
- 展示最近 run summary、domain、template id。
...
- Coding Improvement Loop 的 `domain_learning_generates_reviewable_drafts_from_quality_runs` 覆盖 Research / Writing / Data Analysis / Inbox quality run 进入 Domain Learning proposal、draft apply 与 promotion preview。
