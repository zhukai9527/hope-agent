...
- 支持运行领域复核和刷新。
- 内嵌「交付守门」卡片，调用 `evaluate_domain_artifact_export_guard` 展示最终产物是否已创建、是否复核、是否存在 private/connector/sensitive/pending/redacted evidence，以及是否缺少显式 export review。
