...
| `knowledge_passive_recall` | `enabled` (default false), `topN` (default 5, 1–20), `maxChars` (default 800, 100–4000), `cacheTtlSecs` (default 120), `showSnippet` (default false). Read bridge ③: when enabled, each user turn injects the top accessible-KB note **titles** as an untrusted reference block (retrieval-only, no LLM). Off by default; incognito sessions and KBs the session isn't attached to never surface. |
...
| `knowledge_search` | `textWeight` (default 0.4), `vectorWeight` (default 0.6), `rrfK` (default 60, 1–1000), `mmrLambda` (default 0.7, 0–1), `candidateMultiplier` (default 3, 1–10) | Hybrid `note_search` ranking: keyword (BM25) + semantic (vector) over note chunks → RRF fusion → MMR diversity re-rank. Pure query-time, **no reindex**. Weights' ratio sets keyword↔semantic balance (raise textWeight for code/jargon, vectorWeight for meaning; vector arm is inert if the KB vector model is off); `rrfK` smooths fusion; `mmrLambda` trades relevance (1.0) vs. variety (lower); `candidateMultiplier` sizes the pre-MMR pool. Defaults suit most libraries — send the defaults above to restore them. Tunable here or in Settings → Knowledge → Advanced (with a one-click Restore defaults). |
| `knowledge_maintenance` | `enabled`, `idleTrigger.{enabled,idleMinutes}`, `cronTrigger.{enabled,cronExpr}`, `manualEnabled`, `tasks.{autoLink,orphanRescue,frontmatterFill,dedupMerge,knowledgeGap,autoTag,mocUpkeep,memoryToNote}`, `autoApprove`, `maxProposalsPerCycle`, `dedupSimilarity`, `llmTimeoutSecs`, `llmMaxTokens` | Layer-2 autonomous maintenance: background cycles scan knowledge bases and queue note-maintenance **proposals** for owner review (knowledge view → maintenance panel). ⚠️ `enabled` lets background cycles run; `autoApprove=true` writes approved-free changes **directly to the user's notes** (skipping review). Both are approval-policy / autonomous-write switches — confirm with the user before enabling either. Takes effect on the next cycle. |
| `sprite` | `enabled`, `proactive`, `triggers.{editIdle,noteOpen,conversation,periodic,paste}`, `idleEditSecs`, `minChangeChars`, `periodicSecs`, `pasteMinChars`, `cooldownSecs`, `maxPerSessionPerHour`, `senses.{doc,edit,conversation,memory,awareness}`, `maxTokens`, `timeoutSecs` | Knowledge-space sprite / inspiration mode: while the user works on a note, it makes a **bounded, proactive LLM call** on several occasions and may surface a transient suggestion bubble in the chat panel. ⚠️ `enabled` makes unprompted LLM calls — has a cost (users normally toggle it from the chat panel's sprite button, not here). `proactive` (default true) biases speaking vs. staying quiet; `triggers.*` toggle the occasions (edit-idle / note-open / after a chat turn / periodic while writing / large paste); `idleEditSecs`+`minChangeChars` gate edit-idle, `periodicSecs` the periodic streak, `pasteMinChars` the paste trigger; `cooldownSecs` / `maxPerSessionPerHour` throttle overall, `senses.*` toggle which context is fused. The persona prompt is built-in (English), not free-text configurable. Confirm with the user before enabling. Incognito sessions never trigger it. Only fires while the knowledge-space AI chat panel is open. |
