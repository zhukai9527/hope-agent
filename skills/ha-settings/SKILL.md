...
| `sprite` | `enabled`, `idleEditSecs`, `minChangeChars`, `cooldownSecs`, `maxPerSessionPerHour`, `senses.{doc,edit,conversation,memory,awareness}`, `maxTokens`, `timeoutSecs` | Knowledge-space sprite / inspiration mode: while the user edits a note, an edit-idle trigger makes a **bounded, proactive LLM call** and may surface a transient suggestion bubble in the chat panel. ⚠️ `enabled` makes unprompted LLM calls — has a cost (note: users normally toggle it from the chat panel's sprite button, not here); `cooldownSecs` / `maxPerSessionPerHour` throttle it, `senses.*` toggle which context is fused. The persona prompt is built-in (English) and not configurable. Confirm with the user before enabling. Incognito sessions never trigger it. |
