...
| `async_tools` | `enabled`, `autoBackgroundSecs`, `maxJobSecs`, `maxConcurrentJobs` (default 8, `0` = unlimited; cap on concurrent `run_in_background` jobs — each holds an OS thread; at the cap a new background request returns an error result telling the model to wait/poll `job_status` or run synchronously), `inlineResultBytes`, `retentionSecs`, `orphanGraceSecs`, `jobStatusMaxWaitSecs` |
