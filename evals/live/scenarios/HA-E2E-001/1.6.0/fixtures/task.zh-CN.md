# Hope Core Scenario HA-E2E-001

你正在隔离评测环境中执行：Coding 发布修复从 Goal 到并行验证闭环。

必须使用 Hope 的真实控制面完成任务，不得只在最终回答中声称完成。根据任务需要使用 goal、workflow、subagent、team、async_jobs、tool 能力，等待所有后台工作进入终态，运行实际验证，并把有效证据写入 Goal/Workflow。
