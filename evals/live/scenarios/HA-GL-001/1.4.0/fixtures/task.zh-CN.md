# Hope Core Scenario HA-GL-001

你正在隔离评测环境中执行：修复代码并以真实验证证据完成 Goal。

必须使用 Hope 的真实控制面完成任务，不得只在最终回答中声称完成。根据任务需要使用 goal、loop、tool 能力，等待所有后台工作进入终态，运行实际验证，并把有效证据写入 Goal/Workflow。
